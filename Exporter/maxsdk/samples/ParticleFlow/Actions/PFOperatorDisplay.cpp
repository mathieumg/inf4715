...
typedef std::map<Mesh*, Mesh*> MappingData;	
...
public:
	...
	~MeshMap()
	{
		for (MappingData::iterator it = mMeshMap.begin();
			it != mMeshMap.end();
			++it)
		{
			delete it->second;
	...
	MappingData& GetMapping() {return mMeshMap;}
private:
	MappingData mMeshMap;
};
...
private:
	...
	PFOperatorDisplayExtensionHelper(PFOperatorDisplay* pOwner):mpOwner(pOwner) {}
...
bool PFOperatorDisplayExtensionHelper::UpdateDisplay(
	const MaxSDK::Graphics::GenerateMeshRenderItemsContext& generateRenderItemsContext, 
	TimeValue t, 
	IObject* pCont, 
	Object* pSystem, 
	...
	Tab<Matrix3> &matrices) 
{
	...
	float visPercent = GetPFFloat(mpOwner->pblock(), kDisplay_visible, t);
	...
	bool subSelPartMode = (/*TODO [Nitrous] psNode->Selected() &&*/ (pSystem->GetSubselState() == 1));
	if (subSelPartMode) {
	...
	if (chSelect != NULL) {
		for(i=0; i<count; i++) {
			if (doVisibleFilter)
				if (mpOwner->isInvisible(chVisibleR->GetValue(i))) continue;
	...
	if ((ordCat == geomCat) || (selCat == geomCat))
	{
		...
		for(int runType = 0; runType < 2; runType++)
		{
			...
			*/
			...
			typedef std::map<Mesh*, Matrix3Array> InstanceData;
			InstanceData meshParts;
			MeshMap ModifiedMesh;
			int blockCount = block->Count() ;
			for(i=0; i<blockCount; i++) {
				index = (*block)[i];
				Mesh* mesh = NULL;
				if (chShape != NULL)
					mesh = const_cast <Mesh*>(chShape->GetValue(index));
				Mesh* pModified = mesh;
				if (pModified) {
				  	Matrix3 gwTM;
					gwTM.IdentityMatrix();
				...
				//	 set vertex color
					...
					*/
					if ((chMtlIndex != NULL) || (chMapping != NULL)) {
						MappingData::iterator it = ModifiedMesh.GetMapping().find(mesh);
						if (it == ModifiedMesh.GetMapping().end())
						{
							pModified = new Mesh(*mesh);
							bool assignedMtlIndices = AssignMeshMaterialIndices(pModified, chMtlIndex, index);
							bool assignedMapping = AssignMeshMapping(pModified, chMapping, index);
							if (assignedMtlIndices || assignedMapping) 
							{
								ModifiedMesh.GetMapping()[mesh] = pModified;
					...
					meshParts[pModified].push_back(gwTM);
				}
			}

			MaxSDK::Graphics::RenderItemHandleArray tempMeshRenderItemArray;

			for (InstanceData::iterator it = meshParts.begin(); it != meshParts.end(); ++it)
			{
				if (NULL != it->first)
				{
					it->first->GenerateRenderItems(tempMeshRenderItemArray,generateRenderItemsContext);	
				}

				MaxSDK::Graphics::VertexBufferHandle hInstanceData = 
					MaxSDK::Graphics::GenerateInstanceData(&(it->second)[0], it->second.size());

				for (size_t i = 0; i < tempMeshRenderItemArray.GetNumberOfRenderItems(); ++i)
				{
					MaxSDK::Graphics::RenderItemHandle hInstanceItem;
					if (MaxSDK::Graphics::GenerateInstanceRenderItem(hInstanceItem, tempMeshRenderItemArray.GetRenderItem(i), hInstanceData))
					{
						renderItemHandleArray.AddRenderItem(hInstanceItem) ;
					}
					else
					{
						renderItemHandleArray.AddRenderItem(tempMeshRenderItemArray.GetRenderItem(i));
					}
				}
			}
			Matrix3 matIdentity;
			matIdentity.IdentityMatrix();
			for (size_t i = 0; i < renderItemHandleArray.GetNumberOfRenderItems(); ++i)
			{
				// Save the transform matrix for all of the render item in the Array.
				matrices.Append(1, &matIdentity);
			}
		}
	}
	return true ;
...
int PFOperatorDisplay::Display(IObject* pCont,  TimeValue time, Object* pSystem, INode* psNode, INode* pgNode, ViewExp *vpt, int flags)
{
	...
	{ // show born index along with each particle
		...
		if (num > 0) {
			...
			for(i=0; i<num; i++) {
				index = blockNon[i];
				_stprintf(numText, _T("%d"), chID->GetParticleBorn(index)+1); // indices shown are 1-based
				pos[0] = chPos->GetValue(index);
				gw->text(pos, numText);
			}
		}
...
void PFOperatorDisplay::MaybeEnlargeViewportRect(IObject* pCont, TimeValue time, GraphicsWindow *gw, Rect &rect)
{
	...
	TCHAR dummy[256];
	SIZE size;
	_stprintf(dummy, _T("%d"), maxNumber);
