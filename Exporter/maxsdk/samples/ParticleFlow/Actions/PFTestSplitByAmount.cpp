...
//|							From BaseObject									 |
//+>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>+
const TCHAR* PFTestSplitByAmount::GetObjectName()
{
...
bool PFTestSplitByAmount::HasDynamicName(TSTR& nameSuffix)
{
	...
	case kSplitByAmount_testType_fraction:
		...
		if (isAnimated) {
			...
			_stprintf(buf,_T("%d%%"),int(pblock()->GetFloat(kSplitByAmount_fraction, 0)*100.0f + 0.5f));
			nameSuffix = buf;
		}
		break;
	case kSplitByAmount_testType_everyN:
		...
		if (isAnimated) {
			...
			_stprintf(buf,_T(" %d"),n);
			nameSuffix += buf;
			if (n == 1) {
				nameSuffix = GetString(IDS_ALL);
			} else {
				nameSuffix = GetString(IDS_EVERY);
				_stprintf(buf,_T(" %d"),pblock()->GetInt(kSplitByAmount_everyN, 0));
				nameSuffix += buf;
	...
	case kSplitByAmount_testType_firstN:
		...
		_stprintf(buf,_T(" %d"),pblock()->GetInt(kSplitByAmount_firstN, 0));
		nameSuffix += buf;
		break;
