...
class BKeySizeValidator {
	BKeySizeValidator() {}  // not creatable
	MaxSDK::Util::StaticAssert< (sizeof(BKey) == sizeof(TimeValue)+sizeof(DWORD)+sizeof(float)) > validator;
};
				...
				if (keys.length()==0) {
					if (Animating() && 
							(	GetCOREInterface10()->GetAutoKeyDefaultKeyOn() && 
								t!=0 && 
								!(t == GetTicksPerFrame() &&  GetCOREInterface10()->GetAutoKeyDefaultKeyTime()==GetTicksPerFrame() )
							)
						)
					{
				   BKey k;
