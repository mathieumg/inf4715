...
public:
	...
	{
		if (mType == kSlaveState && GetOwner()) {
			Reactor* r = (Reactor*)GetOwner();
			if (r) {
				DbgAssert(GetIndex() < r->slaveStates.Count());
				if (GetIndex() < r->slaveStates.Count())
					return r->slaveStates.Addr(GetIndex());
			}
		}
		return NULL;
	}
...
class SlaveListSizeRestore : public RestoreObj {
	public:
		...
		TSTR Description() {return _T("Slave Count");}
	};
...
class MgrListSizeRestore : public RestoreObj {
	public:
		...
		TSTR Description() {return _T("Manager Size");}
	};
