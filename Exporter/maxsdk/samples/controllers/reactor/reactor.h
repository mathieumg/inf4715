...
public:
	...
	AnimEntry(const TCHAR* n, int l, Tab<SubAnimPair*> p) :
...
//	AnimEntry	entry;
	...
	bool		add_subanim_params(PickObjectCallback* cb, Tab<AnimEntry*>& wparams, const TCHAR* name, Tab<ReferenceTarget*>& parents, int level, Tab<ReferenceTarget*>& roots);
...
private:
	virtual void SetReference(int i, RefTargetHandle rtarg) { client = rtarg; }
public:
	...
	MasterState* AddState(const TCHAR* name, int &id, bool setDefaults, TimeValue t);
...
class Reactor : public IReactor, public ResourceMakerCallback , public ILockedTrackImp {
	...
	public:
		...
		SlaveState*	CreateReactionAndReturn(BOOL setDefaults = TRUE, const TCHAR *buf=NULL, TimeValue t = GetCOREInterface()->GetTime(), int masterID = -1);
		BOOL	CreateReaction(const TCHAR *buf=NULL, TimeValue t = GetCOREInterface()->GetTime());
		...
		const TCHAR*	getReactionName(int i);
