...
bool ReactionDlg::AddSlaveRow(HWND hList, ReactionListItem* listItem, int itemID)
{
	...
	if (buf.Length() > 0)
	{
		...
		TSTR buf2 = _T("     ");
		buf2 += buf;
		slaveItem.pszText = const_cast<TCHAR*>(buf2.data());
				...
				for(int k = 0; k < set->SlaveCount(); k++)
				{
					...
					if (r) {
						for(int x = 0; x < r->slaveStates.Count(); x++)
						{
							...
							{
								listItem = new StateListItem(StateListItem::kSlaveState, NULL, r, x);
