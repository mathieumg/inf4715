    ...
    case WM_INITDIALOG:
    {
		...
		for (int i=0; i<info->layerNames.Count(); i++)
		{
			SendDlgItemMessage(hDlg,IDC_LAYERS,CB_ADDSTRING,0,
					(LPARAM)(LPCTSTR)info->layerNames[i]->data());
		}
...
class RemoveLayerItemRestore : public RestoreObj {
	public:   		
		...
		TSTR Description() {return _T("Layer control remove item");}
	};
...
class CopyRestore : public RestoreObj, public ReferenceMaker {	
	public:	
		...
		void Restore(int isUndo) {}
		void Redo() {}
		TSTR Description() {return _T("Layer Control Copy");}
		int NumRefs() {return 1;}
		RefTargetHandle GetReference(int i) {return cont;}
private:
		virtual void SetReference(int i, RefTargetHandle rtarg) {cont=(Control*)rtarg;}
public:
			...
			case REFMSG_CHANGE:
				...
				if(pblock)
				{
					...
					if(inNotify==false)
					{
						...
						{
							int param_index = i - kX +1;
							LayerOutputControl * control = (LayerOutputControl*) pblock->GetControllerByIndex(param_index,0);
							if(control)
...
LayerControl::~LayerControl()
{
	...
	if(pblock)
	{
		...
		{
			int index = i-kX +1;
			LayerOutputControl* control = (LayerOutputControl*) pblock->GetControllerByIndex(index,0);
			if(control)
				control->lC = NULL;
		}
	}
...
TSTR LayerControl::SubAnimName(int i)	
{
	...
	if(man->GetFilterActiveOnlyTrackView()==FALSE)
	{
		...
		else if(i<conts.Count()*2)
		{
			...
			if (mLC&&mLC->GetName() && mLC->GetName().Length()>0) 
			{
				name =  TSTR(GetString(IDS_AF_LIST_WEIGHT)) + _T(": ") + mLC->GetName();
			}
			else if (conts[i])
			{
				TSTR cname; conts[i]->GetClassName(cname);
				name = TSTR(GetString(IDS_AF_LIST_WEIGHT)) + _T(": ") + cname;
			} 
	...
	else
	{
		...
		else if(i==1)
		{
			...
			if(mLC&&mLC->GetName() &&mLC->GetName().Length()>0)
			{
				name = mLC->GetName();
				name += _T(" ");
				name += GetString(IDS_AF_LIST_WEIGHT);
			}
			else if(conts[active])
			{
				conts[active]->GetClassName(name);
...
TSTR AssignControllerRestore::Description() { return _T("Assign Controller"); }
...
{ 
	public:
		TSTR GetLocalName(ReferenceMaker* owner, ParamID id, int tabIndex) {
			...
			return GetString(IDS_MASTERLAYER);
		}
};
	...
	case WM_COMMAND:
		...
		case IDOK:
			if (which>=0)
			{
				...
				while (_tcslen(buf) > 0 && (buf[_tcslen(buf)-1] == _T(' ') || buf[_tcslen(buf)-1] == _T('\t')))
					buf[_tcslen(buf)-1] = _T('\0');
				while (_tcslen(bufp) > 0 && (bufp[0] == _T(' ') || bufp[0] == _T('\t')))
					bufp++;
				if (_tcslen(bufp) == 0)
				{
