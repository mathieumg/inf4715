...
class ListControlRestore : public RestoreObj {
	public:
		...
		TSTR Description() {return _T("List Control");}
	};
...
class RemoveItemRestore : public RestoreObj {
	public:   		
		...
		TSTR Description() {return _T("List control remove item");}
	};
...
class ListSizeRestore : public RestoreObj {
	public:
		...
		TSTR Description() {return _T("List size");}
	};
...
class CutRestore : public RestoreObj, public ReferenceMaker {	
	public:	
		...
		void Restore(int isUndo) {}
		void Redo() {}
		TSTR Description() {return _T("List Control Cut");}
...
//		LB_GETCURSEL,0,0);
	...
	if (sel!=LB_ERR) {
		...
		iWeight->SetKeyBrackets(cont->pblock->KeyFrameAtTimeByID(kListCtrlWeight, GetCOREInterface()->GetTime(), sel));
	} else {
...
static MaxSDK::Array<ListCtrlWindow> listCtrlWindows;

static void RegisterListCtrl(HWND hWnd, HWND hParent, Control *cont)
{
	ListCtrlWindow rec(hWnd,hParent,cont);
	listCtrlWindows.append(rec);
}

static void UnRegisterListCtrlWindow(HWND hWnd)
{	
	for (int i=0; i<listCtrlWindows.length(); i++) {
		if (hWnd==listCtrlWindows[i].hWnd) {
			listCtrlWindows.removeAt(i);
			return;
		}
	}	
}
