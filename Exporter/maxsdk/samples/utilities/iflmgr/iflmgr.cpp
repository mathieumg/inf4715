...
void IFLMgr::Create ( HWND hWnd ) {
	...
	if (workpath[0])
		...
		ofn.lpstrInitialDir	= ip->GetAssetDir(0,MaxSDK::AssetManagement::kBitmapAsset);
...
void IFLMgr::Select ( HWND hWnd ) {
	...
	if (workpath[0])
		_tcscpy(path,workpath);
	else
		_tcscpy(path,ip->GetAssetDir(0,MaxSDK::AssetManagement::kBitmapAsset));
