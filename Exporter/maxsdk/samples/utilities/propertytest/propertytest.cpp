...
void PropertyTest::GetProperties(HWND hDlg)
	...
	for (int i=0; i<numProps; i++) {
		...
		if (pPropSpec->ulKind == PRSPEC_PROPID) {
			_stprintf(szBuf, _T("%ld"), pPropSpec->propid);
			}
		else {
			_tcscpy(szBuf, TSTR::FromOLESTR(pPropSpec->lpwstr));
			}
...
void PropertyTest::VariantToString(const PROPVARIANT* pProp, TCHAR* szString, int bufSize)
	...
	switch (pProp->vt) {
		case VT_BSTR:
		case VT_LPWSTR:
			_tcscpy(szString, TSTR::FromUTF16(pProp->pwszVal));
			break;
		case VT_LPSTR:
			_tcscpy(szString, TSTR::FromACP(pProp->pszVal));
			break;
#if !defined( WIN64 )
      ...
      case VT_I8:            
         ...
         break;
      case VT_INT:
         _stprintf(szString, _T("%ld"), V_INT(pProp));
         break;
		case VT_R4:
		...
		case VT_R8:
			...
			break;
		case VT_BOOL:
			_stprintf(szString, _T("%s"), pProp->boolVal ? GetString(IDS_PROPYES) : GetString(IDS_PROPNO));
			break;
		case VT_FILETIME:
			SYSTEMTIME sysTime;
