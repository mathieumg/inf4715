...
/* -------------------- dotNetClass  ------------------- */
// 
class dotNetClassClass : public ValueMetaClass
{
public:
	dotNetClassClass(const MCHAR* name);
	void collect();
	ScripterExport Value* apply(Value** arglist, int count, CallContext* cc=NULL);
};
extern dotNetClassClass dotNetClass_class;
...
public:
	...
	dotNetMethod(const MCHAR* pMethodName, DotNetObjectWrapper* pWrapper);
	~dotNetMethod();
