...
{
public:
				MSDispatch(IDispatch* ipdisp, const TCHAR* iname, ITypeInfo* ti=NULL);
				MSDispatch() { }
				~MSDispatch();
...
public:
	...
	MSDispatch	*m_dispatch; // LAM - 3/10/03 - defect 468513
				...
				MSComMethod() { }
				MSComMethod(const TCHAR* name, DISPID id, MSComObject* com_obj, CMethodInfo *pMethodInfo=NULL);
