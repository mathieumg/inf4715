...
{  
   ...
   _tcscat(buf, MaxSDK::GetResourceStringAsMSTR(IDS_NEEDS_ARG_OF_TYPE));
   for (int i=0; i < count; i++) 
   {
      _tcscat(buf, params[i].val->to_string());
      if (i < count-1) _tcscat(buf, _T(" | #"));
   }  
   _tcscat(buf, MaxSDK::GetResourceStringAsMSTR(IDS_GOT));
...
CurvePointsArray::sprin1(CharStream* s)
{
   ...
   {
      s->printf(MaxSDK::GetResourceStringAsMSTR(IDS_CPARRAY_FOR_DELETED_CURVE));
      return;
   }/**/
