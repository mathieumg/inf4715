...
bit_shift_cf(Value** arg_list, int count)
{
	...
	if (is_int64(arg_list[0]))
	{
		...
		range_check(shiftCount, -int64_num_bits, int64_num_bits, MaxSDK::GetResourceStringAsMSTR(IDS_BIT_INDEX_OR_SHIFT_COUNT_OUT_OF_RANGE));
		if (shiftCount > 0)
			val <<= shiftCount;
	...
	else if (is_intptr(arg_list[0]))
	{
		...
		range_check(shiftCount, -intptr_num_bits, intptr_num_bits, MaxSDK::GetResourceStringAsMSTR(IDS_BIT_INDEX_OR_SHIFT_COUNT_OUT_OF_RANGE));
		if (shiftCount > 0)
			val <<= shiftCount;
		else if (shiftCount < 0)
			val >>= (UINT) abs(shiftCount);
	...
	else
	{
		...
		range_check(shiftCount, -int_num_bits, int_num_bits, MaxSDK::GetResourceStringAsMSTR(IDS_BIT_INDEX_OR_SHIFT_COUNT_OUT_OF_RANGE));
		if (shiftCount > 0)
			val <<= shiftCount;
		else if (shiftCount < 0)
			val >>= (UINT) abs(shiftCount);
...
bit_set_cf(Value** arg_list, int count)
{
	...
	if (is_int64(arg_list[0]))
	{
		...
		mask <<= (whichBit-1);
		int int64_num_bits = sizeof(INT64) * 8;
		range_check(whichBit, 1, int64_num_bits, MaxSDK::GetResourceStringAsMSTR(IDS_BIT_INDEX_OR_SHIFT_COUNT_OUT_OF_RANGE));
	...
	else if (is_intptr(arg_list[0]))
	{
		...
		mask <<= (whichBit-1);
		int intptr_num_bits = sizeof(INT_PTR) * 8;
		range_check(whichBit, 1, intptr_num_bits, MaxSDK::GetResourceStringAsMSTR(IDS_BIT_INDEX_OR_SHIFT_COUNT_OUT_OF_RANGE));
	...
	else
	{
		...
		int int_num_bits = sizeof(int) * 8;
		range_check(whichBit, 1, int_num_bits, MaxSDK::GetResourceStringAsMSTR(IDS_BIT_INDEX_OR_SHIFT_COUNT_OUT_OF_RANGE));
