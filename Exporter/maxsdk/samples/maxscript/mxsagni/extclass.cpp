...
visible_class_instance (DataPair, "DataPair")
...
DataPairClass::apply(Value** arg_list, int count,  CallContext* cc)
{
	...
	else if (count == 5 && arg_list[0] ==&keyarg_marker)
	{
		...
		type_check(vl.v1_name, Name, _T("DataPair variable name"));
		type_check(vl.v2_name, Name, _T("DataPair variable name"));
...
NodeEventCallbackValue::CallHandler(Value *eventName, NodeKeyTab& nodes)
{
	...
	if (fn != &undefined)
	{
		...
		catch (MAXScriptException& e)
		{
			...
			MAXScript_signals = 0;
			show_source_pos();
			error_message_box(e, MaxSDK::GetResourceStringAsMSTR(IDS_NODEEVENTCALLBACK));
		}
		catch (...)
		{
			restore_current_frames();
			show_source_pos();
			error_message_box(UnknownSystemException (), MaxSDK::GetResourceStringAsMSTR(IDS_NODEEVENTCALLBACK));
		}
