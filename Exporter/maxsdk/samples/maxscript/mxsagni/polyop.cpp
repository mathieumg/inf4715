...
polyop_getVertFlags_cf(Value** arg_list, int count)
{
	...
	range_check(vertIndex+1, 1, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_VERTEX_INDEX_OUT_OF_RANGE));
...
polyop_getEdgeFlags_cf(Value** arg_list, int count)
{
	...
	range_check(edgeIndex+1, 1, nEdges, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_EDGE_INDEX_OUT_OF_RANGE)) 
...
polyop_getFaceFlags_cf(Value** arg_list, int count)
{
	...
	range_check(faceIndex+1, 1, nFaces, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_FACE_INDEX_OUT_OF_RANGE))
...
polyop_getVert_cf(Value** arg_list, int count)
{
	...
	int vertIndex =  arg_list[1]->to_int()-1;
	range_check(vertIndex+1, 1, poly->VNum(), MaxSDK::GetResourceStringAsMSTR(IDS_POLY_VERTEX_INDEX_OUT_OF_RANGE))
...
polyop_weldVerts_cf(Value** arg_list, int count)
{
	...
	range_check(v1+1, 1, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_VERTEX_INDEX_OUT_OF_RANGE)) 
	int v2=arg_list[2]->to_int()-1;
	range_check(v2+1, 1, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_VERTEX_INDEX_OUT_OF_RANGE)) 
...
polyop_weldEdges_cf(Value** arg_list, int count)
{
	...
	range_check(e1+1, 1, nEdges, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_EDGE_INDEX_OUT_OF_RANGE)) 
	int e2=arg_list[2]->to_int()-1;
	range_check(e2+1, 1, nEdges, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_EDGE_INDEX_OUT_OF_RANGE)) 
	EPoly* epi = NULL;
	if (owner) {
		epi = (EPoly*)((Animatable*)owner)->GetInterface(EPOLY_INTERFACE);
	}
...
polyop_setDiagonal_cf(Value** arg_list, int count)
{
	...
	range_check(v1+1, 1, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_FACE_VERTEX_INDEX_OUT_OF_RANGE)) 
	int v2=arg_list[3]->to_int()-1;
	range_check(v2+1, 1, nVerts, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_FACE_VERTEX_INDEX_OUT_OF_RANGE)) 
	EPoly* epi = NULL;
	if (owner) {
		epi = (EPoly*)((Animatable*)owner)->GetInterface(EPOLY_INTERFACE);
	}
...
polyop_cutEdge_cf(Value** arg_list, int count)
{
	...
	int nEdges = poly->ENum();
	range_check(edge1Index+1, 1, nEdges, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_EDGE_INDEX_OUT_OF_RANGE)) 
	range_check(edge2Index+1, 1, nEdges, MaxSDK::GetResourceStringAsMSTR(IDS_POLY_EDGE_INDEX_OUT_OF_RANGE)) 
...
// can have map verts without having map faces
	...
	checkMapChannel (poly, channel);
	range_check(index+1, 1, poly->M(channel)->numv, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_VERTEX_INDEX_OUT_OF_RANGE));
...
polyop_setMapFace_cf(Value** arg_list, int arg_count)
{
	...
	range_check(faceindex+1, 1, poly->M(channel)->numf, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_FACE_INDEX_OUT_OF_RANGE));
	...
	for (int i = 0; i < degree; i++) {
		int vertIndex=vertArray->data[i]->to_int()-1;
		range_check(vertIndex+1, 1, nMapVerts, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_MAP_VERTEX_INDEX_OUT_OF_RANGE))
		v[i]=vertIndex;
	}
...
polyop_setVDataChannelSupport_cf(Value** arg_list, int count)
{
	...
	range_check(channel+1, 1, MAX_VERTDATA, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTDATA_CHANNEL_OUT_OF_RANGE));
	EPoly* epi = NULL;
	if (owner) {
		epi = (EPoly*)((Animatable*)owner)->GetInterface(EPOLY_INTERFACE);
	}
...
polyop_getVDataValue_cf(Value** arg_list, int count)
{
	...
	int index=arg_list[2]->to_int()-1;
	range_check(index+1, 1, poly->vd[channel].Count(), MaxSDK::GetResourceStringAsMSTR(IDS_MESH_VERTDATA_INDEX_OUT_OF_RANGE));
...
polyop_setNumEDataChannels_cf(Value** arg_list, int count)
{
	...
	int numChannels=arg_list[1]->to_int();
	range_check(numChannels, 0, MAX_EDGEDATA, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_NUMEDGEDATA_OUT_OF_RANGE));
	Value* tmp = NULL;
	bool keep = bool_key_arg(keep, tmp, TRUE) == TRUE; 
	EPoly* epi = NULL;
	if (owner) {
...
polyop_setEDataChannelSupport_cf(Value** arg_list, int count)
{
	...
	int channel = arg_list[1]->to_int()-1;
	range_check(channel+1, 1, MAX_EDGEDATA, MaxSDK::GetResourceStringAsMSTR(IDS_MESH_EDGEDATA_CHANNEL_OUT_OF_RANGE));
	EPoly* epi = NULL;
	if (owner) {
		epi = (EPoly*)((Animatable*)owner)->GetInterface(EPOLY_INTERFACE);
	}
...
polyop_getEDataValue_cf(Value** arg_list, int count)
{
	...
	if (!poly->eDataSupport(channel))
		throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_VERTEXDATA_SUPPORT_NOT_ENABLED_FOR_CHANNEL), arg_list[1]);
	int index=arg_list[2]->to_int()-1;
	range_check(index+1, 1, poly->ed[channel].Count(), MaxSDK::GetResourceStringAsMSTR(IDS_MESH_EDGEDATA_INDEX_OUT_OF_RANGE));
