...
GetID(paramType params[], int count, Value* val, int def_val)
{
	...
	TCHAR buf[2056]; buf[0] = _T('\0');
	
	_tcscat(buf, MaxSDK::GetResourceStringAsMSTR(IDS_FUNC_NEED_ARG_OF_TYPE));	
	for (int i=0; i < count; i++) 
	{
		_tcscat(buf, params[i].val->to_string());
		if (i < count-1) _tcscat(buf, _T(" | #"));
	}	
	_tcscat(buf, MaxSDK::GetResourceStringAsMSTR(IDS_FUNC_GOT_ARG_OF_TYPE));
...
void 
ValueToBitArray(Value* inval, BitArray &theBitArray, int maxSize, const TCHAR* errmsg, int selTypesAllowed) {
...
void 
ValueToBitArrayM(Value* inval, BitArray &theBitArray, int maxSize, const TCHAR* errmsg, int selTypesAllowed, Mesh* mesh) {
...
set_up_spline_access(INode* node, int index)
{
	...
	range_check(index, 1, s->SplineCount(), MaxSDK::GetResourceStringAsMSTR(IDS_SHAPE_SPLINE_INDEX_OUT_OF_RANGE))
	return s;
}
...
_get_polyForValue(Value* value, int accessType, ReferenceTarget** owner, const TCHAR* fn) {
	...
	if (value->is_kind_of(class_tag(MAXNode))) {
		...
		if (accessType == MESH_WRITE_ACCESS && obj != base_obj)
			throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_CANNOT_CHANGE_POLY_WITH_MODIFIERS_PRESENT), obj->GetObjectName());
		else if (accessType == MESH_BASE_OBJ || accessType == MESH_WRITE_ACCESS)
		{
			if (base_obj->ClassID() != EPOLYOBJ_CLASS_ID)
				throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_POLY_OPERATION_ON_NONPOLY), obj->GetObjectName());
			if (owner != NULL)
				*owner = base_obj;
...
void 
ValueToBitArrayP(Value* inval, BitArray &theBitArray, int maxSize, const TCHAR* errmsg, int selTypesAllowed, MNMesh* poly) {
	if (inval == n_selection) {
		switch (selTypesAllowed)
		{
			case MESH_VERTSEL_ALLOWED:
