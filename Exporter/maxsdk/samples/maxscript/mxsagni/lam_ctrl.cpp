...
getLinkTime_cf(Value** arg_list, int count)
{
	...
	range_check(i, 1, ((ILinkCtrl*)c)->GetNumTargets(), MaxSDK::GetResourceStringAsMSTR(IDS_LINK_CONTROL_INDEX_OUT_OF_RANGE));
...
setLinkNode_cf(Value** arg_list, int count)
{
	...
	if (pLinkerInst)
	{
		range_check(i, 1, pLinkerInst->GetNumTargets(), MaxSDK::GetResourceStringAsMSTR(IDS_LINK_CONTROL_INDEX_OUT_OF_RANGE));
...
Attachment_GetKey_cf(Value** arg_list, int count)
{
	...
	range_check(index, 1, c->NumKeys(), MaxSDK::GetResourceStringAsMSTR(IDS_ATTACH_KEY_INDEX_OUT_OF_RANGE));
...
euler_set_order(ReferenceTarget* obj, Value* prop, TimeValue t, Value* val)
{
	...
	range_check(order, 1, 9, MaxSDK::GetResourceStringAsMSTR(IDS_AXIS_ORDER_INDEX_OUT_OF_RANGE));
	IEulerControl* ec = (IEulerControl*)obj->GetInterface(I_EULERCTRL);
...
ListController_GetName_cf(Value** arg_list, int count)
{
	...
	int index = arg_list[1]->to_int();
	range_check(index, 1, ((IListControl*)c)->GetListCount(), MaxSDK::GetResourceStringAsMSTR(IDS_LIST_CONTROL_INDEX_OUT_OF_RANGE));
...
ListController_SetName_cf(Value** arg_list, int count)
{
	...
	range_check(index, 1, lc->GetListCount(), MaxSDK::GetResourceStringAsMSTR(IDS_LIST_CONTROL_INDEX_OUT_OF_RANGE));
