...
stricmp_cf(Value** arg_list, int count)
{
	...
	return Integer::intern(_tcsicmp(arg_list[0]->to_string(),arg_list[1]->to_string()));
}
...
get_max_file_asset_metadata_cf(Value** arg_list, int count)
{
		...
		SearchPath(NULL, fileName, NULL, MAX_PATH, buf, NULL))
	{
		...
		while (nBytesRead < nBytes)
		{
			...
			vl.filename = new String(assetMetaData.filename.ToMCHAR()); // don't want to specify assetId in the ctor as this isn't a current asset
...
test_function1_cf(Value** arg_list, int count)
{
	...
	return_protected(Double::intern(timer.EndTimer()));
}
...
test_function4_cf(Value** arg_list, int count)
{
	...
	TCHAR file[MAX_PATH];
	TCHAR ext[MAX_PATH];
	SplitFilename(resFileName, dir, file, ext);
