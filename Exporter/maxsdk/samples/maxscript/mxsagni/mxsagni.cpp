	...
	if (pPanel)
	{
		...
		for (int i = 0; i < MaxVptPerViewPanel; ++i)
		{
			...
			if (vpt.IsAlive())
			{
				if (vpt.GetViewID() == id)
				{
					return Integer::intern(i + 1); // 0 based to 1 based.
				}
			}
		}
	}
...
get_rend_viewID()
{
	return Integer::intern(MAXScript_interface14->GetRendViewID()+1); // Convert 0-based to 1-based
}
...
set_INI_force_utf16_default_cf(Value** arg_list, int count)
{
	...
	return old_value ? &true_value : &false_value;
}
...
set_INI_setting_cf(Value** arg_list, int count)
{
		...
		&& (arg_list[3]->to_string() != NULL) ) 
	{
		...
		if( dwStringLengths > MAX_INI_FILESIZE ) {
			// Throw exception
			throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_INI_FILESIZE_EXCEEDED));
			return 0;
		}
