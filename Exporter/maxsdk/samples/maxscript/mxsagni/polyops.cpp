	...
	if (MAXScript_interface->GetCommandPanelTaskMode() == TASK_MODE_MODIFY)
	{
		...
		if (is_node(obj))
		{
			...
			if ((BaseObject*)obj != MAXScript_interface->GetCurEditObject())
				throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_EDITABLEPOLY_NOT_ACTIVE), obj->GetObjectName());
			if (obj->GetInterface(IID_XREF_ITEM))
				throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_POLYOP_ON_NON_POLY), obj->GetObjectName());
