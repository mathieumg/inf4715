...
CreatePreview_cf(Value** arg_list, int count)
{
	...
	//		dspGeometry:<bool> dspShapes:<bool> dspLights:<bool> dspCameras:<bool> dspHelpers:<bool>
	//		dspParticles:<bool> dspBones:<bool> dspGrid:<bool> dspSafeFrame:<bool> dspFrameNums:<true>
	...
	if ((v != &unsupplied) )
	{
		...
		previewParams.rndLevel = GetID(previewRenderLevel, elements(previewRenderLevel), v, -1);
	}

	previewParams.dspBkg		= bool_key_arg(dspBkg, v, previewParams.dspBkg);

	MAXScript_interface->CreatePreview(&previewParams);
	return &ok;
}
...
setup_indexed_particle(TCHAR* op, int& index, Value** arg_list, int count)
{
	...
	if (os.obj->SuperClassID() != GEOMOBJECT_CLASS_ID ||
		(sp = (SimpleParticle*)os.obj->GetInterface(I_SIMPLEPARTICLEOBJ)) == NULL)
			throw RuntimeError (MaxSDK::GetResourceStringAsMSTR(IDS_SIMPLEPARTICLE_OPERATION_ON_NONSIMPLEPARTICLESYSTEM), os.obj->GetObjectName());
...
fp_get_property_val_cf(Value** arg_list, int count)
{
	...
	switch(pProp->vt) {
		...
		case VT_LPSTR:	
		case VT_BSTR:	
			return new String(VariantToString(pProp).ToMCHAR().data()); // a BSTR is a WORD followed by a UNICODE string. The pointer points just after the WORD
			...
			if ((idx = ip->FindProperty(PROPSET_DOCSUMMARYINFO, &spec)) != -1) {
				if ((pPropDocPart = ip->GetPropertyVariant(PROPSET_DOCSUMMARYINFO, idx)) != NULL) {
					...
					for (UINT i=0; i<pHeading->cElems; i+=2) {
						...
						if(_tcsicmp(VariantToString(pHeading->pElems[i]).ToMCHAR(), arg_list[0]->to_string()))
						{
							nDocPart += ndocs;
							continue;
						}
