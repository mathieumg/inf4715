...
bool CVisualMSDoc::CheckScriptLinkage() 
{
	...
	{
		if(YesNoBoxf(MaxSDK::GetResourceStringAsMSTR(IDS_ASK_TO_UNLINK)) == IDYES)
		{
...
BOOL CVisualMSDoc::SaveModified() 
{
	...
	int res = ::MessageBox(
		...
		(LPCTSTR)str,
		MaxSDK::GetResourceStringAsMSTR(IDS_APP_TITLE),
...
bool CVisualMSDoc::ExportScript(const CString &fname)
{
	...
	{
		MessageBoxf(MaxSDK::GetResourceStringAsMSTR(IDS_CANT_OPEN__FOR_WRITING), fname);
		return FALSE;
	}
