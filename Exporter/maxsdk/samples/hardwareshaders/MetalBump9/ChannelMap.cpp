...
{		
	...
	{
		if(_tcsstr(m_Name,_T(".AVI")) || _tcsstr(m_Name,_T(".avi")))
		{ 
			m_Movie = new Movie;
			m_Type	= TEX_STANDARD;
		...
		{
			SAFE_RELEASE(m_Texture);
			TSTR Str = TSTR(GetString(IDS_LOAD_FILES)) + m_Name + GetString(IDS_CHECK_PATHS);
			GetCOREInterface()->Log()->LogEntry(SYSLOG_WARN, DISPLAY_DIALOG, GetString(IDS_ERROR), Str);
		}
...
{		
	...
	if(Index < 0)
	{
		...
		if(GetFileResource(MAKEINTRESOURCE(IDR_SPECULAR),_T("RT_RCDATA"),
						   &Data,Size))
		{
...
bool ChannelMap::LoadCubeTexture(LPDIRECT3DDEVICE9 Device)
{
	...
	if(Index < 0)
	{
		...
		Hr = D3DXCreateCubeTextureFromFileEx( Device,
											  MapPath.data(), 
											  D3DX_DEFAULT,
											  D3DX_DEFAULT,
											  0,
