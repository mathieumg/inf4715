...
// Tab<BOOL>   mapOn;
   ...
   BOOL     loadingOld;
   void     AddMtl(ReferenceTarget *rt = NULL, int id=0, const TCHAR *name=0);
   void     DeleteSelMtl();
            ...
            case IDC_MTL_ID9: 
                   ...
                   HIWORD(wParam)==EN_CHANGE ) 
               {
                  ...
                  if ( HIWORD(wParam)==EN_KILLFOCUS || iIndex[n]->GotReturn() ) 
                  {
					 ...
					 // UH 01/19/10 Check if the ID is inside the valid range
					 if ( newMtlId < 0 ) newMtlId = 0; 
					 if ( newMtlId > 999 ) newMtlId = 999; 
...
class SetSubRestore: public RestoreObj {
   ...
   public:
      ...
      SetSubRestore(Multi *m, int i, Mtl *mt, int mid, const TCHAR *newName=NULL) {
         multi = m;
         nsub = i;
         mtl = mt;
         mtlid = mid;
      ...
      void Redo() {
         ...
         // multi->SetSubMtlAndName(mtlid,sm,newnm); //twice?
         if (doname) 
