...
public:
	...
	private:
		...
		bool CreateViewportDisplayBitmap(PBITMAPINFO& pOutputMap, TimeValue time, TexHandleMaker& callback, Interval& validInterval, iterator& it);
	};
}
...
bool Composite::Texture::CreateViewportDisplayBitmap(PBITMAPINFO& pOutputMap, TimeValue time, TexHandleMaker& callback, Interval& validInterval, iterator& it)
{
	pOutputMap = NULL;
	if (it->IsVisible()) 
	{
		...
		PBITMAPINFO bmi = it->Texture()->GetVPDisplayDIB(time, callback, theInterval, false);
		if(!bmi){
			return false;
		}
...
void Composite::Texture::InitMultimap( TimeValue  time,
	...
	for( int i = 0; i < nb_tex && it != end(); ++it ) {
		if (it->IsVisible()) {
			...
			if (CreateViewportDisplayBitmap(bmi, time, callback, GetValidity(), it) && bmi)
			{
...
void Composite::Texture::SetupTextures(TimeValue t, DisplayTextureHelper &cb)
{
	...
	// Are we still valid?  If not, initialize...
	if (!m_DisplayTexValidInterval.InInterval(t) || UpdateColorCorrectionMode(mColorCorrectionMode)) 
	{
		// Set the interval and initialize the map.
		ResetDisplayTexHandles();
		m_DisplayTexValidInterval.SetInfinite();
		BITMAPINFO* bmiDiffuse = NULL;
		Texmap* texDiffuse = NULL;
		iterator it( begin() );
		for( ; it != end(); ++it ) 
		{
			if (it->IsVisible()) 
			{
				...
				if (CreateViewportDisplayBitmap(bmi, t, cb, tempInterval, it) && bmi)
				{
					m_DisplayTexValidInterval &= tempInterval;
					if (!bmiDiffuse)
					{
						bmiDiffuse = bmi;
						texDiffuse = it->Texture();
					}
					else
					{	//combine the existing bmiDiffuse with the new layer bitmap if possible.
						Texmap* pTexmap = it->Texture();
						if (IsSameSize(bmi, bmiDiffuse) && IsSameUV(texDiffuse, pTexmap))
						{
							BlendBitmaps(bmiDiffuse, bmi);
							free(bmi);
							bmi = NULL;
						}
					}
		...
		if (bmiDiffuse)
		{
			m_DisplayTexHandle = cb.MakeHandle( bmiDiffuse );
		}
	}
	...
	if (m_DisplayTexHandle)
	{
		...
		for( ; it != end(); ++it )
		{
			...
			{	// use the uv setting of the first visible map.
				cb.UpdateTextureMapInfo( t, ISimpleMaterial::UsageDiffuse, it->Texture());
				break;
			}
		}
