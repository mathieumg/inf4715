...
namespace ColorCorrection { namespace {
   ...
   ParamBlockDesc2 ParamBlock ( 
      ...
      DECLARE_HELPER__( RGBGain,   _T("gainRGB"),   GAIN_RGB,   100.0f, -1000.00f,   1000.0f, 0.10f )
      DECLARE_HELPER__( RGain,     _T("gainR"),     GAIN_R,     100.0f, -1000.00f,   1000.0f, 0.10f )
      DECLARE_HELPER__( GGain,     _T("gainG"),     GAIN_G,     100.0f, -1000.00f,   1000.0f, 0.10f )
      DECLARE_HELPER__( BGain,     _T("gainB"),     GAIN_B,     100.0f, -1000.00f,   1000.0f, 0.10f )

      DECLARE_HELPER__( RGBGamma,  _T("gammaRGB"),  GAMMA_RGB,    1.0f,     0.01f, 100000.0f, 0.01f )
      DECLARE_HELPER__( RGamma,    _T("gammaR"),    GAMMA_R,      1.0f,     0.01f, 100000.0f, 0.01f )
      DECLARE_HELPER__( GGamma,    _T("gammaG"),    GAMMA_G,      1.0f,     0.01f, 100000.0f, 0.01f )
      DECLARE_HELPER__( BGamma,    _T("gammaB"),    GAMMA_B,      1.0f,     0.01f, 100000.0f, 0.01f )

      DECLARE_HELPER__( RGBPivot,  _T("pivotRGB"),  PIVOT_RGB,    1.0f,     0.01f, 100000.0f, 0.01f )
      DECLARE_HELPER__( RPivot,    _T("pivotR"),    PIVOT_R,      1.0f,     0.01f, 100000.0f, 0.01f )
      DECLARE_HELPER__( GPivot,    _T("pivotG"),    PIVOT_G,      1.0f,     0.01f, 100000.0f, 0.01f )
      DECLARE_HELPER__( BPivot,    _T("pivotB"),    PIVOT_B,      1.0f,     0.01f, 100000.0f, 0.01f )

      DECLARE_HELPER__( RGBLift,   _T("liftRGB"),   LIFT_RGB,     0.0f,    -2.00f,      2.0f, 0.01f )
      DECLARE_HELPER__( RLift,     _T("liftR"),     LIFT_R,       0.0f,    -2.00f,      2.0f, 0.01f )
      DECLARE_HELPER__( GLift,     _T("liftG"),     LIFT_G,       0.0f,    -2.00f,      2.0f, 0.01f )
      DECLARE_HELPER__( BLift,     _T("liftB"),     LIFT_B,       0.0f,    -2.00f,      2.0f, 0.01f )

      DECLARE_HELPER__( Printer,   _T("printerLights"), PRINT,    5.0f,   -40.00f,   6000.0f, 0.01f )
...
namespace ColorCorrection {
	...
	class ColorProc : public ParamMap2UserDlgProc
	{
		...
		void DrawSpectrum( HWND ctrl, float h = -180, float s = 1.0f, float l = 0.5f )
		{
			...
			for( int i = 0; i < width; i++, hsl[0] += inc ) {
				...
				int   r  ( clamp( int(rgb[0] * 255.0f), 0, 255 ) );
				int   g  ( clamp( int(rgb[1] * 255.0f), 0, 255 ) );
				int   b  ( clamp( int(rgb[2] * 255.0f), 0, 255 ) );
	...
	public:
		LightnessProc( HWND editor_wnd, IMtlParams *imp, ColorCorrection::Texture *mtl )
			...
			, m_Map      ( mtl )
		{}
		virtual LightnessProc::~LightnessProc()
		{
			m_ParamBlock = nullptr;
			if (m_Map) 
...
{ 
	...
	if ( false == m_ViewportIval.InInterval(t) || UpdateColorCorrectionMode(mColorCorrectionMode))
	{
...
void ColorCorrection::Texture::SetupTextures(TimeValue t, DisplayTextureHelper &cb)
{
	...
	// Are we still valid?  If not, initialize...
	if ( !m_DisplayTexInterval.InInterval(t) || UpdateColorCorrectionMode(mColorCorrectionMode))
	{
		// Set the interval and initialize the map.
		ResetDisplayViewportTex();
...
MSTR ColorCorrection::Texture::GetSubTexmapSlotName( int i )
{
	switch ( i ) 
	{
	case 0:  return GetString(IDS_DS_TEXMAP); 
	default: return _T("");
	}
}
...
TSTR ColorCorrection::Texture::SubAnimName( int i )
{
	if (i == 0)
