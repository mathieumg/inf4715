	...
	if (GetActiveBitmap()==NULL)
		...
		if (!tieTimeToMatID)
			...
			// fixup for imported files
				...
				if (len > 4)
					d += len - 4;
				if ((m_bi.Flags() & MAP_HAS_ALPHA) && _tcsicmp(d, _T(".psd")) != 0) {
					SetAlphaAsMono(true);
...
TSTR BMTex::SubAnimName(int i) {
	switch (i) {
		case 0: return GetString(IDS_DS_COORDINATES);		
		case 1: return GetString(IDS_DS_PARAMETERS);		
		case 2: return GetString(IDS_DS_OUTPUT);		
		case 3: return GetString(IDS_DS_PARAMETERS);		
...
void BMTex::BitmapLoadDlg()	{
	...
	if (_tcscmp(s_lastName, _T(""))) {
		...
		if (UseSystemGammaByDefault())
			m_bi.ResetCustomFlag(BMM_CUSTOM_FILEGAMMA | BMM_CUSTOM_GAMMA);
	}
