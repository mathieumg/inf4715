...
class CameraMapTexture : public Texmap, private imrShaderTranslation {
	public:
		...
		
		{
			...
			TSTR name;
			if (sm1 != NULL)
				name.printf(_T("%s"),GetString(IDS_TEXTURE));
			else name.printf(_T("%s"),GetString(IDS_EMPTY));
				return name; 
...
void CameraMapTexture::GetClassName(TSTR& s)
{
...
void CameraMapTexture::TranslateParameters(imrTranslation& translationInterface, imrShader* shader, TimeValue t, Interval& valid) {
	...
	if(paramsPBlock != NULL) {
		...
		TranslateMRShaderParameter(paramsPBlock, _T("useZBuffer"), (zbuffer != NULL) && (zBufferPB != NULL) && useZBuffer);
		...
		TranslateMRShaderParameter(paramsPBlock, _T("affectBehindCam"), affectBehindCam);
