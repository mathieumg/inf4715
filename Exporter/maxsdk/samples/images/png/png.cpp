...
static float GetPNGGamma(float gamma)
{
	...
	if (doAdjustGamma == -1)
	{
		...
		doAdjustGamma = GetPrivateProfileInt(_T("PNG"), _T("GammaHeuristic"), 1, filename);
	}

	// If we have the adjustment turned on, and gamma > 1, then simply return it as-is
	if (doAdjustGamma == 1 && gamma > 1.0f) return gamma;
	...
	BitmapIO_PNG::ReadPNGFile(BitmapInfo *fbi, BitmapManager *manager) 
{
	...
	if (info->valid & PNG_INFO_gAMA && info->gamma > 0.0)
		// Set the adjusted gamma (inverted as needed)
		fbi->SetGamma(GetPNGGamma(info->gamma));
	// else
	//    fbi->SetGamma (1.0f);
