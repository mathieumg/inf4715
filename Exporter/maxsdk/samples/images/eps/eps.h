...
class BitmapIO_EPS : public BitmapIO {
    friend INT_PTR CALLBACK OutputCtrlDlgProc (HWND hDlg, UINT message, 
					    WPARAM wParam, LPARAM lParam);
    friend INT_PTR CALLBACK ImageInfoDlg (HWND hWnd, UINT message, WPARAM wParam,
				       LPARAM lParam) ;
private:
    ...
    FILE         *mOutStream;     // Output file stream
    FILE         *mInputStream;     // Input file stream
    int          includesPreview;
    ...
    double GetValue (const TCHAR *string);
    int GetOn (const TCHAR *string);
    int GetDataFormat (const TCHAR *string);
    int GetOrientation (const TCHAR *string);
    int GetColorType (const TCHAR *string);
    int GetUnits (const TCHAR *string);
    ...
    // UI functions
    INT_PTR    OutputControl (HWND hDlg, UINT message, 
			      WPARAM wParam, LPARAM lParam);
    INT_PTR    ImageInfoProc (HWND hWnd, UINT message, WPARAM wParam,
			      LPARAM lParam) ;

    // For file writing and reading
    TCHAR       *LocaleFloat (const TCHAR *string);
    TCHAR       *PeriodFloat (double num, const TCHAR *fmt);
...
public:
    ...
    // Descriptions
    const TCHAR *     LongDesc();     // Long TCHAR description
    const TCHAR *     ShortDesc();    // Short TCHAR description
