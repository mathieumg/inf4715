	...
	Bitmap *bitmap)
{
	...
	*/
	...
	if (fwrite (&header, EPSF_HEADER_SIZE, 1, mOutStream) != 1)
	...
	Bitmap *bitmap)
{
	...
	if (fwrite (&tiffHeader, sizeof(struct TiffHeader), 1, mOutStream) != 1)
		return BMMRES_IOERROR;
	if (fwrite (&numTagEntries, 2, 1, mOutStream) != 1)
		return BMMRES_IOERROR;
	...
	fwrite (&tiffDirectory.offsetNextIFD, 4, 1, mOutStream);
	fwrite (&tiffDirectory.x_num, 4, 1, mOutStream);
	fwrite (&tiffDirectory.x_denom, 4, 1, mOutStream);
	fwrite (&tiffDirectory.y_num, 4, 1, mOutStream);
	fwrite (&tiffDirectory.y_denom, 4, 1, mOutStream);
	fwrite (&tiffDirectory.red, 2, 1, mOutStream);
	fwrite (&tiffDirectory.green, 2, 1, mOutStream);
	fwrite (&tiffDirectory.blue, 2, 1, mOutStream);
	fwrite (&tiffDirectory.dummy, 2, 1, mOutStream);
	...
	*/
	if (orientation == PORTRAIT) {
		...
		for (i = 0; i < tiffLength; i++) {
			...
			if (fwrite (buffer, numBytes, 1, mOutStream) != 1) {
				status = BMMRES_IOERROR;
				goto cleanup;
			}
		}
	...
	BitmapIO_EPS::WritePSLength (int colorType, Bitmap *bitmap)
{
	...
	* file
	*/
	if (fseek (mOutStream, 8, SEEK_SET))
		return BMMRES_IOERROR;
	if (fwrite (&psLength, 4, 1, mOutStream) != 1)
		return BMMRES_IOERROR;
...
{
	if (fwrite (&entry->id, 2, 1, mOutStream) != 1)
		return BMMRES_IOERROR;
	if (fwrite (&entry->type, 2, 1, mOutStream) != 1)
		return BMMRES_IOERROR;
	if (fwrite (&entry->numValues, 4, 1, mOutStream) != 1)
		return BMMRES_IOERROR;
	if (fwrite (&entry->valueOrOffset, 4, 1, mOutStream) != 1)
		return BMMRES_IOERROR;
	return BMMRES_SUCCESS;
}
	...
	BitmapIO_EPS::WriteIFDShort (struct IFDShortEntry *entry)
{
	...
	if (fwrite (&entry->numValues, 4, 1, mOutStream) != 1)
		return BMMRES_IOERROR;
	if (fwrite (&entry->valueOrOffset, 2, 1, mOutStream) != 1)
		return BMMRES_IOERROR;
	if (fwrite (&entry->dummy, 2, 1, mOutStream) != 1)
		return BMMRES_IOERROR;
	return BMMRES_SUCCESS;
}
