 ...
 */
int
TIFFRGBAImageOK(TIFF* tif, TCHAR emsg[1024])
{
	...
	break;
    default:
	_stprintf(emsg, GetString(IDS_BAD_BIT_SAMPS),//"Sorry, can not handle images with %d-bit samples",
	    td->td_bitspersample);
	return (0);
    }
	...
	case 3:
	    ...
	    break;
	default:
	    _stprintf(emsg, GetString(IDS_MISS_NEED)/*"Missing needed %s tag"*/, photoTag);
	    return (0);
	}
    }
    switch (photometric) {
	...
	if (td->td_planarconfig == PLANARCONFIG_CONTIG && td->td_samplesperpixel != 1) {
	    _stprintf(emsg,
		GetString(IDS_BAD_CONTIG_WITH),//"Sorry, can not handle contiguous data with %s=%d, and %s=%d",
		photoTag, photometric,
		_T("Samples/pixel"), td->td_samplesperpixel);
	    return (0);
	}
	break;
    case PHOTOMETRIC_YCBCR:
	if (td->td_planarconfig != PLANARCONFIG_CONTIG) {
	    _stprintf(emsg, GetString(IDS_BADYCBCR),//"Sorry, can not handle YCbCr images with %s=%d",
		_T("Planarconfiguration"), td->td_planarconfig);
	    return (0);
	}
	break;
    case PHOTOMETRIC_RGB: 
	if (colorchannels < 3) {
	    _stprintf(emsg, GetString(IDS_RGB_CHAN),//"Sorry, can not handle RGB image with %s=%d",
		_T("Color channels"), colorchannels);
	    return (0);
	}
	break;
#ifdef CMYK_SUPPORT
	...
	if (td->td_inkset != INKSET_CMYK) {
	    _stprintf(emsg, GetString(IDS_SEPAR_WITH),//"Sorry, can not handle separated image with %s=%d",
		_T("InkSet"), td->td_inkset);
	    return (0);
	}
	...
	if (td->td_compression != COMPRESSION_SGILOG) {
	    _stprintf(emsg, GetString(IDS_LOGL_DATA),//"Sorry, LogL data must have %s=%d",
		_T("Compression"), COMPRESSION_SGILOG);
	    return (0);
	}
	break;
	...
	if (td->td_compression != COMPRESSION_SGILOG &&
	    ...
	    _stprintf(emsg, GetString(IDS_LOGLUV_DATA),//"Sorry, LogLuv data must have %s=%d or %d",
		GetString(IDS_COMPRESSION), COMPRESSION_SGILOG, COMPRESSION_SGILOG24);
	    return (0);
	}
	if (td->td_planarconfig != PLANARCONFIG_CONTIG) {
	    _stprintf(emsg, GetString(IDS_WACKED_OUT_LOGLUV),//"Sorry, can not handle LogLuv images with %s=%d",
		_T("Planarconfiguration"), td->td_planarconfig);
	    return (0);
	}
	break;
    default:
	_stprintf(emsg, GetString(IDS_BAD_IMAGE),//"Sorry, can not handle image with %s=%d",
	    photoTag, photometric);
	return (0);
    }
    return (1);
...
int
TIFFRGBAImageBegin(TIFFRGBAImage* img, TIFF* tif, int stop, TCHAR emsg[1024])
{
    uint16* sampleinfo;
    uint16 extrasamples;
    uint16 planarconfig;
	...
	break;
    default:
	_stprintf(emsg, GetString(IDS_SORRY_BIT_SAMPS),//"Sorry, can not image with %d-bit samples",
	    img->bitspersample);
	return (0);
    }
    img->alpha = 0;
	...
	if (!TIFFGetField(tif, TIFFTAG_COLORMAP,
	    ...
	    TIFFError(TIFFFileName(tif), GetString(IDS_NEED_COLORMAP));//"Missing required \"Colormap\" tag");
	    return (0);
	}
	    ...
	    TIFFError(TIFFFileName(tif), GetString(IDS_OOM));//"Out of memory for colormap copy");
	    return (0);
        }
	...
	if (planarconfig == PLANARCONFIG_CONTIG && img->samplesperpixel != 1) {
	    _stprintf(emsg,
		GetString(IDS_CONTIG_DATA),//"Sorry, can not handle contiguous data with %s=%d, and %s=%d",
		photoTag, img->photometric,
		_T("Samples/pixel"), img->samplesperpixel);
	    return (0);
	}
	break;
    case PHOTOMETRIC_YCBCR:
	if (planarconfig != PLANARCONFIG_CONTIG) {
	    _stprintf(emsg, GetString(IDS_BADYCBCR),//"Sorry, can not handle YCbCr images with %s=%d",
		_T("Planarconfiguration"), planarconfig);
	    return (0);
	}
	...
	if (compress != COMPRESSION_SGILOG && compress != COMPRESSION_SGILOG24) {
	    _stprintf(emsg, GetString(IDS_LOGLUV_DATA),//"Sorry, LogLuv data must have %s=%d or %d",
		_T("Compression"), COMPRESSION_SGILOG, COMPRESSION_SGILOG24);
	    return (0);
	}
	if (planarconfig != PLANARCONFIG_CONTIG) {
	    _stprintf(emsg, GetString(IDS_WACKED_OUT_LOGLUV),//"Sorry, can not handle LogLuv images with %s=%d",
	...
	break;
    default:
	_stprintf(emsg, GetString(IDS_OH_NO),//"Sorry, can not handle image with %s=%d",
	    photoTag, img->photometric);
	return (0);
    }
    img->Map = NULL;
...
{
    if (img->get == NULL) {
	TIFFError(TIFFFileName(img->tif), GetString(IDS_NO_GET));//"No \"get\" routine setup");
	return (0);
    }
    if (img->put.any == NULL) {
	TIFFError(TIFFFileName(img->tif),
	    GetString(IDS_NO_PUT));//"No \"put\" routine setupl; probably can not handle image format");
	return (0);
    }
...
gtTileContig(TIFFRGBAImage* img, uint32* raster, uint32 w, uint32 h)
{
    ...
    if (buf == 0) {
	TIFFError(TIFFFileName(tif), GetString(IDS_NO_SPACE_TILE_BUFFER));//"No space for tile buffer");
	return (0);
    }
...
gtStripContig(TIFFRGBAImage* img, uint32* raster, uint32 w, uint32 h)
{
    ...
    if (buf == 0) {
	TIFFError(TIFFFileName(tif), GetString(IDS_NO_STRIP));//"No space for strip buffer");
	return (0);
    }
    y = setorientation(img, h);
    orientation = img->orientation;
...
initYCbCrConversion(TIFFRGBAImage* img)
{
	...
	if (img->ycbcr == NULL) {
	    TIFFError(TIFFFileName(img->tif),
		GetString(IDS_NO_SPC_CONVERSION));//"No space for YCbCr->RGB conversion state");
	    return (NULL);
	}
...
makebwmap(TIFFRGBAImage* img)
{
    ...
    if (img->BWmap == NULL) {
	TIFFError(TIFFFileName(img->tif), GetString(IDS_NO_BW_MAPPING));//"No space for B&W mapping table");
	return (0);
    }
...
setupMap(TIFFRGBAImage* img)
{
    ...
    if (img->Map == NULL) {
	TIFFError(TIFFFileName(img->tif),
	    GetString(IDS_NO_SPC_PHOTO_CONV));//"No space for photometric conversion table");
	return (0);
    }
...
makecmap(TIFFRGBAImage* img)
{
	...
	256*sizeof (uint32 *)+(256*nsamples*sizeof(uint32)));
    if (img->PALmap == NULL) {
	TIFFError(TIFFFileName(img->tif), GetString(IDS_NO_SPC_PALETTE));//"No space for Palette mapping table");
	return (0);
    }
...
buildMap(TIFFRGBAImage* img)
{
	...
	if (checkcmap(img) == 16)
	    ...
	    TIFFWarning(TIFFFileName(img->tif), GetString(IDS_ASSUMING_8));//"Assuming 8-bit colormap");
	/*
...

{
    ...
    if( TIFFIsTiled( tif ) )
    {
        TIFFError(TIFFFileName(tif),
                  GetString(IDS_CANT_USE_TILED));//"Can't use TIFFReadRGBAStrip() with tiled file.");
	return (0);
    }
    ...
    {
        TIFFError(TIFFFileName(tif),
                GetString(IDS_ROW_MUST_FIRST));//"Row passed to TIFFReadRGBAStrip() must be first in a strip.");
	return (0);
    }
...

{
    ...
    {
        TIFFError(TIFFFileName(tif),
                  GetString(IDS_CANT_READ_TILE_STRIP));//"Can't use TIFFReadRGBATile() with stripped file.");
	return (0);
    }
    ...
    {
        TIFFError(TIFFFileName(tif),
                  GetString(IDS_ROW_COL_MUST_TOP));//"Row/col passed to TIFFReadRGBATile() must be top""left corner of a tile.");
	return (0);
    }
