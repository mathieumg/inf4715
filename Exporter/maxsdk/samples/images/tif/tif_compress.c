...
static int
TIFFNoEncode(TIFF* tif, TCHAR* method)
{
...
_TIFFNoRowEncode(TIFF* tif, tidata_t pp, tsize_t cc, tsample_t s)
{
	...
	return (TIFFNoEncode(tif, GetString(IDS_SCANLINE)));
}
...
{
	(void) pp; (void) cc; (void) s;
	return (TIFFNoEncode(tif, GetString(IDS_STRIP)));
}
...
{
	(void) pp; (void) cc; (void) s;
	return (TIFFNoEncode(tif, GetString(IDS_TILE)));
}

static int
TIFFNoDecode(TIFF* tif, TCHAR* method)
{
	const TIFFCodec* c = TIFFFindCODEC(tif->tif_dir.td_compression);
...
{
	(void) pp; (void) cc; (void) s;
	return (TIFFNoDecode(tif, GetString(IDS_SCANLINE)));
}
...
{
	(void) pp; (void) cc; (void) s;
	return (TIFFNoDecode(tif, GetString(IDS_STRIP)));
}
...
{
	(void) pp; (void) cc; (void) s;
	return (TIFFNoDecode(tif, GetString(IDS_TILE)));
}
...
_TIFFNoSeek(TIFF* tif, uint32 off)
{
	(void) off;
	TIFFError(tif->tif_name,
		GetString(IDS_COMP_NO_RAND_ACCESS));
	return (0);
}
...
TIFFRegisterCODEC(uint16 scheme, const char* name, TIFFInitMethod init)
{
	...
	if (cd != NULL) {
		...
		TIFFError(_T("TIFFRegisterCODEC"),
		    GetString(IDS_NO_SPACE_COMP_SCHEME), name);
	return (cd->info);
}
...
TIFFUnRegisterCODEC(TIFFCodec* c)
{
	...
	TIFFError(_T("TIFFUnRegisterCODEC"),
		GetString(IDS_CANT_MOVE_SCHEME), 
		c->name);
}
