...
static	void MissingRequired(TIFF*, const TCHAR*);
...
static char *
CheckMalloc(TIFF* tif, tsize_t n, const TCHAR* what)
{
	char *cp = (char*)_TIFFmalloc(n);
	if (cp == NULL)
		TIFFError(tif->tif_name, GetString(IDS_NO_SPACE), what);
	return (cp);
}
...
TIFFReadDirectory(TIFF* tif)
{
	...
	if (!isMapped(tif)) {
		if (!SeekOK(tif, tif->tif_diroff)) {
			TIFFError(tif->tif_name,
			    GetString(IDS_DIR_SEEK_ERR));//"Seek error accessing TIFF directory"
			return (0);
		}
		if (!ReadOK(tif, &dircount, sizeof (uint16))) {
			TIFFError(tif->tif_name,
				GetString(IDS_BAD_READ_DIR_COUNT));			    //"Can not read TIFF directory count");
			return (0);
		}
		if (tif->tif_flags & TIFF_SWAB)
			TIFFSwabShort(&dircount);
		dir = (TIFFDirEntry *)CheckMalloc(tif,
		    dircount * sizeof (TIFFDirEntry), GetString(IDS_TO_READ_DIR));//"to read TIFF directory"
		if (dir == NULL)
			return (0);
		if (!ReadOK(tif, dir, dircount*sizeof (TIFFDirEntry))) {
			TIFFError(tif->tif_name, GetString(IDS_BAD_READ_DIR));//"Can not read TIFF directory"
			goto bad;
		}
		/*
	...
	for (dp = dir, n = dircount; n > 0; n--, dp++) {
		...
		if (dp->tdir_tag < tif->tif_fieldinfo[fix]->field_tag) {
			if (!diroutoforderwarning) {
				TIFFWarning(tif->tif_name,
					GetString(IDS_DIR_NO_TAG_SORT));//"invalid TIFF directory; tags are not sorted in ascending order"
		...
		if (fix == tif->tif_nfields ||
		    tif->tif_fieldinfo[fix]->field_tag != dp->tdir_tag) {
			TIFFWarning(tif->tif_name,
				GetString(IDS_UNKNOWN_FLD_W_TAG),//"unknown field with tag %d (0x%x) ignored"
	...
	ignore:
		...
		while (dp->tdir_type != (u_short) fip->field_type) {
			...
			if (fix == tif->tif_nfields ||
				...
				TIFFWarning(tif->tif_name,
					GetString(IDS_WRONT_DATA_TYPE),//"wrong data type %d for \"%s\"; tag ignored",
	...
	if (!TIFFFieldSet(tif, FIELD_IMAGEDIMENSIONS)) {
		MissingRequired(tif, _T("ImageLength"));
		goto bad;
	}
	if (!TIFFFieldSet(tif, FIELD_PLANARCONFIG)) {
		MissingRequired(tif, _T("PlanarConfiguration"));
		goto bad;
	}
	/* 
	...
	if (!TIFFFieldSet(tif, FIELD_STRIPOFFSETS)) {
		MissingRequired(tif,
		    isTiled(tif) ? _T("TileOffsets") : _T("StripOffsets"));
		goto bad;
	}
	...
	for (dp = dir, n = dircount; n > 0; n--, dp++) {
		...
		case TIFFTAG_TRANSFERFUNCTION:
			...
			cp = CheckMalloc(tif, dp->tdir_count * sizeof (uint16),
				GetString(IDS_TO_READ_TRANSFN));//"to read \"TransferFunction\" tag");
			if (cp != NULL) {
...
/* END REV 4.0 COMPATIBILITY */
	...
	if (td->td_photometric == PHOTOMETRIC_PALETTE &&
		...
		MissingRequired(tif, _T("Colormap"));
		goto bad;
	}
	/*
	...
	if (!TIFFFieldSet(tif, FIELD_STRIPBYTECOUNTS)) {
		...
		if ((td->td_planarconfig == PLANARCONFIG_CONTIG &&
		    ...
		    MissingRequired(tif, _T("StripByteCounts"));
		    goto bad;
		}
		TIFFWarning(tif->tif_name,
			GetString(IDS_DIR_MISS_CALCULATING),//"TIFF directory is missing required \"%s\" field, calculating from imagelength",
...
#define	BYTECOUNTLOOKSBAD \
    ...
    (td->td_compression == COMPRESSION_NONE && \
     td->td_stripbytecount[0] > TIFFGetFileSize(tif) - td->td_stripoffset[0]))
		 ...
		 */
		TIFFWarning(tif->tif_name,
			GetString(IDS_INSANELY_BOGUS_FIELD),//"Bogus \"%s\" field, ignoring and calculating from imagelength",
		    _TIFFFieldWithTag(tif,TIFFTAG_STRIPBYTECOUNTS)->field_name);
		EstimateStripByteCounts(tif, dir, dircount);
	}
	if (dir)
...
EstimateStripByteCounts(TIFF* tif, TIFFDirEntry* dir, uint16 dircount)
{
	...
	td->td_stripbytecount = (uint32*)
		...
		GetString(IDS_FOR_SBC_ARRAY));//"for \"StripByteCounts\" array");
...
static void
MissingRequired(TIFF* tif, const TCHAR* tagname)
{
	TIFFError(tif->tif_name,
		GetString(IDS_MISS_REQD),//"TIFF directory is missing required \"%s\" field"
		tagname);
}
...
CheckDirCount(TIFF* tif, TIFFDirEntry* dir, uint32 count)
{
	if (count != dir->tdir_count) {
		TIFFWarning(tif->tif_name,
			GetString(IDS_BAD_FIELD_COUNT),//"incorrect count for field \"%s\" (%lu, expecting %lu); tag ignored",
...
TIFFFetchData(TIFF* tif, TIFFDirEntry* dir, char* cp)
{
	...
	return (cc);
bad:
	TIFFError(tif->tif_name, GetString(IDS_FETCH_DATA_ERR),//"Error fetching data for field \"%s\"",
	    _TIFFFieldWithTag(tif, dir->tdir_tag)->field_name);
	return ((tsize_t) 0);
}
...
cvtRational(TIFF* tif, TIFFDirEntry* dir, uint32 num, uint32 denom, float* rv)
{
	if (denom == 0) {
		TIFFError(tif->tif_name,
			GetString(IDS_ZERO_DENOM),//"%s: Rational with zero denominator (num = %lu)",
...
TIFFFetchRationalArray(TIFF* tif, TIFFDirEntry* dir, float* v)
{
	...
	l = (uint32*)CheckMalloc(tif,
	    ...
	    GetString(IDS_FETCH_RATIONAL));//"to fetch array of rationals");
	if (l) {
...
TIFFFetchAnyArray(TIFF* tif, TIFFDirEntry* dir, double* v)
{
	...
	default:
		...
		/* TIFF_UNDEFINED */
		TIFFError(tif->tif_name,
			GetString(IDS_BALLOON),//"Cannot read TIFF_ANY type %d for field \"%s\"",
		    _TIFFFieldWithTag(tif, dir->tdir_tag)->field_name);
		return (0);
	}
	return (1);
...
{
	static const TCHAR mesg[] = _T("to fetch tag value");
	int ok = 0;
	...
	if (CheckDirCount(tif, dir, (uint32) samples)) {
		...
		if (TIFFFetchShortArray(tif, dir, v)) {
			...
			for (i = 1; i < samples; i++)
				if (v[i] != v[0]) {
					TIFFError(tif->tif_name,
						GetString(IDS_BAD_PER_SAMPLE),//"Cannot handle different per-sample values for field \"%s\"",
			   _TIFFFieldWithTag(tif, dir->tdir_tag)->field_name);
					goto bad;
				}
			*pl = v[0];
...
TIFFFetchStripThing(TIFF* tif, TIFFDirEntry* dir, long nstrips, uint32** lpp)
{
	...
	if (*lpp == NULL &&
	    (*lpp = (uint32 *)CheckMalloc(tif,
	      nstrips * sizeof (uint32), _T("for strip array"))) == NULL)
		return (0);
	lp = *lpp;
	if (dir->tdir_type == (int)TIFF_SHORT) {
		...
		uint16* dp = (uint16*) CheckMalloc(tif,
		    dir->tdir_count* sizeof (uint16), GetString(IDS_OOHH_STRIP));//"to fetch strip tag");
		if (dp == NULL)
			return (0);
...
{
	static const TCHAR mesg[] = _T("for \"ReferenceBlackWhite\" array");
	char* cp;
	int ok;
...
ChopUpSingleUncompressedStrip(TIFF* tif)
{
	...
	newcounts = (uint32*) CheckMalloc(tif, nstrips * sizeof (uint32),
				GetString(IDS_CHOPPED_SBC_ARR));//"for chopped \"StripByteCounts\" array");
	newoffsets = (uint32*) CheckMalloc(tif, nstrips * sizeof (uint32),
				GetString(IDS_CHOP_SO_ARRAY));//"for chopped \"StripOffsets\" array");
