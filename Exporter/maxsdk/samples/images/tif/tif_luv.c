...
LogL16Decode(TIFF* tif, tidata_t op, tsize_t occ, tsample_t s)
{
	...
	for (shft = 2*8; (shft -= 8) >= 0; ) {
		...
		GetString(IDS_LOGL16_NED),//"LogL16Decode: Not enough data at row %d (short %d pixels)",
...
LogLuvDecode24(TIFF* tif, tidata_t op, tsize_t occ, tsample_t s)
{
	...
	if (i != npixels) {
		TIFFError(tif->tif_name,
	    GetString(IDS_LOGLUV24_NED),//"LogLuvDecode24: Not enough data at row %d (short %d pixels)",
		    tif->tif_row, npixels - i);
		return (0);
	}
	(*sp->tfunc)(sp, op, npixels);
...
LogLuvDecode32(TIFF* tif, tidata_t op, tsize_t occ, tsample_t s)
{
	...
	for (shft = 4*8; (shft -= 8) >= 0; ) {
		...
		if (i != npixels) {
			TIFFError(tif->tif_name,
				GetString(IDS_LOGLUV32_NED),//"LogLuvDecode32: Not enough data at row %d (short %d pixels)",
			    tif->tif_row, npixels - i);
			tif->tif_rawcp = (tidata_t) bp;
			tif->tif_rawcc = cc;
			return (0);
...
LogL16InitState(TIFF* tif)
{
	...
	LogLuvState* sp = DecoderState(tif);
	static const TCHAR module[] = _T("LogL16InitState");
	...
	default:
		TIFFError(tif->tif_name,
		    GetString(IDS_NO_LOGL_CONV_SUPP));//"No support for converting user data format to LogL");
		return (0);
	}
	sp->tbuflen = (short)(td->td_imagewidth * td->td_rowsperstrip);
	sp->tbuf = (tidata_t*) _TIFFmalloc(sp->tbuflen * sizeof (int16));
	if (sp->tbuf == NULL) {
		TIFFError(module, GetString(IDS_NO_SGILOG_SPACE),//"%s: No space for SGILog translation buffer",
		    tif->tif_name);
		return (0);
	}
	return (1);
...
LogLuvInitState(TIFF* tif)
{
	TIFFDirectory* td = &tif->tif_dir;
	LogLuvState* sp = DecoderState(tif);
	static const TCHAR module[] = _T("LogLuvInitState");
	...
	if (td->td_planarconfig != PLANARCONFIG_CONTIG) {
		TIFFError(module,
		    GetString(IDS_SGILOG_PROBS));//"SGILog compression cannot handle non-contiguous data");
		return (0);
	}
	if (sp->user_datafmt == SGILOGDATAFMT_UNKNOWN)
	...
	default:
		TIFFError(tif->tif_name,
		    GetString(IDS_LOGLUV_CONVERSION_NONE));//"No support for converting user data format to LogLuv");
		return (0);
	}
	sp->tbuflen = (short)(td->td_imagewidth * td->td_rowsperstrip);
...
LogLuvSetupDecode(TIFF* tif)
{
	...
	default:
		TIFFError(tif->tif_name,
    GetString(IDS_INAPPROPRIATE_SGILOG_PHOT),//"Inappropriate photometric interpretation %d for SGILog compression; %s",
		    td->td_photometric, GetString(IDS_LOGLUV_LOGL));//"must be either LogLUV or LogL");
		break;
	}
	return (0);
}
...
LogLuvSetupEncode(TIFF* tif)
{
	...
	default:
		TIFFError(tif->tif_name,
    GetString(IDS_SGILOG_INAPP_INTERP),//"Inappropriate photometric interpretation %d for SGILog compression; %s",
    		    td->td_photometric, GetString(IDS_LOGLUV_LOGL));//"must be either LogLUV or LogL");
		break;
	}
	return (1);
notsupported:
	TIFFError(tif->tif_name,
	    GetString(IDS_SGILOG_SUPPORT),//"SGILog compression supported only for %s, or raw data",
	    td->td_photometric == PHOTOMETRIC_LOGL ? _T("Y, L") : _T("XYZ, Luv"));
	return (0);
}
...
LogLuvVSetField(TIFF* tif, ttag_t tag, va_list ap)
{
	...
	case TIFFTAG_SGILOGDATAFMT:
		...
		default:
			TIFFError(tif->tif_name,
			    GetString(IDS_UNKNOWN_FMT_LOGLUV),//"Unknown data format %d for LogLuv compression",
			    sp->user_datafmt);
			return (0);
		}
...
static const TIFFFieldInfo LogLuvFieldInfo[] = {
    { TIFFTAG_SGILOGDATAFMT,	  0, 0,	TIFF_SHORT,	FIELD_PSEUDO,
      TRUE,	FALSE,	_T("SGILogDataFmt")}
};

int
TIFFInitSGILog(TIFF* tif, int scheme)
{
	static const TCHAR module[] = _T("TIFFInitSGILog");
	LogLuvState* sp;
	...
	return (1);
bad:
	TIFFError(module, GetString(IDS_LOGLUV_NO_SPACE),//"%s: No space for LogLuv state block"
		tif->tif_name);
	return (0);
}
