...
TIFFWriteDirectory(TIFF* tif)
{
	...
	if (tif->tif_flags & TIFF_POSTENCODE) {
		...
		if (!(*tif->tif_postencode)(tif)) {
			TIFFError(tif->tif_name,
			    GetString(IDS_ERROR_POST_ENC));//"Error post-encoding before directory write");
			return (0);
		}
	}
	...
	if (tif->tif_rawcc > 0 && !TIFFFlushData1(tif)) {
		TIFFError(tif->tif_name,
		    GetString(IDS_CANT_FLUSH_BEF));//"Error flushing data before directory write"
		return (0);
	}
	...
	if (data == NULL) {
		TIFFError(tif->tif_name,
			GetString(IDS_NO_DIR_SPACE));//"Cannot write directory, out of space");
		return (0);
	}
	/*
	...
	if (!WriteOK(tif, &dircount, sizeof (dircount))) {
		TIFFError(tif->tif_name, GetString(IDS_COUNT_ERR));//"Error writing directory count");
		goto bad;
	}
	if (!WriteOK(tif, data, dirsize)) {
		TIFFError(tif->tif_name, GetString(IDS_ERR_DIR_WRITE));//"Error writing directory contents");
		goto bad;
	}
	if (!WriteOK(tif, &diroff, sizeof (diroff))) {
		TIFFError(tif->tif_name, GetString(IDS_ERR_WRITE_LINK));//"Error writing directory link");
		goto bad;
	}
		...
		if (fv < 0) {
			if (type == TIFF_RATIONAL) {
				TIFFWarning(tif->tif_name,
					GetString(IDS_INFO_RAT_LOST),//"\"%s\": Information lost writing value (%g) as (unsigned) RATIONAL",
...
TIFFWriteTransferFunction(TIFF* tif, TIFFDirEntry* dir)
{
	...
	tsize_t n = (uint32)(1L<<td->td_bitspersample) * sizeof (uint16);
...
TIFFWriteData(TIFF* tif, TIFFDirEntry* dir, char* cp)
{
	...
	TIFFError(tif->tif_name, GetString(IDS_ERROR_WRITE_FIELD),//"Error writing data for field \"%s\"",
...
TIFFLinkDirectory(TIFF* tif)
{
	...
	if (tif->tif_flags & TIFF_INSUBIFD) {
		...
		if (!WriteOK(tif, &diroff, sizeof (diroff))) {
			TIFFError(module,
				GetString(IDS_ERROR_WRITE_IFD),//"%s: Error writing SubIFD directory link",
			    tif->tif_name);
			return (0);
		}
		/*
...
#define	HDROFF(f)	((toff_t)(uintptr_t) &(((TIFFHeader*) 0)->f))
		(void) TIFFSeekFile(tif, HDROFF(tiff_diroff), SEEK_SET);
		if (!WriteOK(tif, &diroff, sizeof (diroff))) {
			TIFFError(tif->tif_name, GetString(IDS_ERROR_HEADER));//"Error writing TIFF header");
			return (0);
		}
		return (1);
	}
	...
	do {
		...
		if (!SeekOK(tif, nextdir) ||
			...
			TIFFError(module, GetString(IDS_BAD_READ_DIR_COUNT));
			return (0);
		}
		if (tif->tif_flags & TIFF_SWAB)
			TIFFSwabShort(&dircount);
		...
		if (!ReadOK(tif, &nextdir, sizeof (nextdir))) {
			TIFFError(module, GetString(IDS_ERROR_FETCH_LINK));//"Error fetching directory link");
			return (0);
		}
		if (tif->tif_flags & TIFF_SWAB)
	...
	if (!WriteOK(tif, &diroff, sizeof (diroff))) {
		TIFFError(module, GetString(IDS_ERROR_WRITE_LINK));//"Error writing directory link");
		return (0);
	}
	return (1);
}
