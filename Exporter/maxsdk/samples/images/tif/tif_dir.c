...
bad:
	TIFFError(_T("TIFFSetField"), 
		GetString(IDS_INVALID_INKNAMES),
	    tif->tif_name,
...
_TIFFVSetField(TIFF* tif, ttag_t tag, va_list ap)
{
	...
	case TIFFTAG_ORIENTATION:
		...
		if (v < ORIENTATION_TOPLEFT || ORIENTATION_LEFTBOT < v) {
			TIFFWarning(tif->tif_name,
			    GetString(IDS_BAD_TAG_VALUE),//"Bad value %ld for \"%s\" tag ignored",
	...
	case TIFFTAG_TILEWIDTH:
		...
		if (v32 % 16) {
			if (tif->tif_mode != O_RDONLY)
				goto badvalue32;
			TIFFWarning(tif->tif_name,
			    GetString(IDS_BAD_TILE_WIDTH),//"Nonstandard tile width %d, convert file"
				v32);
		}
		td->td_tilewidth = v32;
	...
	case TIFFTAG_TILELENGTH:
		...
		if (v32 % 16) {
			if (tif->tif_mode != O_RDONLY)
				goto badvalue32;
			TIFFWarning(tif->tif_name,
				GetString(IDS_BAD_TILE_LENGTH), //"Nonstandard tile length %d, convert file", 
				v32);
		}
		td->td_tilelength = v32;
		tif->tif_flags |= TIFF_ISTILED;
	...
	case TIFFTAG_SUBIFD:
		if ((tif->tif_flags & TIFF_INSUBIFD) == 0) {
			...
			TIFFError(tif->tif_name, GetString(IDS_NO_NEST_IFD));//"Sorry, cannot nest SubIFDs");
			status = 0;
		}
		break;
#endif
	...
	default:
		 ...
		 */
		TIFFError(_T("TIFFSetField"),
			GetString(IDS_NO_TAG_SUPPORT),//"%s: Invalid %stag \"%s\" (not supported by codec)",
		    tif->tif_name, isPseudoTag(tag) ? GetString(IDS_PSEUDO) : _T(""),
	...
	return (status);
badvalue:
	TIFFError(tif->tif_name, GetString(IDS_D_BAD_VALUE_FOR), v,//"%d: Bad value for \"%s\""
	    _TIFFFieldWithTag(tif, tag)->field_name);
	va_end(ap);
	return (0);
badvalue32:
	TIFFError(tif->tif_name, GetString(IDS_LD_BAD_VALUE_FOR), v32,//"%ld: Bad value for \"%s\""
	    _TIFFFieldWithTag(tif, tag)->field_name);
	va_end(ap);
	return (0);
badvaluedbl:
	TIFFError(tif->tif_name, GetString(IDS_F_BAD_VALUE_FOR), (float)d,//"%f: Bad value for \"%s\""
	    _TIFFFieldWithTag(tif, tag)->field_name);
	va_end(ap);
	return (0);
}
...
OkToChangeTag(TIFF* tif, ttag_t tag)
{
	...
	if (!fip) {			/* unknown tag */
		TIFFError(_T("TIFFSetField"), GetString(IDS_UNKNOWN_TAG),//"%s: Unknown %stag %u"
		    tif->tif_name, isPseudoTag(tag) ? GetString(IDS_PSEUDO) : _T(""), tag);
		return (0);
	}
	if (tag != TIFFTAG_IMAGELENGTH && (tif->tif_flags & TIFF_BEENWRITING) &&
		 ...
		 */
		TIFFError(_T("TIFFSetField"),
			GetString(IDS_NO_MODIFY_TAG),//"%s: Cannot modify tag \"%s\" while writing",
...
{
    static const TCHAR module[] = _T("TIFFAdvanceDirectory");
    uint16 dircount;
    if (isMapped(tif))
    {
        toff_t poff=*nextdir;
        ...
        {
            TIFFError(module, GetString(IDS_DIR_FETCH_ERR),//"%s: Error fetching directory count",
                      tif->tif_name);
            return (0);
        }
        ...
        {
            TIFFError(module, GetString(IDS_DIR_FETCH_LINK_ERR),//"%s: Error fetching directory link",
                      tif->tif_name);
            return (0);
        }
...
{
	static const TCHAR module[] = _T("TIFFUnlinkDirectory");
	toff_t nextdir;
	toff_t off;
	tdir_t n;

	if (tif->tif_mode == O_RDONLY) {
		TIFFError(module, GetString(IDS_NO_UNLINK));//"Can not unlink directory in read-only file");
		return (0);
	}
	/*
	...
	for (n = dirn-1; n > 0; n--) {
		if (nextdir == 0) {
			TIFFError(module, GetString(IDS_NO_DIRECTORY),//"Directory %d does not exist", 
				dirn);
			return (0);
		}
	...
	if (!WriteOK(tif, &nextdir, sizeof (uint32))) {
		TIFFError(module, GetString(IDS_NO_WRITE_DIR_LINK));//"Error writing directory link");
		return (0);
	}
	/*
