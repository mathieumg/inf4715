...
static	int TIFFGrowStrips(TIFF*, int, const TCHAR*);
...
TIFFWriteScanline(TIFF* tif, tdata_t buf, uint32 row, tsample_t sample)
{
	...
	if (row >= td->td_imagelength) {	/* extend image */
		...
		GetString(IDS_CANT_CHANGE_LENGTH_PLANES));//"Can not change \"ImageLength\" when using separate planes");
			return (-1);
		}
	...
	if (td->td_planarconfig == PLANARCONFIG_SEPARATE) {
		if (sample >= td->td_samplesperpixel) {
			TIFFError(tif->tif_name,
			    GetString(IDS_WRITE_SAMPLE_OOR),//"%d: Sample out of range, max %d",
...
TIFFWriteEncodedStrip(TIFF* tif, tstrip_t strip, tdata_t data, tsize_t cc)
{
	...
	if (strip >= td->td_nstrips) {
		if (td->td_planarconfig == PLANARCONFIG_SEPARATE) {
			TIFFError(tif->tif_name,
		GetString(IDS_WRITE_NO_GROW));//"Can not grow image by strips when using separate planes");
			return ((tsize_t) -1);
		}
...
TIFFWriteEncodedTile(TIFF* tif, ttile_t tile, tdata_t data, tsize_t cc)
{
	...
	if (tile >= td->td_nstrips) {
		TIFFError(module, GetString(IDS_WRITE_TILE_OOR),//"%s: Tile %lu out of range, max %lu",
 ...
 */
int
TIFFWriteCheck(TIFF* tif, int tiles, const TCHAR* module)
{
	if (tif->tif_mode == O_RDONLY) {
		TIFFError(module, GetString(IDS_WRITE_FILE_NOT_OPEN),//"%s: File not open for writing",
		    tif->tif_name);
		return (0);
	}
	if (tiles ^ isTiled(tif)) {
		TIFFError(tif->tif_name, tiles ?
			GetString(IDS_WRITE_TILE_STRIPPED) : //"Can not write tiles to a stripped image" :
		    GetString(IDS_WRITE_SCAN_TO_TILE));//"Can not write scanlines to a tiled image");
		return (0);
	}
	/*
	...
	if (!TIFFFieldSet(tif, FIELD_IMAGEDIMENSIONS)) {
		TIFFError(module,
		    GetString(IDS_WRITE_IMAGEWIDTH_SET),//"%s: Must set \"ImageWidth\" before writing data",
		    tif->tif_name);
		return (0);
	}
	if (!TIFFFieldSet(tif, FIELD_PLANARCONFIG)) {
		TIFFError(module,
	    GetString(IDS_WRITE_PLANAR_NOT_SET),//"%s: Must set \"PlanarConfiguration\" before writing data",
		    tif->tif_name);
		return (0);
	}
	if (tif->tif_dir.td_stripoffset == NULL && !TIFFSetupStrips(tif)) {
		tif->tif_dir.td_nstrips = 0;
		TIFFError(module, GetString(IDS_WRITE_NO_SPACE_A),//"%s: No space for %s arrays",
		    tif->tif_name, isTiled(tif) ? GetString(IDS_TILE)/*"tile"*/ : GetString(IDS_STRIP)/*"strip"*/);
		return (0);
	}
...
{
	static const TCHAR module[] = _T("TIFFWriteBufferSetup");
	...
	if (bp == NULL) {
		bp = _TIFFmalloc(size);
		if (bp == NULL) {
			TIFFError(module, GetString(IDS_NO_OUT_BUF_SPACE),//"%s: No space for output buffer",
			    tif->tif_name);
			return (0);
		}
		tif->tif_flags |= TIFF_MYBUFFER;
 ...
 */
static int
TIFFGrowStrips(TIFF* tif, int delta, const TCHAR* module)
{
	TIFFDirectory *td = &tif->tif_dir;
	...
	if (td->td_stripoffset == NULL || td->td_stripbytecount == NULL) {
		td->td_nstrips = 0;
		TIFFError(module, GetString(IDS_WRITE_EXPAND),//"%s: No space to expand strip arrays",
		    tif->tif_name);
		return (0);
	}
...
TIFFAppendToStrip(TIFF* tif, tstrip_t strip, tidata_t data, tsize_t cc)
{
	...
	if (td->td_stripoffset[strip] == 0 || tif->tif_curoff == 0) {
		...
		if (td->td_stripoffset[strip] != 0) {
			if (!SeekOK(tif, td->td_stripoffset[strip])) {
				TIFFError(module,
				    GetString(IDS_WRITE_SEEK_ERR),//"%s: Seek error at scanline %lu",
	...
	if (!WriteOK(tif, data, cc)) {
		TIFFError(module, GetString(IDS_WRITE_ERR),//"%s: Write error at scanline %lu",
		    tif->tif_name, (u_long) tif->tif_row);
		return (0);
	}
	tif->tif_curoff += cc;
