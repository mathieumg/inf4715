...
#ifdef FRED
	{ TIFFTAG_MATRIX,		-1,	-1, TIFF_DOUBLE,	FIELD_MATRIX, 
	TRUE, TRUE, _T("IntergraphMatrix") },
	{ TIFFTAG_MODELTIES,	-1,	-1, TIFF_DOUBLE, FIELD_MODELTIES, 
	TRUE, TRUE, _T("ModelTiepoint") },
	{ TIFFTAG_MODELTRANS,	16,	16, TIFF_DOUBLE,	FIELD_MODELTRANS, 
	TRUE, FALSE, _T("ModelTransformation") },
#endif

#ifdef PHOTOSHOP_SUPPORT
    { TIFFTAG_PHOTOSHOP,    -1,-3, TIFF_UNDEFINED, FIELD_PHOTOSHOP, 
      FALSE,    TRUE,   _T("Photoshop") },
    { TIFFTAG_PHOTOSHOP,    -1,-1, TIFF_BYTE,   FIELD_PHOTOSHOP, 
      FALSE,    TRUE,   _T("Photoshop") },
#endif
#ifdef ICC_SUPPORT
    { TIFFTAG_ICCPROFILE,	-1,-3, TIFF_UNDEFINED,	FIELD_ICCPROFILE,
      FALSE,	TRUE,	_T("ICC Profile") },
#endif

//Finished viz-specific tags


	{ TIFFTAG_STONITS,		 1, 1, TIFF_DOUBLE,	FIELD_STONITS,
      TRUE,	FALSE,	_T("StoNits") },
/* begin Pixar tags */
    { TIFFTAG_PIXAR_IMAGEFULLWIDTH,  1, 1, TIFF_LONG,	FIELD_IMAGEFULLWIDTH,
      TRUE,	FALSE,	_T("ImageFullWidth") },
    { TIFFTAG_PIXAR_IMAGEFULLLENGTH, 1, 1, TIFF_LONG,	FIELD_IMAGEFULLLENGTH,
      TRUE,	FALSE,	_T("ImageFullLength") },
    { TIFFTAG_PIXAR_TEXTUREFORMAT,  -1,-1, TIFF_ASCII,	FIELD_TEXTUREFORMAT,
      TRUE,	FALSE,	_T("TextureFormat") },
    { TIFFTAG_PIXAR_WRAPMODES,	    -1,-1, TIFF_ASCII,	FIELD_WRAPMODES,
      TRUE,	FALSE,	_T("TextureWrapModes") },
    { TIFFTAG_PIXAR_FOVCOT,	     1, 1, TIFF_FLOAT,	FIELD_FOVCOT,
      TRUE,	FALSE,	_T("FieldOfViewCotan") },
    { TIFFTAG_PIXAR_MATRIX_WORLDTOSCREEN,	16,16,	TIFF_FLOAT,
      FIELD_MATRIX_WORLDTOSCREEN,	TRUE,	FALSE,	_T("MatrixWorldToScreen") },
    { TIFFTAG_PIXAR_MATRIX_WORLDTOCAMERA,	16,16,	TIFF_FLOAT,
       FIELD_MATRIX_WORLDTOCAMERA,	TRUE,	FALSE,	_T("MatrixWorldToCamera") },
/* end Pixar tags */

};
#define	N(a)	(_countof (a))
...
_TIFFPrintFieldInfo(TIFF* tif, FILE* fd)
{
	...
	for (i = 0; i < tif->tif_nfields; i++) {
		...
		_ftprintf(fd, _T("field[%2d] %5lu, %2d, %2d, %d, %2d, %5s, %5s, %s\n")
			, i
			, (unsigned long) fip->field_tag
			, fip->field_readcount, fip->field_writecount
			, fip->field_type
			, fip->field_bit
			, fip->field_oktochange ? _T("TRUE") : _T("FALSE")
			, fip->field_passcount ? _T("TRUE") : _T("FALSE")
			, fip->field_name
		);
	}
}
...
_TIFFFieldWithTag(TIFF* tif, ttag_t tag)
{
	...
	if (!fip) {
		TIFFError(_T("TIFFFieldWithTag"),
		    GetString(IDS_INT_UNKNOWN_TAG),//"Internal error, unknown tag 0x%x", 
			(u_int) tag);
		assert(fip != NULL);
		/*NOTREACHED*/
	}
