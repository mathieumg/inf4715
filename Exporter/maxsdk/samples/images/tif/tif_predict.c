...
PredictorSetup(TIFF* tif)
{
	...
	if (sp->predictor == 1)		/* no differencing */
		return (1);
	if (sp->predictor != 2) {
		TIFFError(tif->tif_name, GetString(IDS_PRED_BAD_VAL),//"\"Predictor\" value %d not supported",
		    sp->predictor);
		return (0);
	}
	if (td->td_bitspersample != 8 && td->td_bitspersample != 16) {
		TIFFError(tif->tif_name,
    GetString(IDS_PRED_BAD_HORIZ),//"Horizontal differencing \"Predictor\" not supported with %d-bit samples",
		    td->td_bitspersample);
		return (0);
	}
...
PredictorPrintDir(TIFF* tif, FILE* fd, long flags)
{
	...
	if (TIFFFieldSet(tif,FIELD_PREDICTOR)) {
		_ftprintf(fd, GetString(IDS_PREDICTOR_COLON));//"  Predictor: ");
		switch (sp->predictor) {
		case 1: _ftprintf(fd, GetString(IDS_PRED_NONE)/*"none "*/); break;
		case 2: _ftprintf(fd, GetString(IDS_PRED_HORIZ)/*"horizontal differencing "*/); break;
		}
