...
int
_TIFFgetMode(const TCHAR* mode, const TCHAR* module)
{
	int m = -1;
	...
	case _T('a'):
		...
		break;
	default:
		TIFFError(module, GetString(IDS_SOMETHING_BAD_MODE), mode);//"\"%s\": Bad mode"
		break;
	}
	return (m);
}
...
)
{
	...
	if (m == -1)
		goto bad2;
	tif = (TIFF *)_TIFFmalloc((tsize_t)(sizeof (TIFF) + (_tcslen(name) + 1)*sizeof(TCHAR)));	// SR DCAST64: Downcast to 4G limit.
	if (tif == NULL) {
		TIFFError(module, GetString(IDS_OUT_OF_MEM_TIFF), name);//"%s: Out of memory (TIFF structure)"
		goto bad2;
	}
	...
	if (!ReadOK(tif, &tif->tif_header, sizeof (TIFFHeader))) {
		if (tif->tif_mode == O_RDONLY) {
			TIFFError(name, GetString(IDS_NO_READ_HEADER));//"Cannot read TIFF header");
			goto bad;
		}
		/*
		...
		if (!WriteOK(tif, &tif->tif_header, sizeof (TIFFHeader))) {
			TIFFError(name, GetString(IDS_ERR_WRITE_HEAD));//"Error writing TIFF header");
			goto bad;
		}
		/*
	...
	if (tif->tif_header.tiff_magic != TIFF_BIGENDIAN &&
		...
		TIFFError(name,  GetString(IDS_BAD_MAGIIC),//"Not a TIFF file, bad magic number %d (0x%x)",
	...
	if (tif->tif_header.tiff_version != TIFF_VERSION) {
		TIFFError(name,
		    GetString(IDS_TIFF_BADVER),//"Not a TIFF file, bad version number %d (0x%x)",
	...
	!TIFFMapFileContents(tif, (tdata_t*) &tif->tif_base, &tif->tif_size))
		...
		if (TIFFReadDirectory(tif)) {
                        ...
                        {
                            TIFFError( name, 
										GetString(IDS_NO_OPEN_COMPRESSED));//"Can't open a compressed TIFF file"" with compression for update." );
                            goto bad;
                        }
			tif->tif_rawcc = -1;
