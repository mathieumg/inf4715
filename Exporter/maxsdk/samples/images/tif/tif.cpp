...
bail_out:
	...
	for (y = 0; y<height; y++)  {
		...
		int max_pos_y = GetMaxBitmapYPosition(y,td->td_orientation,height);
	...
	do {
		...
		for (j=0; j<rps; j++) {
			...
			else if (UserData.writeType == tif_write_logluv)
			{
				...
				TIFFError(_T("tif.cpp"), GetString(IDS_NO_WRITE_ALGORITHM));
				packw = 0;
			}
#ifdef ALLOW_LZW_COMPRESSION
...
void MAXWarningHandler(const TCHAR* module, const TCHAR* fmt, va_list ap)
{
	...
	szTmp = szTitle + (lstrlen(szTitle)+2);
...
void MAXErrorHandler(const TCHAR* module, const TCHAR* fmt, va_list ap)
{
#ifndef TIF_PLATFORM_CONSOLE
