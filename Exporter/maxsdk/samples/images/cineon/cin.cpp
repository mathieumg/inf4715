...
CIN_ImageInfoDialogProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG: {
     ...
     // dialog text fields
		SetDlgItemText(hWnd, IDC_INFO_DLG_FILE_NAME,            TSTR::FromCP(codePage, dpxInfo->mCineonImage.GetImageFileName()));
        SetDlgItemText(hWnd, IDC_INFO_DLG_FILE_VERSION,         TSTR::FromCP(codePage, dpxInfo->mCineonImage.GetVersionNumber()));
        SetDlgItemText(hWnd, IDC_INFO_DLG_FILE_CREATION_DATE,   TSTR::FromCP(codePage, dpxInfo->mCineonImage.GetImageCreationDate()));
        SetDlgItemText(hWnd, IDC_INFO_DLG_FILE_CREATION_TIME,   TSTR::FromCP(codePage, dpxInfo->mCineonImage.GetImageCreationTime()));
...
BitmapIO_CIN::ReadINI()
{
    ...
    GetPrivateProfileString(CIN_INI_SECTION, CIN_INI_VERSION, buffer, buffer, _countof(buffer), filename);
    ...
    if (mUserData.version != CIN_USERDATA_VERSION) {
        ...
        GetPrivateProfileString(CIN_INI_SECTION, CIN_INI_REF_WHITE,   buffer, buffer, _countof(buffer), filename);
        mUserData.mRefWhite = (DWORD) _ttoi(buffer);
        GetPrivateProfileString(CIN_INI_SECTION, CIN_INI_REF_BLACK,   buffer, buffer, _countof(buffer), filename);
