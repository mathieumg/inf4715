...
BOOL ImageFilter_Stars::ValidatePiData( ImageFilterInfo *fi, ILoad *pILoad )
{
	...
	WStr cameraName = data.camera_w;
	strncpy(data.camera_a, cameraName.ToCP(codePage), _countof(data.camera_a)-1);
	WStr fileName = data.filename_w;
	strncpy(data.filename_a, fileName.ToCP(codePage), _countof(data.filename_a)-1);
