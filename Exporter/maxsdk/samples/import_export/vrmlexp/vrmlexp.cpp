...
const TCHAR* NodeTable::AddName(const TCHAR* name)
{
   ...
   for (nList = mNames[hashVal]; nList; nList = nList->next) {
      if (nList->name && !_tcscmp(name, nList->name)) { // found a match
       // checkout name for "_0xxx" that is our tag
         matchStr = _tcsrchr(name, _T('_'));
         if (matchStr) {      // possible additional duplicate names
            if (matchStr[1] == _T('0')) {     // assume additional duplicate names
               matchVal = _ttoi(matchStr + 1);   // get number
               _tcsncpy(buf, name, _tcslen(name) - _tcslen(matchStr)); // first part
               buf[_tcslen(name) - _tcslen(matchStr)] = _T('\0'); // terminate
               //sprintf(newName.name, "%s_0%d", buf, matchVal+1);   // add one
                    TSTR newName(buf);
                    _stprintf(buf, _T("_0%d"), matchVal+1);   // add one
                    newName.Append(TSTR(buf));
               return AddName(newName.data()); // check for unique new name
            }
         }
         //_stprintf(newName.name, "%s_0", name); // first duplicate name
            TSTR newName(name);
