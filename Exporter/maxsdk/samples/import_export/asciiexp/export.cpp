...
void AsciiExp::ExportLightObject(INode* node, int indentLevel)
{
	...
	pStream.Printf(_T("%s\t%s "), indent.data(), ID_LIGHT_TYPE);
	switch(ls.type) {
	case OMNI_LIGHT:  pStream.Printf(_T("%s\n"), ID_LIGHT_TYPE_OMNI); break;
	case TSPOT_LIGHT: pStream.Printf(_T("%s\n"), ID_LIGHT_TYPE_TARG);  break;
	case DIR_LIGHT:   pStream.Printf(_T("%s\n"), ID_LIGHT_TYPE_DIR); break;
	case FSPOT_LIGHT: pStream.Printf(_T("%s\n"), ID_LIGHT_TYPE_FREE); break;
	}
...
void AsciiExp::ExportMesh(INode* node, TimeValue t, int indentLevel)
{
	...
	for (i=0; i<mesh->getNumFaces(); i++) {
		pStream.Printf(_T("%s\t\t%s %4d:    A: %4d B: %4d C: %4d AB: %4d BC: %4d CA: %4d"),
			indent.data(),
			ID_MESH_FACE, i,
			mesh->faces[i].v[vx1],
			mesh->faces[i].v[vx2],
		...
		pStream.Printf(_T("\t %s "), ID_MESH_SMOOTHING);
		for (int j=0; j<32; j++) {
			if (mesh->faces[i].smGroup & (1<<j)) {
				if ((j<31) && (mesh->faces[i].smGroup>>(j+1))) {
					pStream.Printf(_T("%d,"),j+1); // Add extra comma
				} else {
					pStream.Printf(_T("%d "),j+1);
				}
			}
		}
		...
		// the material ID.
		pStream.Printf(_T("\t%s %d"), ID_MESH_MTLID, mesh->faces[i].getMatID());
...
void AsciiExp::DumpMaterial(Mtl* mtl, int mtlID, int subNo, int indentLevel)
{
	...
	if (mtl->ClassID() == Class_ID(DMTL_CLASS_ID, 0)) {
		...
		pStream.Printf(_T("%s\t%s "), indent.data(), ID_SHADING);
		switch(std->GetShading()) {
		case SHADE_CONST: pStream.Printf(_T("%s\n"), ID_MAT_SHADE_CONST); break;
		case SHADE_PHONG: pStream.Printf(_T("%s\n"), ID_MAT_SHADE_PHONG); break;
		case SHADE_METAL: pStream.Printf(_T("%s\n"), ID_MAT_SHADE_METAL); break;
		case SHADE_BLINN: pStream.Printf(_T("%s\n"), ID_MAT_SHADE_BLINN); break;
		default: pStream.Printf(_T("%s\n"), ID_MAT_SHADE_OTHER); break;
		}
		...
		pStream.Printf(_T("%s\t%s "), indent.data(), ID_XP_TYPE);
		switch (std->GetTransparencyType()) {
		case TRANSP_FILTER: pStream.Printf(_T("%s\n"), ID_MAP_XPTYPE_FLT); break;
		case TRANSP_SUBTRACTIVE: pStream.Printf(_T("%s\n"), ID_MAP_XPTYPE_SUB); break;
		case TRANSP_ADDITIVE: pStream.Printf(_T("%s\n"), ID_MAP_XPTYPE_ADD); break;
...
void AsciiExp::DumpTexture(Texmap* tex, Class_ID cid, int subNo, float amt, int indentLevel)
{
	...
	if (tex->ClassID() == Class_ID(BMTEX_CLASS_ID, 0x00)) {
		...
		pStream.Printf(_T("%s\t%s "), indent.data(), ID_BMP_FILTER);
		switch(((BitmapTex*)tex)->GetFilterType()) {
		case FILTER_PYR:  pStream.Printf(_T("%s\n"), ID_BMP_FILT_PYR); break;
		case FILTER_SAT: pStream.Printf(_T("%s\n"), ID_BMP_FILT_SAT); break;
...
void AsciiExp::DumpUVGen(StdUVGen* uvGen, int indentLevel)
{
	...
	pStream.Printf(_T("%s%s "), indent.data(), ID_MAPTYPE);
	
	switch (mapType) {
	case UVMAP_EXPLICIT: pStream.Printf(_T("%s\n"), ID_MAPTYPE_EXP); break;
	case UVMAP_SPHERE_ENV: pStream.Printf(_T("%s\n"), ID_MAPTYPE_SPH); break;
	case UVMAP_CYL_ENV:  pStream.Printf(_T("%s\n"), ID_MAPTYPE_CYL); break;
	case UVMAP_SHRINK_ENV: pStream.Printf(_T("%s\n"), ID_MAPTYPE_SHR); break;
	case UVMAP_SCREEN_ENV: pStream.Printf(_T("%s\n"), ID_MAPTYPE_SCR); break;
	}
