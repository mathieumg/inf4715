...
private:
		TCHAR* saved_lc;
public:
	SetLocaleClass():saved_lc(NULL)
	{
		TCHAR* lc = ::_tsetlocale(LC_NUMERIC, NULL); // query the current locale
		if (lc){
			saved_lc = ::_tcsdup(lc); // save it
...
AIExport::DoExport(const TCHAR *filename,ExpInterface *ei,Interface *gi, BOOL suppressPrompts, DWORD options) {
	...
	if(status > 0)	{
#ifdef DBGDXF
DebugPrint(_T("AI status OK!\n"));
#endif
		}
#ifdef DBGDXF
	else
	if(status < 0)
		DebugPrint(_T("Error somewhere in AI!\n"));
#endif
