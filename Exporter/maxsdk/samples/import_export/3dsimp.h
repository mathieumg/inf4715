...
public:
	...
	int				AddNode(ImpNode *node,const TCHAR *name,int type,Mesh *mesh,TCHAR *owner,int mtlNum=-1);
	int				SetNodeId(ImpNode *node,short id);
	int				SetNodesParent(ImpNode *node,ImpNode *parent);
	void *			FindObject(TCHAR *name, int &type, int &cstShad, int &rcvShad, int &mtlNum);
	int				UseObject(TCHAR *name);
	int				CompleteScene();
	int 			SetupEnvironment();
	int 			FixupExclusionLists();
	ImpNode *		FindNode(TCHAR *name);
	ImpNode *		FindNodeFromId(short id);
	WkNodeList *	FindEntry(TCHAR *name);
	...
	ImpNode *		MakeANode(const TCHAR *name, BOOL target,TCHAR *owner);
