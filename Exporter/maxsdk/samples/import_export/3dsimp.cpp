...
ImpNode *
ObjWorker::MakeANode(const TCHAR *name, BOOL target, TCHAR *owner) {
	int type;
	...
	if(!target) {
		// RB: changed cou
		obj = (Object *)FindObject(const_cast<TCHAR*>(name), type, cstShad, rcvShad, mtlNum);
		if(obj == NULL) {
			if(showPrompts)
				MessageBox(const_cast<TCHAR*>(name),IDS_DB_NO_OBJECT);
			return NULL;
			}
		UseObject(const_cast<TCHAR*>(name));
		}
	else {
		type = OBJ_TARGET;
...
int
ObjWorker::AddNode(ImpNode *node,const TCHAR *name,int type,Mesh *msh,TCHAR *owner, int mtlNum) {
...
/*TriObject*/ void *
ObjWorker::FindObject(TCHAR *name, int &type, int &cstShad, int &rcvShad, int &mtlNum) {
	TSTR wname(name);
	WkObjList *ptr = objects;
	while(ptr) {
...
int
ObjWorker::UseObject(TCHAR *name) {
	TSTR wname(name);
	WkObjList *ptr = objects;
	while(ptr) {
		if(_tcscmp(ptr->name,wname)==0) {
...
WkNodeList *
ObjWorker::FindEntry(TCHAR *name) {
	TSTR wname(name);
	WkNodeList *ptr = nodes;
	while(ptr) {
		if(_tcscmp(ptr->name,wname)==0)
...
//	#ifdef LATER
		...
		case BIT_MAP: {
			...
			split_fn(NULL,BG.bkgd_map,TSTR::FromCP(codePage, buf));
...
//			goto skiprest;
		...
		case PROC_NAME:
			...
			if(read_string(t->proc_name,stream,9)==0)
				return(0);
			chunk.size-=(long)(_tcslen(t->proc_name)+1);
			goto skiprest;
		case PROC_DATA: 
			{
			ULONG *plong;
...
//			 	antiquated=1;
			...
			mtlnum = theWorker->GetMatNum(TSTR::FromCP(codePage, mtlname));
...
//#ifdef LATER
		case DL_EXCLUDE: {
			...
			//	return(0);
			d->excList.AddName(TSTR::FromCP(codePage, oname));
			goto skiprest;
			}
