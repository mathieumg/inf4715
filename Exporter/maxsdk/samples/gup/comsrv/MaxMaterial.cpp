...
CMaxMaterial::ReadINI()
{
    ...
    GetPrivateProfileString(COMSRV_INI_SECTION, COMSRV_INI_PROMPT_PUTMTL, buffer, buffer, _countof(buffer), filename);
...
CMaxMaterial::WriteINI()
{
    ...
    _stprintf(buffer, _T("%d"), sdwPutMtlNoPrompt);
    MaxSDK::Util::WritePrivateProfileString(COMSRV_INI_SECTION, COMSRV_INI_PROMPT_PUTMTL, buffer, filename);
...
STDMETHODIMP CMaxMaterial::loadXML(IUnknown *pXMLNode)
{
	...
	CComBSTR temp;
	mpXML->get_xml(&temp);
	MessageBox(NULL, COLE2T(temp.m_str), NULL, MB_OK);
#endif
...
const TCHAR* GetNameInCache(const TCHAR* fname, unsigned int frame)
{
    static TCHAR buffer[MAX_PATH+1];
    ZeroMemory(buffer, sizeof(buffer));

#ifndef USE_PNG
    _stprintf(buffer, _T("%s.%d.bmp"), fname, frame);
#else
    _stprintf(buffer, _T("%s.%d.png"), fname, frame);
#endif
                  ...
                  LPBYTE lpBits)
{
   ...
   if(res<0)
	   return false;
   bitmap.Save(WStr::FromMCHAR(pszFile).data(), &pngClsid, NULL);
   return true;
...
void CMaxMaterial::ThumbnailCache::Invalidate()
{
	...
	{
		ATLASSERT((*it).second);
		if(PathFileExists(COLE2A((*it).second)))
		{
			BOOL res = DeleteFile(COLE2A((*it).second));
			if(!res)
			{
...
INode *DropNode(DWORD_PTR hWnd, POINT point)
{
	...
	ViewExp& vp = GetCOREInterface()->GetViewExp(vpwin);
	ATLASSERT(vp.IsAlive());
	vpwin = vp.GetHWnd();
	if(vp)
	{
		if (vp.IsEnabled())
...
//			GetCursorPos(&pt);
			...
			else
			{
				...
				MessageBox((HWND)hWnd, TSTR( GetString(IDS_HIT_FAILED)), TSTR(GetString(IDS_HIT_FAILED_CAPTION)), MB_OK);
				return S_FALSE;
			}
