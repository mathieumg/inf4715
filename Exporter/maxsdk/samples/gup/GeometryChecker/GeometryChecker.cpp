...
void IGeometryCheckerManager_Imp::LoadPreferences ()
{
	...
	if (pathMgr->DoesFileExist(iniFileName)) 
	{
		...
		if (GetPrivateProfileString(kINI_SECTION_NAME, kINI_SEE_THROUGH, defaultVal, buf, _countof(buf), iniFileName))
		{
			int val = _tstoi(buf);
			mSeeThrough = ( (val==1) ? true: false);
		}
		_stprintf_s(defaultVal, _countof(defaultVal), _T("%d"), ((GetAutoUpdate()== true) ? 1 : 0));
		if (GetPrivateProfileString(kINI_SECTION_NAME,kINI_AUTO_UPDATE, defaultVal, buf, _countof(buf), iniFileName))
		{
			int val = _tstoi(buf);
			mAutoUpdate = ( (val==1) ? true: false);
		}
		_stprintf_s(defaultVal, _countof(defaultVal), _T("%d"), ((GetDisplayTextUpTop()== true) ? 1 : 0));
		if (GetPrivateProfileString(kINI_SECTION_NAME, kINI_DISPLAY_TEXT_UP_TOP, defaultVal, buf, _countof(buf), iniFileName))
		{
			int val = _tstoi(buf);
