...
class NGonObject: public SimpleSpline, public IParamArray {			   
	...
	public:
		...
		const TCHAR *GetObjectName() { return GetString(IDS_TH_NGON); }
		void InitNodeName(TSTR& s) { s = GetString(IDS_TH_NGON); }		
		Class_ID ClassID() { return Class_ID(NGON_CLASS_ID,0); }  
		void GetClassName(TSTR& s) { s = GetString(IDS_TH_NGON_CLASS); }
...
TSTR NGonObject::GetParameterName(int pbIndex) 
	...
	switch (pbIndex) {
		...
		case PB_FILLET:
			return GetString(IDS_AP_FILLET);			
		case PB_SIDES:
			return GetString(IDS_TH_SIDES);			
		case PB_CIRCULAR:
			return GetString(IDS_TH_CIRCULAR);			
		case PB_SCRIBE:
			return GetString(IDS_TH_SCRIBE);			
		default:
			return _T("");
		}
	}
