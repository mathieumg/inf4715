...
class TargetObject: public GeomObject {			   
	...
	public:
		...
		const TCHAR *GetObjectName() { return GetString(IDS_DB_TARGET); }
		...
		void GetClassName(TSTR& s) { s = GetString(IDS_DB_TARGET_CLASS); }
		...
		virtual BaseInterface* GetInterface(Interface_ID iid);
