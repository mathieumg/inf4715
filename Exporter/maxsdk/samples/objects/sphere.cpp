...
#define F(s1, s2, s1r, s1c, s2r, s2c) \
		...
		if (hemi > 0.0f) {
			...
			s->FlipNormals(TRUE);
			s->Renderable(TRUE);
			_stprintf(sname, _T("%s%s%01"), bname, GetString(IDS_CT_CAP));
			s->SetName(sname);
...
TSTR SphereObject::GetParameterName(int pbIndex) 
	...
	switch (pbIndex) {
		...
		case PB_SEGS:
			return GetString(IDS_RB_SEGS);			
		case PB_SMOOTH:
			return GetString(IDS_RB_SMOOTH);			
		case PB_SLICEON: return GetString(IDS_AP_SLICEON);
		case PB_SLICEFROM: return GetString(IDS_AP_SLICEFROM);
		case PB_SLICETO: return GetString(IDS_AP_SLICETO);
		default:
			return _T("");
		}
	}
