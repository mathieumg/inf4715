...
class HelixObject: public SimpleShape, public IParamArray {			   
	...
	public:	
		...
		const TCHAR *GetObjectName() { return GetString(IDS_TH_HELIX); }
		void InitNodeName(TSTR& s) { s = GetString(IDS_TH_HELIX); }		
		Class_ID ClassID() { return Class_ID(HELIX_CLASS_ID,0); }  
		void GetClassName(TSTR& s) { s = GetString(IDS_TH_HELIX_CLASS); }
...
TSTR HelixObject::GetParameterName(int pbIndex) 
	...
	switch (pbIndex) {
		case PB_RADIUS1:
			return GetString(IDS_RB_RADIUS1);
		case PB_RADIUS2:
			return GetString(IDS_RB_RADIUS2);
		case PB_HEIGHT:
			return GetString(IDS_TH_HEIGHT);
		case PB_TURNS:
			return GetString(IDS_TH_TURNS);
		case PB_BIAS:
			return GetString(IDS_TH_BIAS);
		case PB_DIRECTION:
			return GetString(IDS_TH_DIRECTION);
		default:
			return _T("");
		}
	}
