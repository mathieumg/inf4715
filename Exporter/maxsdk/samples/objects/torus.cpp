...
class TorusObject : public SimpleObject, public IParamArray, public RealWorldMapSizeInterface {
	public:
		...
		const TCHAR *GetObjectName() { return GetString(IDS_RB_TORUS); }
		...
		{ 
			if (id == RWS_INTERFACE) 
				...
				BaseInterface* intf = SimpleObject::GetInterface(id);
				if (intf)
					return intf;
				else
...
TSTR TorusObject::GetParameterName(int pbIndex) 
	...
	switch (pbIndex) {
		...
		case PB_ROTATION:	return GetString(IDS_RB_ROTATION2);
		case PB_TWIST:		return GetString(IDS_RB_TWIST);
		case PB_SEGMENTS:	return GetString(IDS_RB_SEGS);					
		...
		case PB_PIESLICE1:	return GetString(IDS_RB_SLICETO);
		case PB_PIESLICE2:	return GetString(IDS_RB_SLICEFROM);		
		default: return _T("");
		}
	}
