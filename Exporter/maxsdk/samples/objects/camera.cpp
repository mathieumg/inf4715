...
public:
  ...
  const TCHAR *GetObjectName()(void) {return GetString(IDS_PARALLEL_CAMERA);}
  ...
  void GetClassName(TSTR& s) {s = GetString(IDS_PARALLEL_CAMERA_CLASS);}
...
{ // stolen from simple camera - used to keep constant sizes despite viewport zoom changes
	...
	tm = inode->GetObjectTM(t);
	tm.NoScale();
	float scale = vpt.NonScalingObjectSize() * vpt.GetVPWorldWidth(tm.GetTrans()) / 360.0f;
	if(scale != 1.0f)
    ...
    case REFMSG_GET_PARAM_NAME:
      {
        ...
        {
        case PB_PCAM_AXIS_ANGLE_X:
          gpn->name = GetString(IDS_PCAM_AXIS_ANGLE_X);
          break;
        case PB_PCAM_AXIS_ANGLE_Y:
          gpn->name = GetString(IDS_PCAM_AXIS_ANGLE_Y);
          break;
        case PB_PCAM_AXIS_ANGLE_Z:
          gpn->name = GetString(IDS_PCAM_AXIS_ANGLE_Z);
          break;
        case PB_PCAM_HEIGHT:
          gpn->name = GetString(IDS_PCAM_HEIGHT);
          break;
        case PB_PCAM_WIDTH:
          gpn->name = GetString(IDS_PCAM_WIDTH);
          break;
        case PB_PCAM_ASPECT:
          gpn->name = GetString(IDS_PCAM_ASPECT);
          break;
        case PB_PCAM_CLIP_NEAR:
          gpn->name = GetString(IDS_RB_NEARPLANE);
          break;												
