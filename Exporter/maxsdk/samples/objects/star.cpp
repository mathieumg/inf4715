...
class StarObject: public SimpleSpline, public IParamArray {			   
	...
	public:
		...
		const TCHAR *GetObjectName() { return GetString(IDS_TH_STAR); }
		void InitNodeName(TSTR& s) { s = GetString(IDS_TH_STAR); }		
		Class_ID ClassID() { return Class_ID(STAR_CLASS_ID,0); }  
		void GetClassName(TSTR& s) { s = GetString(IDS_TH_STAR_CLASS); }
...
TSTR StarObject::GetParameterName(int pbIndex) 
	...
	switch (pbIndex) {
		case PB_RADIUS1:
			return GetString(IDS_TH_RADIUS1);
		case PB_RADIUS2:
			return GetString(IDS_TH_RADIUS2);
		case PB_FILLET1:
			return GetString(IDS_AP_FILLET1);
		case PB_FILLET2:
			return GetString(IDS_AP_FILLET2);
		case PB_POINTS:
			return GetString(IDS_TH_STARPOINTS);
		case PB_DISTORT:
			return GetString(IDS_TH_DISTORTION);
		default:
			return _T("");
		}
	}
