...
class RectangleObject: public SimpleSpline, public IParamArray {			   
	...
	public:	
		...
		const TCHAR *GetObjectName() { return GetString(IDS_TH_RECTANGLE); }
		void InitNodeName(TSTR& s) { s = GetString(IDS_TH_RECTANGLE); }		
		Class_ID ClassID() { return Class_ID(RECTANGLE_CLASS_ID,0); }  
		void GetClassName(TSTR& s) { s = GetString(IDS_TH_RECTANGLE_CLASS); }
...
TSTR RectangleObject::GetParameterName(int pbIndex) 
	...
	switch (pbIndex) {
		case PB_LENGTH:
			return GetString(IDS_RB_LENGTH);			
		case PB_WIDTH:
			return GetString(IDS_RB_WIDTH);
		case PB_FILLET:
