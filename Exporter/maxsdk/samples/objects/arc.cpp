...
TSTR ArcObject::GetParameterName(int pbIndex) 
	...
	switch (pbIndex) {
		case PB_RADIUS:
			return GetString(IDS_TH_RADIUS);			
		case PB_FROM:
			return GetString(IDS_TH_FROM);
		case PB_TO:
			return GetString(IDS_TH_TO);
		default:
			return _T("");
		}
	}
...
const TCHAR *ArcObject::GetObjectName() 
{ 
	switch(mArcType)
	{
...
void ArcObject::GetClassName(TSTR& s)
{
	switch(mArcType)
	{
		case QUARTERROUND: s = GetString(IDS_TH_QUARTERROUND_CLASS);
		case HALFROUND: s = GetString(IDS_TH_HALFROUND_CLASS );
		default: s= GetString(IDS_TH_ARC_CLASS);
	}
}
