...
class MorphKeySizeValidator {
	MorphKeySizeValidator() {}  // not creatable
	MaxSDK::Util::StaticAssert< (sizeof(MorphKey) == sizeof(DWORD)+sizeof(TimeValue)) > validator;
};
...
// saved as a binary blob to scene file. Changes in size will break file i/o
class CubicMorphKeySizeValidator {
	CubicMorphKeySizeValidator() {}  // not creatable
	MaxSDK::Util::StaticAssert< (sizeof(CubicMorphKey) == sizeof(MorphKey)+7*sizeof(float)+sizeof(int)) > validator;
};
...
// saved as a binary blob to scene file. Changes in size will break file i/o
class TargetWeightSizeValidator {
	TargetWeightSizeValidator() {}  // not creatable
	MaxSDK::Util::StaticAssert< (sizeof(TargetWeight) == sizeof(int)+sizeof(float)) > validator;
};
...
class BaryMorphKeySizeValidator {
	BaryMorphKeySizeValidator() {}  // not creatable
#ifdef _WIN64
	MaxSDK::Util::StaticAssert< (sizeof(BaryMorphKey) == sizeof(MorphKey)+7*sizeof(float)+sizeof(void*)+4) > validator; // 4 bytes padding - 64 bit build only
#else
	MaxSDK::Util::StaticAssert< (sizeof(BaryMorphKey) == sizeof(MorphKey)+7*sizeof(float)+sizeof(void*)) > validator;
#endif
};
