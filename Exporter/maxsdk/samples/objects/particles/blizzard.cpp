...
//          macroRec->Enable();
   ...
   abort:
      ...
      case MOUSE_FREEMOVE:
         ...
         if (createCB) res =createCB->proc(vpx.ToPointer(),msg,point,flag,m,mat);
#endif
...
{   
	...
	_tcscpy(filename,ip->GetDir(APP_PLUGCFG_LN_DIR));
...
{ TCHAR filename[MAX_PATH];
  ...
  { MessageBox(NULL,GetString(IDS_AP_WRITEPRO),_T(""), MB_ICONINFORMATION);
   return 0;
  }
  if (custsettings==1) 
...
const TCHAR *SuprSprayParticle::GetObjectName() {return GetString(IDS_AP_SUPRSPRAYGC);}
...
const TCHAR *BlizzardParticle::GetObjectName() {return GetString(IDS_AP_BLIZZARDGC);}
...
// from IDisplay
bool CommonParticle::RequiresSupportForLegacyDisplayMode() const
{
	return true ;
}

bool CommonParticle::UpdateDisplay(
	const MaxSDK::Graphics::MaxContext& maxContext, 
	const MaxSDK::Graphics::UpdateDisplayContext& displayContext)
{
	...
	BOOL doupdate=((!cancelled)&&((t!=tvalid)||!valid));
	if (!doupdate) doupdate=CheckMtlChange(cnode->GetMtl(),wasmulti);
	if ((t!=dispt)||doupdate||!dispmesh)
	{
		NullView nullView;
		BOOL needdel;
		if (dispmesh) delete dispmesh;
		SetAFlag(A_NOTREND);
		dispmesh=GetRenderMesh(t,cnode,nullView,needdel);  
		ClearAFlag(A_NOTREND);
	}
	if(dispmesh)
	{
		...
		dispmesh->GenerateRenderItems(mRenderItemHandles,generateRenderItemsContext);
		return true;
	 }
