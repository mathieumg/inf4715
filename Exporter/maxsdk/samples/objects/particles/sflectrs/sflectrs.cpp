...
// monoflector and the dynaflectors have been removed
...
int RayIntersectP(Ray& ray, float& at, Point3& norm,Mesh *amesh,MaxSDK::VertexNormal* vnorms,Point3 *fnorms)
   {
   DWORD fi;
   Point3 bary;
...
{ 
  if (_tcslen(name)>MAXNAME) name[MAXNAME]=_T('\0');
}
