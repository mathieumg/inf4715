...
extern BOOL doLocationDialog(HWND hParent, IObjParam* ip, float* latitude,
	float* longitude, TCHAR* cityName);
...
TSTR SunMaster::SubAnimName(int i)
{
	switch (i) {
	case SUBANIM_PBLOCK:
		return MaxSDK::GetResourceStringAsMSTR(IDS_DB_PARAMETERS);
	case SUBANIM_LOOKAT:
		return MaxSDK::GetResourceStringAsMSTR(IDS_LOOKAT);
	case SUBANIM_MULTIPLIER:
		return MaxSDK::GetResourceStringAsMSTR(IDS_MULTIPLIER);
	}
	return TSTR();
}
...
static TCHAR *JapaneseCityName =_T("Tokyo Japan");
static TCHAR *ChineseCityName =_T("Beijing China");
static TCHAR *KoreanCityName =_T("Seoul Korea");
static TCHAR *EnglishCityName =_T("San Francisco, CA");

void SunMaster::SetCityInfo(TCHAR* name, int& zone, float& lati, float& longi)
{
...
// Sets the city name.
void SunMaster::SetCity(const TCHAR* name)
{
	const TCHAR* nm = name;
	if (nm != NULL)
	{
...
//
void SunMaster::SetDate(TimeValue t, const JulianCalendar& jd) { 
	double t1;
...
void SunMaster::SetTime(TimeValue t, const JulianCalendar& jd) { 
...
{
	s = MaxSDK::GetResourceStringAsMSTR(id == LIGHT_TM ? IDS_SLAVE_POS_CLASS : IDS_SLAVE_FLOAT_CLASS);
}
...
SClass_ID SlaveControl::SuperClassID()
{
	...
	case LIGHT_TM:
		{
			// In Rampage, we corrected the superclass ID (from POSITION).  To support STP, 
			// we need to return the old SCID for versions 12 and down...
			int iSaveVer = GetSavingVersion();
			if (iSaveVer > 0 && iSaveVer < MAX_RELEASE_R14)
