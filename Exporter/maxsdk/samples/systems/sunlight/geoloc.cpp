...
class LocationDialog
{
	...
	private:
		...
		HANDLE openMapFile(TCHAR* mapName);
...
// Handle a change of city.
{
	...
	_tcscpy(lastCityName, CityList::GetInstance()[itemIndex].name);
...
HANDLE LocationDialog::openMapFile(TCHAR* mapName)
{
	HANDLE hMap = NULL;
...
// Update the list of cities.
{
	...
	{
		CityList::Entry& cl = CityList::GetInstance()[i];
...
int LocationDialog::mapClick(HWND hDlg, HWND hButton)
{
	...
	{ 
		...
		if (foundClosest)
		{
			...
			_tcscpy(lastCityName, CityList::GetInstance()[closestCity].name);
