...
// Link to the mrSky environment texture
...
const TCHAR* NatLightAssembly::GetObjectName()
{
...
)
{
	switch (message) {
		case REFMSG_CHANGE: {
			...
			if (pBGTex != NULL && pBGTex->ClassID() == MRSKYENVSHADER_CLASS_ID)
			{
...
{
	s = MaxSDK::GetResourceStringAsMSTR(IDS_NAT_LIGHT_NAME);
}
