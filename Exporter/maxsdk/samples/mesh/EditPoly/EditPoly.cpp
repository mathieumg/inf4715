...
void EditPolyMod::LoadPreferences ()
{
	...
	if (pathMgr->DoesFileExist(iniFileName)) 
	{
		...
		if (GetPrivateProfileString(kINI_SECTION_NAME, kINI_CREATE_SHAPE_SMOOTH_OPT, defaultVal, buf, _countof(buf), iniFileName))
		{
		...
		if (GetPrivateProfileString(kINI_SECTION_NAME, kINI_DETACH_TO_OBJECT_OPT, defaultVal, buf, _countof(buf), iniFileName))
		{
      ...
      mDetachToObject = (detachToObject != 0);
		...
		if (GetPrivateProfileString(kINI_SECTION_NAME, kINI_DETACH_AS_CLONE_OPT, defaultVal, buf, _countof(buf), iniFileName))
		{
...
TSTR EditPolyMod::SubAnimName(int i)
{
   ...
   if (mPointRange.Count()>2) {
      ...
      if (j!=mPointRange.Count()) 
	      buf.printf (_T("%s: %s %d"), mPointNode[j-1], GetString(IDS_VERTEX), i+1-mPointRange[j-1]);
   } else {
      // Edit Poly only applied to one node, so leave out the node name.
      buf.printf (_T("%s %d"), GetString (IDS_VERTEX), i+1);
   }
   return buf;
}
...
void EditPolyMod::DoubleClickSelect(HitRecord *hitRec)
{
	...
	if (!pData->GetNewSelection ()) {
		...
		pData->GetNewSelection()->Set (hitRec->hitInfo, true);
		pData->GetMesh()->SelectEdgeLoop(*pData->GetNewSelection());
	}
	...
	BOOL Controltriggered = GetKeyState(VK_CONTROL) & 0x8000;
	if (pData->ApplyNewSelection(this, Controltriggered? true: false, false, true))
	{
...
public:     
   ...
   void Restore(int isUndo);
   void Redo();            
   TSTR Description() {return _T("Add Set Name");}
};
...
public:     
   ...
   TSTR Description() {return _T("Delete Set Name");}
};
