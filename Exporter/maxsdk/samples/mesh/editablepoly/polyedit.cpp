...
EditPolyObject::~EditPolyObject()
{
	...
	if (NULL != mpAddRenderItemsHelper)
	{
		mpAddRenderItemsHelper->DeleteInterface();
	}
}
		...
		if ((ip->GetCommandMode()==createFaceMode) ||
			...
			(selLevel == EP_SL_VERTEX)) {
				...
				for (i=0; i<pMeshToDisplay->numv; i++) {
					...
					if (pVert->GetFlag (MN_HIDDEN|MN_DEAD)) continue;
#ifdef MESH_CAGE_BACKFACE_CULLING
...
bool EditPolyObject::UpdateDisplay(
	...
	const MaxSDK::Graphics::UpdateDisplayContext& displayContext)
{
	...
	if (useSubdiv) 
	{
		...
		UpdatePolyObjectDisplay(subdivResult,mRenderItemHandles,displayContext);
		return true;
	}
	else if (EpPreviewOn())
	{
		mm.InvalidateHardwareMesh(1);
		UpdatePolyObjectDisplay(mPreviewMesh,mRenderItemHandles,displayContext);
		return true;
	}
	else
	{
		return PolyObject::UpdateDisplay(maxContext, displayContext);
	}
...
int EditPolyObject::Display (TimeValue t, INode* inode, ViewExp *vpt, int flags) {
	...
	if (pMeshToDisplay) {
		...
		if ( vsw ) {
			for (std::vector<MNMapFace>::iterator it = vertexSelFaces.begin();
				it != vertexSelFaces.end();
				it++)
			{
				(*it).Clear();
			}
	...
	if(selLevel>0)
	{
		...
		{ 
			...
			// by default, Max uses 1.005f to draw edges. In Nitrous, we need the highlighted sub objects over the edges.
...
void EditPolyObject::DoubleClickSelect(HitRecord *hitRec) {
	...
	if ((GetKeyState(VK_SHIFT) & 0x8000) || (GetKeyState(VK_MENU) & 0x8000)) return;
	...
	BitArray hitArray;
	hitArray.SetSize (mm.nume);
	hitArray.Set (hitRec->hitInfo);

	mm.SelectEdgeLoop (hitArray);
	...
	SetEdgeSel (hitArray, this, TimeValue(0));
	mm.getEdgeSel (esel);
...
void EditPolyObject::ClearSelection(int sl) {
	...
	bool inMayaSelectionMode = MaxSDK::CUI::GetIMouseConfigManager()->GetMayaSelection();
	if (!inMayaSelectionMode && (GetKeyState(VK_SHIFT) & 0x8000)) return; //Do not clear selection when attempting to select loop or ring
	BitArray sel;
	switch (sl) {
...
void EditPolyObject::InvalidateSoftSelectionCache () {
	...
	if (pblock) pblock->GetValue (ep_surf_subdivide, TimeValue(0), useSubdiv, FOREVER);
...
class EditPolyObjectAddRenderItemsHelper : public MaxSDK::Graphics::IAddRenderItemsHelper
{
public:
	EditPolyObjectAddRenderItemsHelper(EditPolyObject* pObject);

	virtual bool AddRenderItems(
		const MaxSDK::Graphics::MaxContext& maxContext, 
		MaxSDK::Graphics::RenderNodeHandle& hRenderNode, 
		MaxSDK::Graphics::IRenderItemContainer& targetRenderItemContainer);

private:
	EditPolyObject* mpObject;
};

EditPolyObjectAddRenderItemsHelper::EditPolyObjectAddRenderItemsHelper(EditPolyObject* pObject) : 
	mpObject(pObject)
{ }

bool EditPolyObjectAddRenderItemsHelper::AddRenderItems(
	const MaxSDK::Graphics::MaxContext& maxContext, 
	MaxSDK::Graphics::RenderNodeHandle& hRenderNode, 
	MaxSDK::Graphics::IRenderItemContainer& targetRenderItemContainer)
{
	...
	const MaxSDK::Graphics::RenderItemHandleArray& renderItems = mpObject->GetRenderItems();
	IObjectDisplay::DefaultAddRenderItems(maxContext,
		hRenderNode,
		targetRenderItemContainer,
		renderItems);

	// Next we check if diagonal render items is required.
	Interface* coreInterface = GetCOREInterface();
	bool inModifyPanel = (coreInterface->GetCommandPanelTaskMode() == TASK_MODE_MODIFY);
	if (!inModifyPanel || !hRenderNode.GetSelected())
	{
		return true;
	}

	if (EditPolyObject::editTriMode == coreInterface->GetCommandMode() ||
		EditPolyObject::turnEdgeMode == coreInterface->GetCommandMode())
	{
		MeshElementDescription diagonalDescription = GetBuiltInMeshElementDescription(ElementDescriptionDiagonal);
		unsigned long descriptionBits = diagonalDescription.GetDescriptionBits();
		if (!targetRenderItemContainer.FindRenderItem(descriptionBits).IsValid())
		{
			RenderItemHandle& diagonal = renderItems.FindRenderItem(descriptionBits);
			if (diagonal.IsValid())
			{
				targetRenderItemContainer.AddRenderItem(diagonal);
			}
		}

		diagonalDescription.SetMeshPart(MeshPartSelected);
		descriptionBits = diagonalDescription.GetDescriptionBits();
		if (!targetRenderItemContainer.FindRenderItem(descriptionBits).IsValid())
		{
			RenderItemHandle& selDiagonal = renderItems.FindRenderItem(descriptionBits);
			if (selDiagonal.IsValid())
			{
				targetRenderItemContainer.AddRenderItem(selDiagonal);
			}
		}
	}
...
BaseInterface *EditPolyObject::GetInterface (Interface_ID id) {
	...
	if (id == IADD_RENDERITEMS_HELPER_INTERFACE_ID) 
	{
		if (NULL == mpAddRenderItemsHelper)
		{
			mpAddRenderItemsHelper = new EditPolyObjectAddRenderItemsHelper(this);
		}

		return mpAddRenderItemsHelper;
	}
...
void EditPolyObject::ApplyDelta (Tab<Point3> & delta, EPoly *pEPoly, TimeValue t) {
	...
	IMNMeshUtilities8* pMNMeshUtility = static_cast<IMNMeshUtilities8*>(
		mm.GetInterface( IMNMESHUTILITIES8_INTERFACE_ID ));
