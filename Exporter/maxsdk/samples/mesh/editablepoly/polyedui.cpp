...
protected:
	...
	void OverrideDoubleClickProc(ViewExp* vpt, int flags) {
		...
		if (NULL != pPolyObj && pPolyObj->GetSelLevel() == IMESHSEL_EDGE)
		{
			theHold.Begin();
...
{
private:
	PolySubModSelectionProcessor polyMouseProc;
	ModBoxType					 theProc;			
public:
	PolyDoubleClickModeDecorator(BaseObject* o, IObjParam* i) :
	  BasePolyMode(o, i),
	  theProc(o, i),
	  polyMouseProc(&theProc, o, i)
	{
		polyMouseProc.SetSupportTransformGizmo(TRUE);
	}
	  
	MouseCallBack *MouseProc(int *numPoints) 
	{ 
		*numPoints=2;
		return &polyMouseProc; 
	}
};

template <class BasePolyMode>
class PolyDoubleClickModeDecorator<BasePolyMode, SelectModBox>: public BasePolyMode
{
private:
	PolySubModSelectionProcessor polyMouseProc;
public:
...
void EditPolyObject::BeginEditParams (IObjParam  *ip, ULONG flags,Animatable *prev) {
	...
	moveMode       = new PolyDoubleClickModeDecorator<MoveModBoxCMode, MoveModBox>(this,ip);
	rotMode        = new PolyDoubleClickModeDecorator<RotateModBoxCMode, RotateModBox>(this,ip);
	uscaleMode     = new PolyDoubleClickModeDecorator<UScaleModBoxCMode, ScaleModBox>(this,ip);
	nuscaleMode    = new PolyDoubleClickModeDecorator<NUScaleModBoxCMode, ScaleModBox>(this,ip);
	squashMode     = new PolyDoubleClickModeDecorator<SquashModBoxCMode, ScaleModBox>(this,ip);
	selectMode     = new PolyDoubleClickModeDecorator<SelectModBoxCMode, SelectModBox>(this,ip);
