...
void MorphViewUtil::LoadMorpherInfo( HWND hWnd )
{
	...
	{
		TCHAR s[255];
    _stprintf(s,_T("%i - %s"),i+1,mp->chanBank[i].mName);
...
void MorphViewUtil::LoadChannelInfo( HWND hWnd, int idx )
{
	...
	TCHAR s[255];
	_stprintf(s,_T("%i"),mp->chanBank[idx].mNumPoints);
	SetWindowText(GetDlgItem(hWnd,IDC_NPTS),s);
	_stprintf(s,_T("%i"),mp->chanBank[idx].mPoints.size());
