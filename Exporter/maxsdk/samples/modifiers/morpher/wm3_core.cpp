	...
	for(i=0;i<100;i++)
	{
		if( chanBank[i].mActive )
		{
			...
			if(chanBank[i].mUseLimit||glUseLimit)
			{
				...
				{
					Pmax = (int)chanBank[i].mSpinmax;
					Pmin = (int)chanBank[i].mSpinmin;
				}
			...
			for(pointnum=0; pointnum < chanBank[i].mNumPoints; pointnum++)
			{
				...
				if ( (!chanBank[i].mUseSel && glUsesel==0)  || pointSelected)
				{
					...
					{
						deltX=(float)(((chanBank[i].mDeltas[pointnum].x)*fChannelPercent)*decay);
						deltY=(float)(((chanBank[i].mDeltas[pointnum].y)*fChannelPercent)*decay);
						deltZ=(float)(((chanBank[i].mDeltas[pointnum].z)*fChannelPercent)*decay);
					}
					else  
					{	//DO PROGRESSIVE MORPHING
						...
						totaltargs = (float)(chanBank[i].mNumProgressiveTargs +1);
						...
						if(segment==1) {
							...
							if(fabs(length) < epsilon ) {
								...
								splinepoint[2] = endpoint[1] - 
									(DotProd(temppoint[0], temppoint[1]) * chanBank[i].mCurvature / length) * temppoint[1];
								splinepoint[1] = endpoint[0] +  
							...
							if(fabs(length) < epsilon) {
								...
								Point3 p = (DotProd(temppoint[1], temppoint[0]) * chanBank[i].mCurvature / length) * temppoint[1];
								splinepoint[1] = endpoint[1] - 
									(DotProd(temppoint[1], temppoint[0]) * chanBank[i].mCurvature / length) * temppoint[1];
...
BOOL IMorphClass::AddProgessiveMorph(MorphR3 *mp,int morphIndex, INode *node)
{
	...
	int refnum = (morphIndex*MAX_PROGRESSIVE_TARGETS) + 201 + bank.mNumProgressiveTargs;
