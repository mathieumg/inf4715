...
const TCHAR *BonesDefMod::GetBoneName(int index)
{
...
static const int RIGHT_MARGIN_ARROW_BTN = 23;
static const int TOP_MARGIN_ARROW_BTN = 6;
static const int LEFT_MARGIN_BTN_TEXT = 7;
static const int TOP_MARGIN_BTN_TEXT = 5;
static const int DOWN_ARROW_INDEX = 0;
static const int UP_ARROW_INDEX = 1;

LRESULT CALLBACK PaintUpArrowProc(HDC hdc, Rect rect, BOOL in, BOOL checked, BOOL enabled)
{
	COLORREF oldTextColor = SetTextColor(hdc, GetCustSysColor(COLOR_BTNTEXT));
	DLTextOut( hdc, rect.left + LEFT_MARGIN_BTN_TEXT, rect.top + TOP_MARGIN_BTN_TEXT, GetString(IDS_BONEDEF_NAME));
	ImageList_Draw(hArrowImages, UP_ARROW_INDEX, hdc, rect.right - RIGHT_MARGIN_ARROW_BTN, rect.top + TOP_MARGIN_ARROW_BTN, ILD_NORMAL);
	SetTextColor(hdc, oldTextColor);
	return TRUE;
}

LRESULT CALLBACK PaintDownArrowProc(HDC hdc, Rect rect, BOOL in, BOOL checked, BOOL enabled)
{
	COLORREF oldTextColor = SetTextColor(hdc, GetCustSysColor(COLOR_BTNTEXT));
	DLTextOut( hdc, rect.left + LEFT_MARGIN_BTN_TEXT, rect.top + TOP_MARGIN_BTN_TEXT, GetString(IDS_BONEDEF_NAME));
	ImageList_Draw(hArrowImages, DOWN_ARROW_INDEX, hdc, rect.right - RIGHT_MARGIN_ARROW_BTN, rect.top + TOP_MARGIN_ARROW_BTN, ILD_NORMAL);
	SetTextColor(hdc, oldTextColor);
	return TRUE;
}
...
//	int i;
	...
	switch (msg) {
		case WM_INITDIALOG:
			...
			{
				const int ARROW_IMAGE_WIDTH = 12;
				const int ARROW_IMAGE_HEIGHT = 12;
			...
			{
				mod->iSortText->SetDisplayProc(PaintUpArrowProc);
			}
			else 
			{
				mod->iSortText->SetDisplayProc(PaintDownArrowProc);
			}
...
*/
         ...
         EnableWindow(GetDlgItem(hWnd,IDC_SHRINK),FALSE);
			...
			mod->SelectBoneByParmIndex(mod->ModeBoneIndex);
                ...
                case IDC_SORTBYNAME:
                    {
                        ...
                        mod->RefillListBox();
                        mod->mParameterListSelection = 
                            mod->ConvertSelectedBoneToListID(mod->ModeBoneIndex);

                        SendMessage(GetDlgItem(hWnd,IDC_LIST1),
                            LB_SETCURSEL , mod->mParameterListSelection, 0);
...
//					mod->ClearVertexSelections();
				...
				case IDC_LIST1:
					...
					if (HIWORD(wParam)==LBN_SELCHANGE) {
						...
						fsel = SendMessage(GetDlgItem(hWnd,IDC_LIST1),LB_GETCURSEL,0,0);
...
BOOL BonesDefMod::LoadEnvelope(const TCHAR *fname, BOOL asText)
{
	...
	{
		MessageBox(GetCOREInterface()->GetMAXHWnd(),_T("Error loading file, file not found"),_T("Load Error"),MB_OK);
		return FALSE;
	}
	//ver
	TSTR lineRead;
	...
	if ( (ver < 0) || (ver > 4) ) 
	{
		MessageBox(GetCOREInterface()->GetMAXHWnd(),_T("Error loading file, corrupt file"),_T("Load Error"),MB_OK);
		return FALSE;
	}
...
//bone name length size
		...
		else
		{
			...
			dataList[i]->name = new TCHAR[fnameLength+1];
			_tcscpy(dataList[i]->name, nameStr.data());
			delete [] temp;
		}
	...
	//read the node name
			for (int i =0; i < nodeCount; i++)
			{
				...
				else
				{
					...
					fnameLength = nameStr.Length();
					vertexLoadList[i]->name = new TCHAR[fnameLength+1];
					_tcscpy(vertexLoadList[i]->name, nameStr.data());
					delete [] temp;
				}
...
void BonesDefMod::RefreshNamePicker()
{
	...
	if(!IsNamePickerEmpty())
	{
		ClearNamePicker();
		RefillListBox();
		mParameterListSelection = ConvertSelectedBoneToListID(ModeBoneIndex);
