...
int BonesDefMod::ConvertSelectedListToBoneID(int fsel)
{
	...
	if (!hParam) return 0;
	if (0 == SendMessage(GetDlgItem(hParam, IDC_LIST1), LB_GETCOUNT, 0, 0)) return 0;

	int sel = SendMessage(GetDlgItem(hParam, IDC_LIST1), LB_GETITEMDATA, fsel, 0);
...
// bug fix 207093 9/8/99	watje
	...
	ModeBoneIndex = sel;
	SelectBoneByParmIndex(ModeBoneIndex);
					   ...
					   Point3& val, BOOL localOrigin)
{
	...
	if (mode >0 )
	{
		...
		if (ModeBoneEndPoint == 0)
		{
			...
			macroRecorder->FunctionCall(_T("skinOps.setStartPoint"), 3, 0, mr_reftarg, this, mr_int, tempID,
				mr_point3,&val);
		}
		else if (ModeBoneEndPoint == 1)
		{
			...
			macroRecorder->FunctionCall(_T("skinOps.setEndPoint"), 3, 0, mr_reftarg, this, mr_int, tempID, mr_point3,&val);
		...
		HitRecord *hitRec, BOOL selected, BOOL all, BOOL invert)
{
	...
	while (hitRec) 
	{
		...
		if((iSelectedBoneId != -1) && (BoneData[iSelectedBoneId].bFilterOut == TRUE))
		{
			RefreshNamePicker();
		}
         ...
         for( int i = 0 ; i < BoneData.Count() ; i++)
			{
				if (BoneData[i].Node != NULL) 
				{
					...
					bool bUseObjectTM = BoneData[i].flags & BONE_SPLINE_FLAG;
					if(bUseObjectTM)
					{
...
ofn.lpstrFile       = fname;
ofn.nMaxFile        = 256;    
ofn.Flags           = OFN_HIDEREADONLY|OFN_FILEMUSTEXIST|OFN_PATHMUSTEXIST|OFN_NOREADONLYRETURN;
...
void BonesDefMod::SaveEnvelope(const TCHAR *fname, BOOL asText)
{
	...
	else file = _tfopen(fname,_T("wb"));

	if( !file && !textfile.CanWrite())
		return;
...
//if failed try to register the dll
		...
		ShellExecute(GetCOREInterface()->GetMAXHWnd(), _T("open"), _T("regsvr32.exe"), _T("/s atl.dll"), NULL, SW_SHOWNORMAL);
		Sleep(3000);
		ShellExecute(GetCOREInterface()->GetMAXHWnd(), _T("open"), _T("regsvr32.exe"), _T("/s MAXComponents.dll"), path, SW_SHOWNORMAL);
		Sleep(3000);
...
//if failed again something bad has happened and bail
		if( FAILED(hr))
			{	
			MessageBox(GetCOREInterface()->GetMAXHWnd(),_T("CoCreateInstance() failed\nPlease check your registry entries\nCLSID {F088EA74-2E87-11D3-B1F3-00C0F03C37D3} and make sure you are logged in as an administrator"),_T("COM Error"),MB_OK);
			CoUninitialize();
			return;
			}
	}
...
//				ref = Cluster->pblock_gizmos->GetReferenceTarget(skin_gizmos_list,0,id);
	...
	bmd->VertexData[i]->LocalPosPostDeform = pout;
...
void BonesDefMod::SelectBone(int currentBone)
{
	...
	if (ip)
	{
		...
		{
			TSTR holdMessage;
			holdMessage.printf(_T("%s %s"),GetString(IDS_PW_SELECT),BoneData[currentBone].Node->GetName());
	...
	if (ip)
		{
			SelectBoneByParmIndex(currentBone);
		}
...
void BonesDefMod::SelectBoneByParmIndex(int paramBlockBoneIndex) 
{
	if (paramBlockBoneIndex < 0)
		return;
	ModeBoneIndex = paramBlockBoneIndex;
	int rsel = ConvertSelectedBoneToListID(paramBlockBoneIndex);
...
void BonesDefMod::SelectBoneByListBoxIndex(int selIndex) 
{
	if (selIndex < 0)
		return;
	ModeBoneIndex = ConvertSelectedListToBoneID(selIndex);
	mParameterListSelection = selIndex;
	SendMessage(GetDlgItem(hParam,IDC_LIST1),
		LB_SETCURSEL ,selIndex,0);
}
