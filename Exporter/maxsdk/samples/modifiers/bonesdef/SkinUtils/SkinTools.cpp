...
void SkinTools::FilloutListBox()
{
	...
	{
		int index = pasteData.liveSourceBones[i];
		SendMessage(GetDlgItem(floaterHWND,IDC_SOURCELIST),LB_ADDSTRING,0,(LPARAM)(const TCHAR*)boneList[index]->GetDisplayName());
	}
	...
	for (int i = 0; i < pasteData.deadTargetBones.Count(); i++)
	{
		...
		SendMessage(GetDlgItem(floaterHWND,IDC_DEADTARGLIST),LB_ADDSTRING,0,(LPARAM)(const TCHAR*)targBoneList[index]->GetDisplayName());
	}
	...
	for (int i = 0; i < pasteData.deadSourceBones.Count(); i++)
	{
		...
		SendMessage(GetDlgItem(floaterHWND,IDC_DEADSOURCELIST),LB_ADDSTRING,0,(LPARAM)(const TCHAR*)boneList[index]->GetDisplayName());
	}
