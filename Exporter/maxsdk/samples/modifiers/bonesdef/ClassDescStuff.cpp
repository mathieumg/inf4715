...
TSTR BonesDefMod::SubAnimName(int index)
	...
	for (int i = 0; i < BoneData.Count(); i++)
		...
		if (BoneData[i].Node)
			...
			if (ct == index) 	
				{	
				animName.printf(_T("%s(%s)"),GetString(IDS_BONESTART),BoneData[i].Node->GetName());
				return animName;
				}
			ct++; 
			if (ct == index) 	
				{	
				animName.printf(_T("%s(%s)"),GetString(IDS_BONEEND),BoneData[i].Node->GetName());
				return animName;
				}
			ct++; 
			for (int j = 0; j < BoneData[i].CrossSectionList.Count(); j++)
				...
				if (ct == index) 
					{	
					animName.printf(_T("    %s(%0.2f)"),GetString(IDS_INNNERRADIUS),BoneData[i].CrossSectionList[j].u);				
					return animName;					
					}
				ct++;
				if (ct == index) 
					{	
					animName.printf(_T("    %s(%0.2f)"),GetString(IDS_OUTERRADIUS),BoneData[i].CrossSectionList[j].u);				
					return animName;
					}
				ct++;
				}
