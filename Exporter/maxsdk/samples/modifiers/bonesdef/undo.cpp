...
void DeleteBoneRestore::Restore(int isUndo) 
{
	...
	if (mod->ip)
	{
		...
		mod->SelectBoneByParmIndex(undoID);
		mod->UpdatePropInterface();
	}
...
void DeleteBoneRestore::Redo()
{
	...
	{
		mod->RefillListBox();
		mod->SelectBoneByParmIndex(redoID);
		mod->UpdatePropInterface();
	}
...
WeightRestore::WeightRestore(BonesDefMod *bmod, BoneModData *md, BOOL updateView) 
	...
	for (int i=0; i<c; i++) 
		...
		undoVertexData[i]->PasteWeights( md->VertexData[i]->CopyWeights());
		undoVertexData[i]->SetClosestBone(md->VertexData[i]->GetClosestBone());
		}
...
void WeightRestore::Restore(int isUndo) 
{
	...
	if (isUndo) 
		...
		for (int i=0; i<c; i++) 
			...
			redoVertexData[i]->PasteWeights(bmd->VertexData[i]->CopyWeights());
	...
	for (int i=0; i<c; i++) 
		...
		bmd->VertexData[i]->PasteWeights(undoVertexData[i]->CopyWeights());
		bmd->VertexData[i]->SetClosestBone(undoVertexData[i]->GetClosestBone());
		}
...
void WeightRestore::Redo()
{
	...
	for (int i=0; i<c; i++) 
		...
		bmd->VertexData[i]->PasteWeights(redoVertexData[i]->CopyWeights());

		bmd->VertexData[i]->SetClosestBone(redoVertexData[i]->GetClosestBone());
...
TSTR AddGizmoRestore::Description() { return TSTR(GetString(IDS_PW_ADDGIZMO)); }
...
TSTR SelectBoneMoveRestore::Description() { return TSTR(GetString(IDS_PW_SELECT)); }
