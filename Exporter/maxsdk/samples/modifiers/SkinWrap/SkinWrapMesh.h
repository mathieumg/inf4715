...
class ParamSpaceDataSizeValidator {
	ParamSpaceDataSizeValidator() {}  // not creatable
	MaxSDK::Util::StaticAssert< (sizeof(ParamSpaceData) == sizeof(Point3)+2*sizeof(float)+2*sizeof(int)) > validator;
};
...
// saved as a binary blob to scene file. Changes in size will break file i/o
class InitialTMDataSizeValidator {
	InitialTMDataSizeValidator() {}  // not creatable
	MaxSDK::Util::StaticAssert< (sizeof(InitialTMData) == sizeof(Matrix3)+sizeof(Point3)+sizeof(int)+2*sizeof(float)) > validator;
};
...
class MeshDeformPW : public Modifier, public IMeshDeformPWModFP 
{
	public:
		...
		// From Animatable
		const TCHAR *GetObjectName() { return GetString(IDS_MESHDEFORM); }
