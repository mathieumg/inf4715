			...
			for (int j =0; j < localData->paramData.Count(); j++)
			{
				...
				if ((ip) && (localData->paramData.Count() > 10000))
				{
					...
					if ((j%200) == 0)
					{
						...
						prompt.printf(_T("%s %d : %d(%d) (%0.0f%%)"), GetString(IDS_STEP2),wrapIndex, j,localData->paramData.Count(),per);
...
void MeshDeformPW::ModifyObject(TimeValue t, ModContext &mc, ObjectState * os, INode *incnode) 
{
	...
	if (localData->selfNode == NULL)
	{
		...
		localData->initialBaseTM = localData->selfNode->GetObjTMBeforeWSM(t, &iv);;
	}
	...
	for (int wrapIndex = 0; wrapIndex < pblock->Count(pb_meshlist); wrapIndex++)
	{
		...
		if (meshos.obj) 
		{
			...
			else wrapMeshData[wrapIndex]->currentMeshTM = node->GetObjTMBeforeWSM(t, &iv);
...
//				wrapMeshData[0]->faceIndex[i] = localData->paramData[i].faceIndex;
	...
	if (rebuilduildParamSpace)
	{
		...
		for (int wrapIndex = 0; wrapIndex < pblock->Count(pb_meshlist); wrapIndex++)
		{
			...
			else wrapMeshData[wrapIndex]->initialMeshTM = node->GetObjTMBeforeWSM(t, &iv);
			...
			case WRAPMESH_DATA:
				{
					...
					if (numberVerts > 0)
					{
						iload->ReadVoid(wrapMeshData[currentWrapMesh]->initialData.Addr(0),sizeof(InitialTMData)*numberVerts, &nb);
					}
...
IOResult MeshDeformPW::Save(ISave *isave)
{
	...
	for (int wrapIndex = 0; wrapIndex < ct; wrapIndex++)
	{
		...
		if(ict>0)
			res = isave->WriteVoid(wrapMeshData[wrapIndex]->initialData.Addr(0), sizeof(InitialTMData)*ict, &nb); // no TCHAR data
...
//free our lookup list
	...
	if (ld->selfNode)
	{
		...
		Matrix3 baseTM = ld->selfNode->GetObjTMBeforeWSM(t, &iv);
	...
	for (int wrapIndex = 0; wrapIndex < pblock->Count(pb_meshlist); wrapIndex++)
	{
		...
		*pCt = ct;
		...
		for (int i = 0; i < ct; i++) {
 		...
 		node->AddAppDataChunk(MESHDEFORMPW_CLASS_ID,OSM_CLASS_ID,CPDATA_APPDATA,sizeof(InitialTMData)*ct,initialDataTemp); // no TCHAR data
	}
}
	...
	for (int i = 0; i < localData->weightData.Count(); i++)
	{
		...
		if ((ip) && (localData->weightData.Count() > 10000))
		{
			...
			if ((i%200) == 0)
			{
				...
				prompt.printf(_T("%s : %d(%d) (%0.0f%%)"), GetString(IDS_STEP4),i,localData->weightData.Count(),per);
				ip->ReplacePrompt( prompt);
			}
		}
	...
	if (ld)
	{
		if (whichVert < ld->weightData.Count())
		{
			...
			for (int i = 0; i < ld->weightData[whichVert]->data.Count(); i++)
			{
				...
				else ScriptPrint(_T("Weight %f Owner NULL\n"),f);
			}
		}
	}
}
