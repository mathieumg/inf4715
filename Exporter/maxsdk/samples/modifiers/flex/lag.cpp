...
class EdgeBondageSizeValidator {
	EdgeBondageSizeValidator() {}  // not creatable
	MaxSDK::Util::StaticAssert< (sizeof(EdgeBondage) == sizeof(float)+3*sizeof(int)) > validator;
};
...
// saved as a binary blob to scene file. Changes in size will break file i/o
class TempStorageClassSizeValidator {
	TempStorageClassSizeValidator() {}  // not creatable
	MaxSDK::Util::StaticAssert< (sizeof(TempStorageClass) == sizeof(Point3)+sizeof(float)+sizeof(BYTE)+sizeof(BOOL)+3) > validator;
};
...
//			MessageBox(NULL,ts,_T("Error"),MB_OK);
