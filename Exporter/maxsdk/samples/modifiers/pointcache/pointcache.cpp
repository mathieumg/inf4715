...
{
	_stprintf(m_signature,_T("POINTCACHE2"));
...
bool PointCache::InitFbxSdk()
{
	...
	if( m_pFbxCache == NULL )
		m_pFbxCache = KFbxCache::Create(m_pFbxSdkManager, "Point Cache");
...
void PointCache::ModifyObject(TimeValue t, ModContext& mc, ObjectState* os, INode* node)
{
	...
	int loadType = (GetCOREInterface()->IsNetworkRenderServer()) ?	m_pblock->GetInt(pc_load_type_slave, 0) :
...
#pragma message("UNICODE WATCH - use of TSTR::FromACP appropriate for CacheDataFileName name conversion? - File: " __FILE__ )
...
bool PointCache::IsMultipleChannelCache( const TCHAR* fname )
{
	...
	pTempFbxCache = KFbxCache::Create(m_pFbxSdkManager, "Point Cache Temp");
	...
	TSTR strPath;
	TSTR fnameStr = fname;
	SplitFilename(fnameStr, &strPath, NULL, NULL);

	MaxString m_strPath, m_fnameStr;
	strPath.ToMaxString(m_strPath);
	fnameStr.ToMaxString(m_fnameStr);
	// convert to UTF8
	pTempFbxCache->SetCacheFileName( m_strPath.ToUTF8(), m_fnameStr.ToUTF8() );
...
bool PointCache::ReadHeader()
{
	if( m_fileType == eMC ) {
		...
		if (_tcscmp(m_signature,m_cacheSignature) != 0) return false;
	...
	if (IsValidFName(file.GetFileName()))
	{
		...
		if (file.GetFullFilePath(path)) 
		{
			...
			if( m_fileType == eMC ) {
				...
				const TSTR& sName = file.GetFileName();

				MaxString m_strPath, m_Name;
				strPath.ToMaxString(m_strPath);
				sName.ToMaxString(m_Name);

				m_pFbxCache->SetCacheFileName(m_strPath.ToUTF8(), m_Name.ToUTF8());
...
bool PointCache::OpenCacheForRecordMC(const TCHAR* fname)
{
	...
	MaxString m_fnameStr, m_strPath;
	fnameStr.ToMaxString(m_fnameStr);
	strPath.ToMaxString(m_strPath);
	// convert to UTF8
	m_pFbxCache->SetCacheFileName((const char*)m_strPath.ToUTF8(), (const char*)m_fnameStr.ToUTF8());
	m_pFbxCache->SetCacheFileFormat( KFbxCache::eMC );
	...
	bool bRes = m_pFbxCache->OpenFileForWrite( 
								...
								GetFrameRate(), (const char*)m_strChannel.ToUTF8(), KFbxCache::kFloatVectorArray );
