...
class FFDNMWSMod : public SimpleWSMMod {
   public:
      ...
      //RefTargetHandle Clone(RemapDir& remap);
      const TCHAR    *GetObjectName() { return GetString(IDS_RB_FFDNMWSMOD); }
...
template <class T>
const TCHAR *FFDNM<T>::GetObjectName()
   {
   static TSTR buf;
   buf.printf(_T("%s %dx%dx%d"),GetString(GetNameID()),
...
class FFDSelMod : public Modifier {    
   public:           
      ...
      RefTargetHandle Clone(RemapDir& remap);
      const TCHAR *GetObjectName() { return GetString(IDS_RB_FFDSELMOD); }
