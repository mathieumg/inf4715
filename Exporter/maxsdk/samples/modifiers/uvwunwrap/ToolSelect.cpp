...
void UnwrapMod::SelectFacesByGroup( MeshTopoData *md,BitArray &sel,int seedFaceIndex, Point3 norm, float angle, BOOL relative, MeshTopoData::GroupBy groupBy,
									Tab<Point3> &normList,
			...
			while (numberWorkableEdges > 0)
			{
	 			...
	 			for (int i = 0; i < edges->edges.Count(); i++)
				{
					//					if (!blockedEdges[i])
					{
						...
						if ( (a>=0) && (b>=0) )
						{
							int smgrpA = md->GetMesh()->faces[a].getSmGroup();
							int smgrpB = md->GetMesh()->faces[b].getSmGroup();
							...
							{
								DWORD matIDA = md->GetMesh()->faces[a].getMatID();
								DWORD matIDB = md->GetMesh()->faces[b].getMatID();
								if (matIDB == matIDA)
									isMatching = TRUE;
							}
				...
				for (int j = 0; j < 2; j++)
				{
					...
					while (!done)
					{
						...
						if ((numberEdgesAtThisVertex == 3) && openEdge)
						{
							...
							if (ld->GetGeomEdgeNumberOfConnectedFaces(currentEdge) == 1)
							{
								...
								{
									int eindex = edgesAtVertex[starVert]->edgeIndex[k];
									int faceCount = ld->GetGeomEdgeNumberOfConnectedFaces(eindex);
