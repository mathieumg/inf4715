        ...
        SuperLUStat_t *stat, int *info)
{
    ...
    *info = sLUMemInit(fact, work, lwork, m, n, Astore->nnz,
	...
	if ( relax_end[jcol] != EMPTY ) { /* start of a relaxed snode */
		...
		*info = sLUMemXpand(jcol, nextlu, LUSUP, &nzlumax, &Glu);
		if ( (*info) )
		    return;
	    }
		...
		*info = spivotL(icol, diag_pivot_thresh, &usepr, perm_r,iperm_r, iperm_c, &pivrow, &Glu, stat);
		if ( (*info) )
		...
		if ((*info = scopy_to_ucol(jj, nseg, segrep, &repfnz[k],
			...
			*info = spivotL(jj, diag_pivot_thresh, &usepr, perm_r, iperm_r, iperm_c, &pivrow, &Glu, stat);
	    	if ( (*info) )
				if ( iinfo == 0 ) iinfo = *info;
