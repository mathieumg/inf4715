        ...
        SuperLUStat_t *stat, int *info)
{
    ...
    *info = cLUMemInit(fact, work, lwork, m, n, Astore->nnz,
	...
	if ( relax_end[jcol] != EMPTY ) { /* start of a relaxed snode */
		...
		*info = cLUMemXpand(jcol, nextlu, LUSUP, &nzlumax, &Glu);
		if ( (*info) )
		    return;
	    }
		...
		*info = cpivotL(icol, diag_pivot_thresh, &usepr, perm_r,iperm_r, iperm_c, &pivrow, &Glu, stat);
		if ( (*info) )
		...
		if ((*info = ccopy_to_ucol(jj, nseg, segrep, &repfnz[k],
			...
			*info = cpivotL(jj, diag_pivot_thresh, &usepr, perm_r,iperm_r, iperm_c, &pivrow, &Glu, stat);
	    	if ( (*info) )
		    if ( iinfo == 0 ) iinfo = *info;
