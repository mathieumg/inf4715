	   ...
	   )
{
	...
	A = (int *) SUPERLU_MALLOC(Alen * sizeof(int));
    if (!(A) )
        ABORT("Malloc fails for A[]");
	p = (int *) SUPERLU_MALLOC((n+1) * sizeof(int));
    if (!(p) )
       ...
       )
{
	...
	marker = (int*) SUPERLU_MALLOC((SUPERLU_MAX(m,n)+1)*sizeof(int));
    if ( !(marker) )
	ABORT("SUPERLU_MALLOC fails for marker[]");
	t_colptr = (int*) SUPERLU_MALLOC((m+1) * sizeof(int));
    if ( !(t_colptr) )
	ABORT("SUPERLU_MALLOC t_colptr[]");
	t_rowind = (int*) SUPERLU_MALLOC(nz * sizeof(int));
    if ( !(t_rowind) )
	...
	*ata_colptr = (int*) SUPERLU_MALLOC( (n+1) * sizeof(int));
    if ( !( *ata_colptr ) )
	ABORT("SUPERLU_MALLOC fails for ata_colptr[]");
    if ( *atanz ) 
	{
		*ata_rowind = (int*) SUPERLU_MALLOC( *atanz * sizeof(int));
		if ( !( *ata_rowind ) )
	  ...
	  )
{
    ...
    if ( !( marker ) )
	ABORT("SUPERLU_MALLOC fails for marker[]");
	t_colptr = (int*) SUPERLU_MALLOC( (n+1) * sizeof(int));
    if ( !( t_colptr ) )
    ...
    *bnz = num_nz;
    ...
    /* Allocate storage for A+A' */
	*b_colptr = (int*) SUPERLU_MALLOC( (n+1) * sizeof(int)) ;
    if ( !(*b_colptr) )
	ABORT("SUPERLU_MALLOC fails for b_colptr[]");
    if ( *bnz) 
	{
		*b_rowind = (int*) SUPERLU_MALLOC( *bnz * sizeof(int));
		if ( !( *b_rowind ) )
