...
#ifdef CHK_DFS
		printf(_T("krep %d, myfnz %d, perm_r[%d] %d\n"), krep, myfnz, krow, kperm);
#endif
...
#ifdef CHK_DFS 
		    printf(_T("  xdfs %d, maxdfs %d: "), xdfs, maxdfs);
		    for (i = xdfs; i < maxdfs; i++) printf(_T(" %d"), lsub[i]);
		    printf(_T("\n"));
#endif
		    do {
			/* 
...
#ifdef CHK_DFS
				    printf(_T("chrep %d,myfnz %d,perm_r[%d] %d\n"),chrep,myfnz,kchild,chperm);
#endif
...
#ifdef CHK_DFS 
			printf(_T("  pop stack: krep %d,xdfs %d,maxdfs %d: "), krep,xdfs,maxdfs);
			for (i = xdfs; i < maxdfs; i++) printf(_T(" %d"), lsub[i]);
			printf(_T("\n"));
#endif
