...
extern int ztrsv_(char*, char*, char*, int*, doublecomplex*, int*, doublecomplex*, int*);
         ...
         SuperMatrix *U, doublecomplex *x, SuperLUStat_t *stat, int *info)
{
    ...
    *info = 0;
	...
	work = doublecomplexCalloc(L->nrow);
    if ( !(work) )
	...
	if ( lsame_(uplo, "L") ) {
		...
		if ( nsupc > 1 ) {
			...
			&x[fsupc], &incx);
#else
		    ztrsv_("L", "T", "U", &nsupc, &Lval[luptr], &nsupr, &x[fsupc], &incx);
#endif
		}
	    }
	} else {
