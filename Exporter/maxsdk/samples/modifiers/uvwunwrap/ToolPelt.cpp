...
unsigned __stdcall  CallPeltThread(void *data)
{
	...
	if (threadData->mod->peltData.peltDialog.iRunPeltButton)
	{
		...
		{
			threadData->mod->peltData.peltDialog.iRunPeltButton->SetText(GetString(IDS_SPLINEMAP_STARTPELT));
...
void UnwrapMod::PeltThreadOp(int operation, HWND hWnd)
{
	...
	{		
		if (peltData.peltDialog.iRunPeltButton)
		{
			...
			peltData.peltDialog.iRunPeltButton->SetText(GetString(IDS_SPLINEMAP_ENDPELT));
		}
	...
	else if (operation == KThreadEnd)
	{
		...
		{
			peltData.peltDialog.iRunPeltButton->SetCheck(FALSE);
			peltData.peltDialog.iRunPeltButton->SetText(GetString(IDS_SPLINEMAP_STARTPELT));
		}
...
unsigned __stdcall  CallPeltRelaxThread(void *data)
{
	...
	if (threadData->mod->peltData.peltDialog.iRunPeltRelaxButton)
	{
		...
		{
			threadData->mod->peltData.peltDialog.iRunPeltRelaxButton->SetText(GetString(IDS_SPLINEMAP_STARTRELAX));
...
void UnwrapMod::PeltRelaxThreadOp(int operation, HWND hWnd)
{
	...
	{		
		...
		if (peltData.peltDialog.iRunPeltRelaxButton)
		{
			...
			peltData.peltDialog.iRunPeltRelaxButton->SetText(GetString(IDS_SPLINEMAP_ENDRELAX));
		}
	...
	else if (operation == KThreadEnd && mRelaxThreadHandle && mRelaxThreadData.mStarted)
	{
		...
		{
			peltData.peltDialog.iRunPeltRelaxButton->SetCheck(FALSE);
			peltData.peltDialog.iRunPeltRelaxButton->SetText(GetString(IDS_SPLINEMAP_STARTRELAX));
		}
...
INT_PTR CALLBACK UnwrapMapRollupWndProc(
	...
	switch (msg) {
		case WM_INITDIALOG:
			...
			mod->peltData.EnablePeltButtons(mod,mod->hMapParams, FALSE);
	...
	void PeltData::PeltMode(UnwrapMod *mod, BOOL start)
	{
		...
		if (!inPeltMode)
		{
			...
			SetPointToPointSeamsMode(mod,FALSE,TRUE);
	...
	void PeltData::StartPeltDialog(UnwrapMod *mod)
	{
		...
		mod->GetUIManager()->Enable(ID_PELT_EDITSEAMS,FALSE);
		mod->GetUIManager()->Enable(ID_PELT_POINTTOPOINTSEAMS,FALSE);
		mod->GetUIManager()->Enable(ID_PW_SELTOSEAM2,FALSE);
		mod->GetUIManager()->Enable(ID_PELT_EXPANDSELTOSEAM,FALSE);
	...
	void PeltData::EndPeltDialog(UnwrapMod *mod)
	{
		...
		mod->GetUIManager()->Enable(ID_PELT_EDITSEAMS,TRUE);
		mod->GetUIManager()->Enable(ID_PELT_POINTTOPOINTSEAMS,TRUE);
		mod->GetUIManager()->Enable(ID_PW_SELTOSEAM2,TRUE);
		mod->GetUIManager()->Enable(ID_PELT_EXPANDSELTOSEAM,FALSE);
	...
	void PeltData::EnablePeltButtons(UnwrapMod *mod, HWND hWnd, BOOL enable)
	{
		mod->GetUIManager()->Enable(ID_PELT_EDITSEAMS,enable);
		mod->GetUIManager()->Enable(ID_PELT_POINTTOPOINTSEAMS,enable);
		mod->GetUIManager()->Enable(ID_PW_SELTOSEAM2,enable);
		mod->GetUIManager()->Enable(ID_PELT_EXPANDSELTOSEAM,enable);
	...
	{
		//get the edit button
		HWND hButton = GetDlgItem(hParams, IDC_UNWRAP_RESET);
	...
	BOOL PeltData::PointToPointSelSeams()
	{
		return mPointToPointSeams;
	}
	void PeltData::SetPointToPointSeamsMode(UnwrapMod *mod,BOOL mode, BOOL selSeams)
	{
		if (mode)
			mPointToPointSeams = selSeams;
...
void PeltDialog::SetUpDialog(HWND hWnd)
{
	...
	irollup->AppendRollup(hInstance,MAKEINTRESOURCE(IDD_UNWRAP_PELTROLLUP1_DIALOG),QuickPeltRollupDialogProc, GetString(IDS_PW_QUICK_PELT),(LPARAM)mod );

	irollup->AppendRollup(hInstance,MAKEINTRESOURCE(IDD_UNWRAP_PELTROLLUP2_DIALOG),PeltOptionsRollupDialogProc, GetString(IDS_PW_PELT_OPTIONS),(LPARAM)mod );
	irollup->Show(0);
	irollup->Show(1);
...
TSTR UnwrapPeltSeamRestore::Description() { return TSTR(GetString(IDS_PW_PIVOTRESTORE)); }
...
TSTR UnwrapPeltVertVelocityRestore::Description() { return TSTR(GetString(IDS_PW_PIVOTRESTORE)); }
