...
MeshTopoData::MeshTopoData(ToolLSCM *lscm) 
{ 
	mLSCM = lscm;
	mLoaded = FALSE;
	Init();
...
MeshTopoData::MeshTopoData(ObjectState *os, int mapChannel, ToolLSCM *lscm)
{
	mLSCM = lscm;
	mLoaded = FALSE;
	Init();
...
LocalModData *MeshTopoData::Clone() {
	MeshTopoData *d = new MeshTopoData(mLSCM);
	d->mesh = NULL;
	d->mnMesh = NULL;
	d->patch = NULL;
...
void MeshTopoData::SetTVVert(TimeValue t,int index, Point3 p, UnwrapMod *mod)
{
	...
	{
		if (mod->fnGetMapMode()==LSCMMAP)//IsTVVertPinned(index))
		{
			mLSCM->InvalidatePin(this,index);
...
ULONG MeshTopoData::LoadTVVerts(ILoad *iload)
{
	...
	return iload->ReadVoid(TVMaps.v.Addr(0), sizeof(UVW_TVVertClass)*TVMaps.v.Count(), &nb);
}
...
//and then selected face 
	...
	{
		//we have topochange
		if (mFilterFaceSelection.GetSize() != mFSel.GetSize())
		{
			//if we don't have matching face sel sizes all we can do is clear and go on
			mFilterFacesVerts.SetSize(GetNumberTVVerts());
			mFilterFacesVerts.SetAll();
		}
		else
		{
			BitArray holdFaceSel(mFSel);
			mFSel = mFilterFaceSelection;
			//if we don't have a face selection associate with this we just clear
			if (mFilterFaceSelection.GetSize() == 0)
				BuilFilterSelectedFaces(FALSE);
			else //rebuild the face selection data
				BuilFilterSelectedFaces(TRUE);
...
void MeshTopoData::BuilFilterSelectedFaces(BOOL filterSelFaces)
{
	...
	{
		mFilterFacesVerts.SetAll();
		//we are not filtering selected faces so clear this and reduce
		mFilterFaceSelection.SetSize(0);
	}
	else
	{
		//hold our face selection associated with this filter selected faces
		mFilterFaceSelection = mFSel;
		
		// Updates based on mFilterFaceSelection
		UpdateFilteredVertices();
	}
}
...
void MeshTopoData::SelectElement(int mode,BOOL addSelection)
{
	...
	while (count != mVSel.NumberSet())
	{
		...
		for (int i = 0; i < GetNumberFaces()/*TVMaps.f.Count()*/;i++)
		{
			// Make sure the face should not be filtered out
			if (!(GetFaceDead(i)) && (mFilterFaceSelection.IsEmpty() || mFilterFaceSelection[i] == TRUE))//TVMaps.f[i]->flags & FLAG_DEAD))
			{
				int pcount = 3;
				pcount = GetFaceDegree(i);//TVMaps.f[i]->count;
				int totalSelected = 0;
...
void MeshTopoData::Stitch(BOOL bAlign, float fBias, BOOL bScale, UnwrapMod *mod)
{
	...
	selectedOppositeVerts.SetSize(TVMaps.v.Count());
...
/*
				if (gDebugLevel >= 1)
				{
					ScriptPrint(_T("Error stitch soft sel vert %d\n"),i); 
				}
*/
			}
			else
...
/*
				if (gDebugLevel >= 3)
				{
					ScriptPrint(_T("Moving stitch soft sel vert %d closest edge %d dist %f\n"),i,closestIndex, closestDist); 
				}           
*/
...
void MeshTopoData::WeldSelectedVerts( float sweldThreshold, UnwrapMod *mod)
{
	...
	mod->pblock->GetValue(unwrap_weldonlyshared,0,weldOnlyShared,FOREVER);
...
void MeshTopoData::WeldFaces(BitArray &selectedFaces, BOOL useEdgeLimit, float edgeLimit, UnwrapMod *mod)
{
	TimeValue t = GetCOREInterface()->GetTime();
...
//				DebugPrint(_T("Building geom vert data %d\n"),i);
				...
				for (int m = 0; m < connectedCount; m++)
				{
					...
					int degree = GetFaceDegree(faceIndex);
					int tvIndex =  mConnectedVerts[i]->mList[m].mUVWVert;
...
//								DebugPrint("		check mList %d face %d \n",j,faceIndex);
								int degree = GetFaceDegree(faceIndex);
								int tvIndex =  mConnectedVerts[i]->mList[j].mUVWVert;
								bool found = false;
								for (int k = 0; k < degree; k++)
								{
									int tvVertIndex = GetFaceTVVert(faceIndex,k);
...
void MeshTopoData::RescaleClusters(const Tab<int> &clusters, const Tab<float> &clustersRescales, UnwrapMod *mod)
{
	int largestID = -1;
	...
	for (int i = 0; i < clusterInfo.Count(); i++)
	{
		...
		{			
			float geoWidth = sqrt(clusterInfo[i].mGeomArea);
			float tvWidth = sqrt(clusterInfo[i].mTVArea);
			float scale = geoWidth/tvWidth;
			clusterInfo[i].mScaleAmount = scale;
			clusterInfo[i].mRatio = sqrt(clusterInfo[i].mTVArea/clusterInfo[i].mGeomArea);
			if (clusterInfo[i].mRatio > largestRatio)
				largestRatio = clusterInfo[i].mRatio;
			if (clusterInfo[i].mRatio < smallestRatio)
				smallestRatio = clusterInfo[i].mRatio;
...
{
	mHoldGeomSel.mFace = GetFaceSelection();
	mHoldGeomSel.mEdge = GetGeomEdgeSelection();
...
void MeshTopoData::RestoreSelection()
{
	...
	if (mHoldGeomSel.mVert.GetSize() == 0)
		return;
	SetFaceSelection(mHoldGeomSel.mFace);
	SetGeomEdgeSelection(mHoldGeomSel.mEdge);

	//sync the edges
	BitArray uvEdgeSel = GetTVEdgeSelection();
	ConvertGeomEdgeSelectionToTV(GetGeomEdgeSelection(), uvEdgeSel );
	SetTVEdgeSelection(uvEdgeSel);
...
void MeshTopoData::UpdateFilteredVertices()
{
	...
	if(mFilterFaceSelection.GetSize() == TVMaps.f.Count())
	{
		// Reset the vertices
		mFilterFacesVerts.SetSize(TVMaps.v.Count());
