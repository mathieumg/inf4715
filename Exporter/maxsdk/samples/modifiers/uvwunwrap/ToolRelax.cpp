...
void UnwrapMod::fnRelax(int iteration, float str, BOOL lockEdges, BOOL matchArea)
{
	...
	for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
	{
		mMeshTopoData[ldID]->Relax(mTVSubObjectMode, iteration, str, lockEdges, matchArea,this);
	}
...
void UnwrapMod::fnFit(int iteration, float str)
{
	...
	for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
	{
		mMeshTopoData[ldID]->RelaxFit(mTVSubObjectMode,iteration,str,this);
	}
...

{
	...
	for (int ldID = 0; ldID < mMeshTopoData.Count(); ldID++)
	{
		mMeshTopoData[ldID]->RelaxVerts2(mTVSubObjectMode, relax, iter, boundary, saddle, this,applyToAll,updateUI);
