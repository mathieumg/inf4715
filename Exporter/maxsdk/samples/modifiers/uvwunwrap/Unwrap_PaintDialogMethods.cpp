...
void UnwrapMod::PaintX(HDC hdc, int x, int y, int size)
{
	...
	for (int whichLD = 0; whichLD < mMeshTopoData.Count(); whichLD++)
	{
		...
		for (int i=0; i< md->GetNumberTVEdges(); i++) 
		{
			...
			if ( (!displayHiddenEdges) && ( numberFacesAtThisEdge > 1))
			{
				...
				{
					hiddenEdge = TRUE;
					if (mTVSubObjectMode == TVEDGEMODE)
					{
			...
			if (paintEdge && md->IsTVVertVisible(a) && md->IsTVVertVisible(b))
			{
				...
				//draw shared edges
				if ((mTVSubObjectMode == TVEDGEMODE) && (hiddenEdge)&& (md->GetTVEdgeSelected(i)))
				{
				...
				else if ((mTVSubObjectMode == TVEDGEMODE) && (md->GetTVEdgeSelected(i)))
				{
					SelectObject(hdc,selPen);
					DrawEdge(hdc,  veca,vecb, md->GetTransformedPoint(a), md->GetTransformedPoint(b), md->GetTransformedPoint(veca), md->GetTransformedPoint(vecb));
				}
		...
		for (int whichLD = 0; whichLD < mMeshTopoData.Count(); whichLD++)
		{
			...
			if (ld)
			{
				for (int i=0; i< ld->GetNumberFaces(); i++) 
				{
					...
					if (ld->IsFaceVisible(i) && (mTVSubObjectMode == TVFACEMODE) && (ld->GetFaceSelected(i)))
					{
						int pcount = 3;
		 ...
		 (mode == ID_SCALE) )
	{
		...
		{	
			...
			for (int i = 0; i < 4; i++)
			{
				...
				if (!inRotation && inFreeFormMode) MiniRect(hdc,prect[i],3);
			}
			Point2 centerEdge;
			...
			if (!inRotation && inFreeFormMode) MiniRect(hdc,centerEdge,3);
			...
			if (inRotation)
			{
				...
				TSTR rotAngleStr;
				rotAngleStr.printf(_T("%3.2f"),currentRotationAngle);
...
//	static BitArray clustersUsed;
	...
	if (!viewValid) 
	{
		...
		if (mTVSubObjectMode == TVVERTMODE)
		{
			PaintVertexTicks(hdc);	
		}

		//now paint the selected faces
		if (mTVSubObjectMode == TVFACEMODE)
		{
			PaintFaces(hdc);
		}
		...
		if (mTVSubObjectMode != TVOBJECTMODE)		
