...
public:
	...
	MeshTopoData(ObjectState *os, int mapChannel, ToolLSCM *lscm);
	MeshTopoData(ToolLSCM *lscm);
	~MeshTopoData();
	LocalModData *Clone();
...
//several relax algorithms
	...
	BOOL	BuildCluster( Tab<Point3> normalList, float threshold, BOOL connected, BOOL cleanUpStrayFaces,  MeshTopoData::GroupBy groupBy, Tab<ClusterClass*> &clusterList, UnwrapMod *mod);
...
//needs to be called to clear out our memory after you are done intersecting
	...
	void WeldFaces(BitArray &selectedFaces, BOOL useEdgeLimit, float edgeLimit, UnwrapMod *mod);
	...
	void RescaleClusters(const Tab<int> &clusters, const Tab<float> &clustRescale, UnwrapMod *mod);
...
private:
	...
	MeshTopoData();  //just makes sure no one can use the default
...
//does a quick intersect test on a specific face
	...
	//this is the filter selected face bitarray used to keep track of visible verts attached to selected faces
	BitArray mFilterFacesVerts;
	//this is the face selection associated with the show selected face filter
	BitArray mFilterFaceSelection;
	...
	// Updates the filtered vertices data from the filtered faces
	void UpdateFilteredVertices();
};
