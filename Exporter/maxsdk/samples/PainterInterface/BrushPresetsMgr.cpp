...
void EditControl::BeginEdit(RECT & rect)
{
   ...
   if (hWnd == NULL)
   {
      hWnd = CreateWindow(_T("EDIT"), _T(""), WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL | ES_LEFT,
...
LRESULT CALLBACK EditControl::EditControlProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
   ...
   case WM_KEYUP:
      ...
      GetWindowText( hWnd, parent->editText.dataForWrite(), 255 );
...
void ListViewCell::SetText( const TCHAR* text ) {
	if( this->text!=NULL ) {
		delete [] this->text;
	}
...
void ListViewColumn::SetName( const TCHAR* name ) {
...
void ListView::UpdateCell( int row, int col ) {
   ...
   if( cell->IsTextType() )   {
      item.mask |= LVIF_TEXT;
      item.pszText = const_cast<TCHAR*>(cell->GetText());
...
void ListView::Update() {
   ...
   for( int i=0; i<numColumns; i++ ) {
      ...
      column.pszText = const_cast<TCHAR*>(GetColumn(i)->GetName());
      column.cx = 1;
      column.iSubItem = i;
