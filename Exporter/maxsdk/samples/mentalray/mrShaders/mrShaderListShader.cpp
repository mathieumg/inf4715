...
void mrShaderListShader::TranslateParameters(imrTranslation& translationInterface, imrShader* shader, TimeValue t, Interval& valid) {
	...
	// Matches parameter list for "max_ShaderList" in
	// <maxroot>\NVIDIA\shaders_standard\mentalray\include\3dsmaxshaders.mi
	const ParamID max_ShaderList_array_pid = 0;
	pBlock->ZeroCount(max_ShaderList_array_pid);
	int count = GetNumShaders();
	pBlock->Resize(max_ShaderList_array_pid, count);
	for(int i = 0; i < count; ++i) {
		Texmap* shader = GetShader(i);
		if((shader != NULL) && GetShaderOn(i))
			pBlock->Append(max_ShaderList_array_pid, 1, &shader);
	}
}
