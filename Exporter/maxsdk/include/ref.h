...
//@{
#define PART_GI_EXCLUDED					(1<<0) //!< Excluded
#define PART_GI_OCCLUDER					(1<<1) //!< Excluder
#define PART_GI_RECEIVER					(1<<2) //!< Receiver
#define PART_GI_DIFFUSE						(1<<3) //!< Diffuse
#define PART_GI_SPECULAR					(1<<4) //!< Specular
#define PART_GI_NBREFINESTEPS				(1<<5) //!< Refine Steps
#define PART_GI_MESHSIZE					(1<<6) //!< Mesh Size
#define PART_GI_MESHINGENABLED				(1<<7) //!< Meshing Enabled
#define PART_GI_USEGLOBALMESHING			(1<<8) //!< Use Global Meshing
#define PART_GI_EXCLUDEFROMREGATHERING		(1<<9) //!< Exclude From Regathering
#define PART_GI_STOREILLUMMESH				(1<<10)//!< Store Illumination Mesh
#define PART_GI_RAYMULT						(1<<11)//!< Ray Multiplier
#define PART_GI_USE_ADAPTIVE_SUBDIVISION	(1<<12)//!< Use Adaptive Subdivision
#define PART_GI_MIN_MESH_SIZE				(1<<13)//!< Minium Mesh size
#define PART_GI_INITIAL_MESH_SIZE			(1<<14)//!< Initial Mesh size
#define PART_GI_CONTRAST_THRESHOLD			(1<<15)//!< Contrast Threshold
//@}
...
//@{
#define PART_REND_PROP_RENDERABLE					(1<<0)	//!< Renderable
#define PART_REND_PROP_CAST_SHADOW					(1<<1)	//!< Cast Shadow
#define PART_REND_PROP_RCV_SHADOW					(1<<2)	//!< Recieve Shadow
#define PART_REND_PROP_RENDER_OCCLUDED				(1<<3)	//!< Render Occluded
#define PART_REND_PROP_VISIBILITY					(1<<4)	//!< Visibility
#define PART_REND_PROP_INHERIT_VIS					(1<<5)	//!< Inherit Visibility
#define PART_REND_PROP_PRIMARY_INVISIBILITY			(1<<6)	//!< Primary Visibility
#define PART_REND_PROP_SECONDARY_INVISIBILITY		(1<<7)	//!< Secondary Visibility
//@}
...
//@{
#define PART_DISP_PROP_IS_HIDDEN					(1<<0)	//!< Is hidden
#define PART_DISP_PROP_IS_FROZEN					(1<<1)	//!< Is frozen
#define PART_DISP_PROP_SHOW_FRZN_WITH_MTL 			(1<<2)	//!< Show frozen with material
#define PART_DISP_PROP_BOX_MODE 					(1<<3)	//!< Box mode 
#define PART_DISP_PROP_BACKCULL 					(1<<4)	//!< Backface culling
#define PART_DISP_PROP_ALL_EDGES 					(1<<5)	//!< All edges
#define PART_DISP_PROP_BONE_ONLY 					(1<<6)	//!< Bones only
#define PART_DISP_PROP_BONE_AS_LINE 				(1<<7)	//!< Bones as lines
#define PART_DISP_PROP_SHOW_PATH 					(1<<8)	//!< Show path
#define PART_DISP_PROP_VERT_TICKS 					(1<<9)	//!< Vertex ticks
#define PART_DISP_PROP_XRAY_MTL 					(1<<10)	//!< XRay material
#define PART_DISP_PROP_IGNORE_EXTENTS 				(1<<11)	//!< Ignore extents
#define PART_DISP_PROP_COLOR_VERTS 					(1<<12)	//!< Vertex colors
//@}
