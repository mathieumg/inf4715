...
function CreateCustomProject(strProjectName, strProjectPath)
{
	try
	{
		...
		var strProjFile = strProjectPath + '\\' + strProjectName + '.vcxproj'
...
function GetPluginLibs(ext)
{
	var libs = "";
	var pmap = "";
	if (ext == "bmi" || ext == "bmf" || ext == "dlv" || ext == "bms" || ext == "dlh" || ext == "")
	{
...
function GetTargetName(strName, strProjectName)
{
	try
	{
		...
		if (strName == 'root.vcxproj')
			strTarget = wizard.FindSymbol('PROJECT_NAME') + '.vcxproj';

		if (strName == 'root.vcxproj.filters')
			strTarget = wizard.FindSymbol('PROJECT_NAME') + '.vcxproj.filters';
