...
class [!output CLASS_NAME] : public [!output SUPER_CLASS_NAME] {
	...
	public:
		...
		BOOL KeyAtTimeByID(int id,TimeValue t) { return pblock->KeyFrameAtTimeByID(id,t); }
...
{
	cs[0]->SetKeyBrackets( pShader->KeyAtTimeByID(pb_diffuse,curTime) );
...
{
	pblock = NULL;
	Get[!output CLASS_NAME]Desc()->MakeAutoParamBlocks(this);	// Create and intialize paramblock2
