...
static void ColorChangeNotifyProc(void* param, NotifyInfo* pInfo)
{
	...
	LoadMAXFileIcon(_T("crwd_pick"),  hPickNodeImages, kBackground, FALSE);
}
...
public:
   ...
   const TCHAR *dialogTitle() {return GetString(IDS_SELECT);}
...
void FormationBhvr::SetName(const TCHAR *newname) 
{
    pblock->SetValue(name,0,newname);
}

const TCHAR *FormationBhvr::GetName()
{
    const TCHAR *n;
...
void FormationBhvr::InitDialog(HWND hDlg)
{
	...
	if (!hPickNodeImages ) 
	{
		...
		BOOL ret = LoadMAXFileIcon(_T("crwd_pick"), hPickNodeImages, kBackground, FALSE);
	...
	iMultPick->SetTooltip(TRUE,(LPTSTR)GetString(IDS_MULTSEL));
}
