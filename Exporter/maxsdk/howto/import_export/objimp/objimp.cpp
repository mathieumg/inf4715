...
int parse(TCHAR *s, TCHAR **v)
{
    ...
    while(1) {
		...
		while((*s == _T(' ')) || (*s == _T('\t')))
            s++;
		if(*s && *s != _T('\n') && *s != _T('\r')) {  /* found a token - set ptr and bump counter */
            v[c++] = s;
		}
        if(*s == _T('"')) {      /* in quoted string */
            v[c-1] += 1;
            s++;
	    	while(*s != _T('"') && *s && *s != _T('\n') && *s != _T('\r'))  /* skip over string */
	        	s++;
           	if(*s == _T('"'))
               	*s++ = _T('\0');
        }
		else                 /* in ordinary token: skip over it */
		    while(*s != _T(' ') && *s != _T('\t') && *s && *s != _T('\n') && *s != _T('\r'))
	    	    s++;
		if(*s == 0 || *s == _T('\n') || *s == _T('\r')) {   /* if at end of input string, leave */
	   		*s = 0;
    	    break;
		}
