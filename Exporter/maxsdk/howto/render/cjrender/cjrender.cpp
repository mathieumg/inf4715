...
int CJRenderer::Open(INode *scene,INode *vnode,ViewParams* viewPar,RendParams& rpar,HWND hwnd,DefaultLight* defaultLights,int numDefLights, RendProgressCallback* prog)
{
	...
	DebugPrint(_T("**** Scanning for nodes.\n"));
	for (idx = 0; idx < scene->NumberOfChildren(); idx++) {
		NodeEnum(scene->GetChildNode(idx));
	}

	DebugPrint(_T("\tFound %d nodes\n"), instTab.Count());
	DebugPrint(_T("\tFound %d lights\n"), lightTab.Count());
	...
	if (lightTab.Count() == 0 && defaultLights) {
		...
		DebugPrint(_T("\tUsing %d default lights\n"), lightTab.Count());
	}
...
{
	int idx;
	DebugPrint(_T("**** Renderer going down.\n"));
...
int CJRenderer::Render(TimeValue t, Bitmap* tobm, FrameRendParams &frp, HWND hwnd, RendProgressCallback* prog, ViewParams* viewPar)
{
	...
	DebugPrint(_T("**** Rendering frame. TimeValue: %d.\n"), t);
	...
	if (prog) {
		prog->SetTitle(_T("Preparing to render..."));
	}
	...
	if (mapFiles.Count() && firstFrame) {
		...
		if (rendParams.bNetRender) {
			...
			for ( i=0; i<mapFiles.Count(); i++ )
				ci->Log()->LogEntry(SYSLOG_ERROR,NO_DIALOG,NULL,_T("Missing Map: %d"),mapFiles[i]);
			return 0;
		}
		else {
			if (MessageBox(hWnd, _T("There are missing maps.\nDo you want to render anyway?"), _T("Warning!"), MB_YESNO) != IDYES) {
				return 0;
			}
		}
