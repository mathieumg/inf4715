...
bool MaterialHandle::UpdateDisplay(
	const MaxSDK::Graphics::MaxContext& maxContext, 
	const MaxSDK::Graphics::UpdateDisplayContext& displayContext)
{
	using namespace MaxSDK::Graphics;
	// prepare SolidColorMaterialHandle with white color
	MaxSDK::Graphics::SolidColorMaterialHandle solidColorMaterialHandle;
	solidColorMaterialHandle.Initialize();
	solidColorMaterialHandle.SetColor(AColor(1.0,0.2,0.2,1.0));

	// set the MaterialRequiredStreams to UpdateDisplayContext to generate proper render item
	const MaterialRequiredStreams* pMaterialRequiredStreams = solidColorMaterialHandle.GetRequiredStreams();
	MaxSDK::Graphics::UpdateDisplayContext* pDisplayContext = const_cast<MaxSDK::Graphics::UpdateDisplayContext*>(&displayContext);
	MaterialRequiredStreams& requiredStream = pDisplayContext->GetRequiredStreams();
	if (pMaterialRequiredStreams != NULL)
	{
		requiredStream.Swap(*(const_cast<MaterialRequiredStreams*>(pMaterialRequiredStreams)));
	}
	//SimpleObject automatically generate render items for mesh
	bool result = __super::UpdateDisplay(maxContext, displayContext);
	
	// set SolidColorMaterialHandle to render item
	MaxSDK::Graphics::RenderItemHandleArray renderItemHandleArray = GetRenderItems();
	for (int index = 0; index < renderItemHandleArray.GetNumberOfRenderItems(); index++)
	{
		RenderItemHandle renderItemHandle = renderItemHandleArray.GetRenderItem(index);
		renderItemHandle.SetCustomMaterial(solidColorMaterialHandle);
	}
