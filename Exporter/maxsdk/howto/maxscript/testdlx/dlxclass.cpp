...
DEFINE_LOCAL_GENERIC_CLASS(ListValue, ListGeneric);
...
{
	throw RuntimeError (_T("Can't directly set list size"), this);
	return arg_list[0];
}
...
SetValue::intersection_vf(Value** arg_list, int count)
{
	...
	type_check(set2, SetValue, _T("intersection()"));
...
SetValue::union_vf(Value** arg_list, int count)
{
	...
	Value* set2 = arg_list[0];
	type_check(set2, SetValue, _T("union()"));
	one_typed_value_local(SetValue* result);
	ListValue* t;
	vl.result = new SetValue();
	for (t = this; t->head; t = t->tail)
