...
const TCHAR* SCPClassDesc::ClassName()
{
	static TSTR className (MaxSDK::GetResourceStringAsMSTR(IDS_CLASSNAME)); 
	return className;
}
const TCHAR* SCPClassDesc::Category()
{
				...
				case IDC_BTN1:
					...
					if(!cancel&&mtlb)
					{
						...
						else
						{
							...
							MessageBox(hWnd,_T("Chosen Material Is NOT a GAMEMTL!"),_T("Error"),MB_OK);
						}
					}
...
void SCPUtility::BeginEditParams(Interface *ip,IUtil *iu) 
{
	...
	hPanel = ip->AddRollupPage(
		...
		DefaultDlgProc,
		MaxSDK::GetResourceStringAsMSTR(IDS_PARAMETERS),
		0);
}
