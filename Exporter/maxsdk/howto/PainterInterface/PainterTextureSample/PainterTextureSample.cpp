...
Bitmap *PainterTextureSample::BuildBitmap(int size) 
	...
	if (bitMapAssetUser.GetId() == MaxSDK::AssetManagement::kInvalidId)
		bitMapAssetUser = MaxSDK::AssetManagement::IAssetManager::GetInstance()->GetAsset(_T("painterTextureSampleTemp"), MaxSDK::AssetManagement::kBitmapAsset);
...
//loop through each face
	...
	for (int i = 0; i < msh->numFaces; i++)
		...
		TSTR name;
		name.printf(_T("Processing Face %d/%d"),i,msh->numFaces-1);
