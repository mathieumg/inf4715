...
#define ERROR_TITLE _T("XRef Util Error")
...
static INT_PTR CALLBACK PickObjDlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG: 
        ...
        for (i = 0; i < theXrefutil.m_objnamesholder.Count(); i++) {
            ...
            ix = SendDlgItemMessage(hWnd,IDC_LIST_NODES,LB_ADDSTRING,0,(LPARAM)(const TCHAR*)oname);
            SendDlgItemMessage(hWnd,IDC_LIST_NODES,LB_SETITEMDATA,ix,i);
        }
        CenterWindow(hWnd,GetParent(hWnd));
        theXrefutil.m_picknameholder = _T("");
        break;
...
static INT_PTR CALLBACK ExportDlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG: 
        ...
        ::SetDlgItemText(hWnd, IDC_EDIT_EXPRES, (const TCHAR*)(*contents));
        CenterWindow(hWnd,GetParent(hWnd));
        break;
...
void Xrefutil::GetAllXRefsObjects_IXROMGR(HWND hWnd)
{
	...
	// Woohoo!  we get to use the IXRefObjectManager interface
	AssetUser asset = IAssetManager::GetInstance()->GetAsset(filename,kXRefAsset);
	int num = m_pIobjrefmgr->GetNumXRefObjects(asset);
	MessageBox(NULL,_itot(num, buffer, 10),_itot(num, buffer, 10),MB_OK);
