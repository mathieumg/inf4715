...
BOOL CNotifyDlg::Browse(int type, TCHAR *filename) {
	...
	ofn.lpstrFilter	= _T("Sound Files\0*.wav\0\0");
	...
	ofn.lpstrTitle	= (TCHAR *)(void *)LPCTSTR(title);

	if (!initDir[0]) {
		GetWindowsDirectory(initDir,MAX_PATH);
		_tcscat(initDir,_T("\\Media"));
	}
...
BOOL CNotifyDlg::OnHelpInfo(HELPINFO* pHelpInfo) {
	MaxSDK::IHelpSystem::GetInstance()->ShowProductHelpForTopic(idh_notify_main_ui);	
