...
void CCUtil::ShowCurveControl()
{
	...
	SetZoom();
	SetScroll();
	mpCCtl->SetTitle(_T("Test Control Curve"));
...
void CCUtil::SetColor(BOOL Active)
{
	...
	GetDlgItemText(hPanel, IDC_SC_R, buf, 10); r = (int)_ttoi(buf);
	GetDlgItemText(hPanel, IDC_SC_G, buf, 10); g = (int)_ttoi(buf);
	GetDlgItemText(hPanel, IDC_SC_B, buf, 10); b = (int)_ttoi(buf);
	GetDlgItemText(hPanel, IDC_STYLE, buf, 10); style = (int)_ttoi(buf);
	GetDlgItemText(hPanel, IDC_WIDTH, buf, 10); width = (int)_ttoi(buf);
...
void CCUtil::SetDisplay()
{
	...
	GetDlgItemText(hPanel, IDC_DISPEDIT, buf, 10); lDisp = _ttol(buf);
...
void CCUtil::GetPtInfo()
{
	...
	if(pCurve)
	{
		_stprintf(buf,_T("%d"),pCurve->GetNumPts());
...
void CCUtil::GetPlacement()
{
	...
	if(mpCCtl)
	{
		...
		if(hCurve)
		{
			...
			_stprintf(buf,_T("%d"),rect.top);
			SetDlgItemText(hPanel, IDC_POS_X, buf); 
	
			_stprintf(buf,_T("%d"),rect.left);
			SetDlgItemText(hPanel, IDC_POS_Y, buf); 

			_stprintf(buf,_T("%d"),rect.bottom-rect.top);
			SetDlgItemText(hPanel, IDC_SIZE_Y, buf); 

			_stprintf(buf,_T("%d"),rect.right-rect.left);
