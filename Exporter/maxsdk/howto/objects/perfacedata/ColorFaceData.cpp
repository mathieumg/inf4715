...
public:
	...
	// From Animatable
	const TCHAR *GetObjectName() { return GetString(IDS_COLOR_CLASS_NAME); }
...
private:
	virtual void SetReference(int i, RefTargetHandle rtarg) { mpParams=(IParamBlock2*)rtarg; }
public:
...
INT_PTR DataToColorDlgProc::DlgProc (TimeValue t, IParamMap2 *map, HWND hWnd,
							  UINT msg, WPARAM wParam, LPARAM lParam) {
	switch (msg) {
	case WM_INITDIALOG:
		break;
...
RefTargetHandle FaceDataToColorMod::Clone(RemapDir& remap)
{
	...
	newmod->ReplaceReference(0,remap.CloneRef(mpParams));
