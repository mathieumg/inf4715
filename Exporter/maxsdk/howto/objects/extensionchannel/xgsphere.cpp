...
BuildNURBSSphere(float radius, BOOL hemi, BOOL recenter, BOOL genUVs)
{
	...
	TCHAR bname[80];
	TCHAR sname[80];
	_tcscpy(bname, GetString(IDS_XGEOSPHERE));
...
void XGSphereObject::CopyParamsToMod(XModifier *pmod)
{
	...
	if(pblock2_x->GetControllerByID(pb_fn_spin))
		pmod->pblock->SetControllerByID(pb_fn_spin,0,pblock2_x->GetControllerByID(pb_fn_spin),FALSE);
	else {
		pblock2_x->GetValue(pb_fn_spin,0,f,ivalid);
		pmod->pblock->SetValue(pb_fn_spin,0,f);
	}

	if(pblock2_x->GetControllerByID(pb_nf_spin))
		pmod->pblock->SetControllerByID(pb_nf_spin,0,pblock2_x->GetControllerByID(pb_nf_spin),FALSE);
	else {
		pblock2_x->GetValue(pb_nf_spin,0,i,ivalid);
		pmod->pblock->SetValue(pb_nf_spin,0,i);
	}

	if(pblock2_x->GetControllerByID(pb_fa_spin))
		pmod->pblock->SetControllerByID(pb_fa_spin,0,pblock2_x->GetControllerByID(pb_fa_spin),FALSE);
	else {
		pblock2_x->GetValue(pb_fa_spin,0,f,ivalid);
		pmod->pblock->SetValue(pb_fa_spin,0,f);
	}

	if(pblock2_x->GetControllerByID(pb_suspdisp))
		pmod->pblock->SetControllerByID(pb_suspdisp,0,pblock2_x->GetControllerByID(pb_suspdisp),FALSE);
	else {
		pblock2_x->GetValue(pb_suspdisp,0,b,ivalid);
		pmod->pblock->SetValue(pb_suspdisp,0,b);
	}

	if(pblock2_x->GetControllerByID(pb_fn_onoff))
		pmod->pblock->SetControllerByID(pb_fn_onoff,0,pblock2_x->GetControllerByID(pb_fn_onoff),FALSE);
	else {
		pblock2_x->GetValue(pb_fn_onoff,0,b,ivalid);
		pmod->pblock->SetValue(pb_fn_onoff,0,b);
	}

	if(pblock2_x->GetControllerByID(pb_nf_onoff))
		pmod->pblock->SetControllerByID(pb_nf_onoff,0,pblock2_x->GetControllerByID(pb_nf_onoff),FALSE);
	else {
		pblock2_x->GetValue(pb_nf_onoff,0,b,ivalid);
		pmod->pblock->SetValue(pb_nf_onoff,0,b);
	}
	
	if(pblock2_x->GetControllerByID(pb_fa_onoff))
		pmod->pblock->SetControllerByID(pb_fa_onoff,0,pblock2_x->GetControllerByID(pb_fa_onoff),FALSE);
	else {
