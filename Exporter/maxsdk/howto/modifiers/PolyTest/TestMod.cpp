...
PolyTestErrorLog::PolyTestErrorLog ()
{
	mNumErrors = 0;
	mUpdated = false;
	for (int i=0; i<kLogSize; i++) mError[i] = _T("");
}
...
public:
	...
	const TCHAR *GetObjectName() {return GetString(IDS_POLY_TEST_MOD);}
...
void PolyTestMod::CheckAllData (MNMesh & mm, UVVert *pVertColor, PolyTestErrorLog *el) {
	...
	if (!filled)
	{
		el->AddError (_T("Mesh not \"filled-in\"\n"));
	}
	...
	for (mapChannel=-NUM_HIDDENMAPS; mapChannel<mm.numm; mapChannel++) {
		...
		{
			buffer.printf (_T("Map error in map %d\n"), mapChannel);
			el->AddError (buffer);
		}
	}
	...
	if (pNorms)
	{
		...
		if (!pNorms->CheckAllData(mm.numf))
			el->AddError (_T("Error in Normals\n"));
	}
	...
	for (i=0; i<mm.numf; i++) {
		...
		for (j=0; j<mm.f[i].deg; j++) {
			...
			int ej = mm.f[i].edg[j];
			if ((vj < 0) || (vj >= mm.numv)) {
				buffer.printf (_T("Face %d has an out-of-range vertex: %d\n"), i, vj);
				el->AddError(buffer);
				vj = -1;
			} else {
				if (mm.v[vj].GetFlag (MN_DEAD)) {
					buffer.printf (_T("Face %d refs dead vertex %d.\n"), i, vj);
					el->AddError(buffer);
				} else {
					if (filled) {
						...
						if (k>=max) {
							buffer.printf (_T("Face %d refs vertex %d, but vertex doesn't ref face.\n"), i, vj);
							el->AddError(buffer);
			...
			if (filled) {	// Don't check edges unless filled in.
				if ((ej<0) || (ej>=mm.nume)) {
					buffer.printf (_T("Face %d has an out-of-range edge: %d\n"), i, ej);
					el->AddError(buffer);
					if (vj>-1 && pVertColor) pVertColor[vj] = red;
				} else {
					if (mm.e[ej].GetFlag (MN_DEAD)) {
						buffer.printf (_T("Face %d uses dead edge %d.\n"), i, ej);
						el->AddError(buffer);
						if (vj>-1 && pVertColor) pVertColor[vj] = red;
					} else {
						if ((mm.e[ej].f1 != i) && (mm.e[ej].f2 != i)) {
							buffer.printf (_T("Face %d refs edge %d, but edge doesn't ref face.\n"), i, ej);
							el->AddError(buffer);
							if (vj>-1 && pVertColor) pVertColor[vj] = red;
						} else {
							if (mm.e[ej].OtherVert(vj) != mm.f[i].vtx[(j+1)%mm.f[i].deg]) {
								buffer.printf (_T("Face %d refs edge %d from vert %d to vert %d, but edge doesn't go there.\n"),
								...
								if (((mm.e[ej].v1 == vj) && (mm.e[ej].f1 != i)) ||
									...
									buffer.printf (_T("Orientation mismatch between face %d and edge %d.\n"), i, ej);
									el->AddError(buffer);
									if (vj>-1 && pVertColor) pVertColor[vj] = red;
								}
							}
		...
		for (j=0; j<dnum; j++) {
			...
			buffer.printf (_T("Face %d has bad diagonal index: %d.\n"), i, mm.f[i].diag[j]);
			el->AddError(buffer);
		}
		for (j=0; j<dnum; j+=2) {
			...
			if (((mm.f[i].diag[j]+1)%mm.f[i].deg == mm.f[i].diag[j+1]) ||
				...
				buffer.printf (_T("Face %d has bad diagonal index: %d, %d.\n"), i,
	...
	for (i=0; i<mm.nume; i++) {
		...
		if (mm.v[mm.e[i].v1].GetFlag (MN_DEAD)) {
			buffer.printf (_T("Edge %d refs dead vertex %d.\n"), i, mm.e[i].v1);
			el->AddError(buffer);
			...
			if (mm.vedg) {
				...
				if (j>=max) {
					buffer.printf (_T("Edge %d refs vertex %d, but vertex doesn't ref edge.\n"), i, mm.e[i].v1);
					el->AddError(buffer);
		...
		if (mm.e[i].f1 == -1) {
			buffer.printf (_T("Edge %d has no face-1"), i);
			el->AddError(buffer);
			if (pVertColor) pVertColor[mm.e[i].v1] = red;
			if (pVertColor) pVertColor[mm.e[i].v2] = red;
			continue;
		...
		if (mm.f[mm.e[i].f1].GetFlag (MN_DEAD)) {
			buffer.printf (_T("Edge %d refs dead face %d.\n"), i, mm.e[i].f1);
			el->AddError(buffer);
			if (pVertColor) pVertColor[mm.e[i].v1] = red;
			if (pVertColor) pVertColor[mm.e[i].v2] = red;
		} else {
			...
			if (j>=max) {
				buffer.printf (_T("Edge %d refs face %d, but face doesn't ref edge.\n"), i, mm.e[i].f1);
				el->AddError(buffer);
				if (pVertColor) pVertColor[mm.e[i].v1] = red;
				if (pVertColor) pVertColor[mm.e[i].v2] = red;
			}
	...
	for (i=0; i<mm.numv; i++) {
		...
		if (mm.vedg) {
			...
			for (j=0; j<max; j++) {
				int ej = mm.vedg[i][j];
				if ((ej>=mm.nume) || (ej<0)) {
					buffer.printf (_T("Vertex %d refs out-of-range edge %d\n"), i, ej);
					el->AddError(buffer);
					if (pVertColor) pVertColor[i] = red;
					continue;
				}
				if (mm.e[ej].GetFlag (MN_DEAD)) {
					buffer.printf (_T("Vertex %d refs dead edge %d.\n"), i, ej);
					el->AddError(buffer);
					if (pVertColor) pVertColor[i] = red;
					continue;
				}
				...
				if (mm.e[ej].v2 != i) {
					buffer.printf (_T("Vertex %d refs edge %d, but edge doesn't ref vertex.\n"), i, ej);
					el->AddError(buffer);
					if (pVertColor) pVertColor[i] = red;
				}
			}
		...
		if (mm.vfac) {
			...
			if (mm.vedg && (max>mm.vedg[i].Count())) {
				buffer.printf (_T("Vertex %d references more faces than edges.\n"), i);
				el->AddError(buffer);
				if (pVertColor) pVertColor[i] = red;
			}
			for (j=0; j<max; j++) {
				int fj = mm.vfac[i][j];
				if (fj>=mm.numf) {
					buffer.printf (_T("Vertex %d refs out-of-range face %d\n"), i, fj);
					el->AddError(buffer);
					if (pVertColor) pVertColor[i] = red;
					continue;
				}
				if (mm.f[fj].GetFlag (MN_DEAD)) {
					buffer.printf (_T("Vertex %d refs dead face %d.\n"), i, fj);
					el->AddError(buffer);
					if (pVertColor) pVertColor[i] = red;
					continue;
				}
				...
				if (k>=mm.f[fj].deg) {
					buffer.printf (_T("Vertex %d refs face %d, but face doesn't ref vertex.\n"), i, fj);
					el->AddError(buffer);
					if (pVertColor) pVertColor[i] = red;
				}
			}
...
void PolyTestMod::UpdateErrorDisplay () {	
	...
	for (int i = 0; i < mcList.Count(); i++) {
		...
		for (; j<20; j++)
			SetDlgItemText (hParams, IDC_TEXT_RESULT+j, _T(""));
	}
}
