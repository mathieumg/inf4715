...
BendManip::BendManip(SimpleMod2* m, INode* n)
	: Manipulator(NULL)
	, modifier(m)
	, node(n)  // Used for the call to invalidate the rectangle in the viewport.
{
	myName = _T("BendManip");
	mouseWithin = FALSE;
}
...
void BendManip::OnMouseMove(TimeValue t, ViewExp* /*pVpt*/, IPoint2& m, DWORD flags, ManipHitData* pHitData) 
{
	...
	if( !pblock2->GetValue( PB_ANGLE, t, angle, FOREVER ) )
		return;
	if( !pblock2->GetValue( PB_DIR, t, direction, FOREVER ) )
		return;
