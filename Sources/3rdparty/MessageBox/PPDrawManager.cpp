...
HICON CPPDrawManager::StretchIcon(HICON hIcon, DWORD dwWidth, DWORD dwHeight)
{
  ...
  if (::GetObject(csOriginal.hbmColor, sizeof(BITMAP), &bmp))
  {
    ...
    {
      HBITMAP hOldSrcBitmap = (HBITMAP)::SelectObject(hSrcDC, csOriginal.hbmColor);
      HBITMAP hOldDestBitmap = (HBITMAP)::SelectObject(hDestDC, csStretched.hbmColor);
...
//----------------------------------------------------
  case EFFECT_HARDBUMP:  // 
    ...
    //set vertical bump
    d=std::min(16,(int)sXSize/6);  //max edge=16
    a=sYSize*sYSize/4;
    posDst =iDst;
    for(i = 0; i < sYSize; i++) {
      y=i-sYSize/2;
