      ...
      for (int y = 0; y < szTable.cy; y++)
      {
        ...
        for (int x = 0; x < szTable.cx; x++)
        {
          ...
          if (sc.nColSpan == i)
          {
            if (i == 1)
            {
              cur_table.width [x] = std::max ((int)cur_table.width [x], (int)sc.szCell.cx);
              if (sc.bFixedWidth)
    ...
    for (i = 1; i <= szTable.cy; i++)
    {
      for (int y = 0; y < szTable.cy; y++)
      {
        ...
        for (int x = 0; x < szTable.cx; x++)
        {
          ...
          {
            if (i == 1)
              cur_table.height [y] = std::max ((int)cur_table.height [y], (int)sc.szCell.cy);
            else
            {
              int span_height = 0;
              for (int z = 0; z < i; z++)
...
//    sc.szText.cy = m_defStyle.nCellHeight;
  ...
  if (MODE_DRAW != m_nNumPass)
  {
    //ENG: On first and second passes we are calculate the dimensions of the cell
    //RUS: На первом и втором проходах вычисляем размеры ячейки.
    sc.szCell.cx = std::max((INT)m_defStyle.nCellWidth, (INT)sc.szText.cx);
    sc.szCell.cy = std::max((INT)m_defStyle.nCellHeight, (INT)sc.szText.cy);
...
//  int nBegin = 0;
  ...
  for (i = nFirstLine; i < m_nCurLine; i++)
  {
    ...
    if (0 != m_hline.nAddPercentWidth)
    {
      ...
      szTextArea.cx = std::max((int)szTextArea.cx, (int)m_hline.nWidthLine);
    } //if
  } //for
//
//  if (NULL != lpSize)
...
int CPPHtmlDrawer::VerticalAlignText(int y, int nHeight)
{
  ...
  if (MODE_FIRSTPASS == m_nNumPass)
  {
    ...
    m_hline.nDescentLine = std::max((int)m_hline.nDescentLine, (int)(nHeight - m_tm.tmAscent));
    m_hline.nHeightLine = std::max((int)m_hline.nHeightLine, (int)m_tm.tmAscent);
  }
  else if (MODE_DRAW == m_nNumPass)
  {
    switch (m_defStyle.nVertAlign)
...
//    rect.right = rect.left + m_nMaxWidth;
  ...
  //ENG: VCNFloat output the prepared string
...
void CPPHtmlDrawer::AddToTextStyle(LPCTSTR lpszStyleName, LPCTSTR lpszAddStyle) const
{
} //End AddToTextStyle
                  ...
                  BOOL bGradient /* = TRUE */, int nDepthX /* = 7 */, int nDepthY /* = 7 */)
{
  ...
  m_nDarkenShadow = std::min(100, (int)nDarkenPercent);
