...
struct AkArrayAllocatorDefault
{
	static AkForceInline void * Alloc( AkMemPoolId in_poolId, size_t in_uSize )
	{
		return AK::MemoryMgr::Malloc( in_poolId, in_uSize );
	}

	static AkForceInline void Free( AkMemPoolId in_poolId, void * in_pAddress )
	{
		AK::MemoryMgr::Free( in_poolId, in_pAddress );
	}
};
...
struct AkArrayAllocatorAlignedSimd
{
	static AkForceInline void * Alloc( AkMemPoolId in_poolId, size_t in_uSize )
	{
		return AK::MemoryMgr::Malign( in_poolId, in_uSize, AK_SIMD_ALIGNMENT );
	}

	static AkForceInline void Free( AkMemPoolId in_poolId, void * in_pAddress )
	{
		AK::MemoryMgr::Falign( in_poolId, in_pAddress );
	}
};

/// Specific implementation of array
template <class T, class ARG_T, class U_POOL, unsigned long TGrowBy = 1, class TAlloc = AkArrayAllocatorDefault> class AkArray
{
public:
	/// Constructor
	AkArray()
...
// Bypass this inner struct and use a proxy in a separate header.
	...
	AKRESULT Reserve( AkUInt32 in_ulReserve )
	{
		...
		if ( in_ulReserve )
		{
			m_pItems = (T *) TAlloc::Alloc( U_POOL::Get(), sizeof( T ) * in_ulReserve );
			if ( m_pItems == 0 )
	...
	void Term()
	{
		...
		{
			RemoveAll();
			TAlloc::Free( U_POOL::Get(), m_pItems );
			m_pItems = 0;
			m_ulReserved = 0;
		}
	}
...
#pragma warning( pop )
	...
	bool GrowArray( AkUInt32 in_uGrowBy = TGrowBy )
	{
		...
		T * pNewItems = (T *) TAlloc::Alloc( U_POOL::Get(), sizeof( T ) * ulNewReserve );
		if ( !pNewItems ) 
			return false;
	...
	void Transfer(AkArray<T,ARG_T,U_POOL,TGrowBy,TAlloc>& in_rSource)
	{
		if (m_pItems)
			Term();
