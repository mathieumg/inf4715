...
struct AkOutputSettings
{
	...
	AkChannelMask	uChannelMask;	///< Channel mask for this output.  One of AK_SPEAKER_SETUP_7POINT1, AK_SPEAKER_SETUP_5POINT1, AK_SPEAKER_SETUP_STEREO or AK_CHANNEL_MASK_DETECT.  Hardware might not support the selected configuration.
};
...
namespace AK
{
	...
	namespace SoundEngine
	{
		...
		/// \return The output configuration (AK_SPEAKER_SETUP_* group of defines). AK_CHANNEL_MASK_INVALID if device does not exist.
		/// \sa 
		/// - AkSpeakerConfig.h
		/// - AkOutputSettings
		AK_EXTERNAPIFUNC( AkChannelMask, GetSpeakerConfiguration )(
			AkSinkType 			in_eSinkType = AkSink_Main,	///< Output sink type. Pass AkSink_Main for main output.
		...
		/// AK::GetNumberOfAnglesForConfig( AK_SUPPORTED_SPEAKER_SETUP ).
		/// If io_pfSpeakerAngles is not NULL, the array is filled with up to io_uNumAngles.
		/// Typical usage:
		/// - AkUInt32 uNumAngles;
		/// - GetSpeakerAngles( 0, AkSink_Main, NULL, uNumAngles );
		/// - AkReal32 * pfSpeakerAngles = AkAlloca( uNumAngles * sizeof(AkReal32) );
		/// - GetSpeakerAngles( 0, AkSink_Main, pfSpeakerAngles, uNumAngles );
		/// \warning Call this function only after the sound engine has been properly initialized.
		...
		/// Set speaker angles of the specified device.
		/// The speaker angles are expressed as an array of loudspeaker pairs, in degrees, relative to azimuth ]0,180].
		/// Supported loudspeaker setups are always symmetric; the center speaker is always in the middle and thus not specified by angles.
		/// Angles must be set in ascending order. 
		/// Typical usage: 
		...
		/// \warning Call this function only after the sound engine has been properly initialized.
		/// \return AK_Success if successful (device exists and angles are valid).
		/// \sa GetSpeakerAngles()
		...
		///
		/// IMPORTANT: Banks loaded with by in-memory data MUST be unloaded using the UnloadBank function
		...
		AK_EXTERNAPIFUNC( AKRESULT, SetListenerSpatialization )(
			...
			AkSpeakerVolumes * in_pVolumeOffsets = NULL	///< Per-speaker volume offset, in dB.
			);
		...
		AK_EXTERNAPIFUNC( AKRESULT, AddSecondaryOutput )(
			AkUInt32 in_iOutputID,			///< Player number or device-unique identifier.			
			AkSinkType in_iDeviceType,		///< Device Type, must be one of the currently supported devices types.  See AkSinkType
			AkUInt32 in_uListenerMask		///< Listener(s) to attach to this device.  Everything heard by these listeners will be sent to this output.  This is a bitmask.
			);

		AK_EXTERNAPIFUNC( AKRESULT, RemoveSecondaryOutput )(
			AkUInt32 in_iOutputID,			///< Player number or device-unique identifier.
			AkSinkType in_iDeviceType		///< Device Type, must be one of the currently supported devices types. 
			);
