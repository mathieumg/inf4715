...
/// These are i/o parameters. Input parameters are the one computed by the system, you can override them or offset them.
/// \sa 
/// - AK::SoundEngine::PostEvent()
/// - \ref soundengine_events
struct AkSpeakerVolumeMatrixCallbackInfo : public AkEventCallbackInfo
{
	AkSpeakerVolumes * pVolumes[ AK_VOICE_MAX_NUM_CHANNELS ];		///< For each channel in the sound, pointer to AkSpeakerVolume describing contribution to destination channels. 
	AkUInt32 uInputConfig;											///< Channel configuration of the sound (for example, AK_SPEAKER_SETUP_STEREO)
	AkUInt32 uOutputConfig;											///< Channel configuration of the output bus (for example, AK_SPEAKER_SETUP_5POINT1)
	AkReal32 * io_pfDryAttenuation;									///< Single attenuation value to apply over the AkSpeakerVolumes to compute the final dry attenuation.
	AkReal32 * io_pfGameDefAuxSendAttenuation;						///< Single attenuation value to apply over the AkSpeakerVolumes to compute the final Game defined auxilliary send attenuation.
	AkReal32 * io_pfUserDefAuxSendDryAttenuation;					///< Single attenuation value to apply over the AkSpeakerVolumes to compute the final User defined auxilliary send attenuation.
};
...
/// Callback information structure corresponding to AK_SpeakerVolumeMatrix.
/// Volumes are read-write, linear values normalized to the [0..1] range. Source channels are following the 
/// order defined in AkCommonDefs.h. This callback will be called at every audio frame for the specified registered busses.
/// registered to the callback; it is crucial that the processing done in the callback be lightweight and non-blocking.
/// \sa 
/// - AK::SoundEngine::RegisterBusVolumeCallback()
struct AkSpeakerVolumeMatrixBusCallbackInfo
{
	AkUniqueID busID;
	AkChannelMask uChannelMask;
	AkSpeakerVolumes * pVolumes[ AK_VOICE_MAX_NUM_CHANNELS ]; ///< For each channel in the bus, pointer to AkSpeakerVolume describing contribution to destination channels. 
};
...
AK_CALLBACK( void, AkBusCallbackFunc )( 
	AkSpeakerVolumeMatrixBusCallbackInfo* in_pCallbackInfo 
	);
