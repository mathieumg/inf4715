...
VCNPolygon::VCNPolygon(void) 
{
  ...
  m_NumI    = 0;
  m_Flag    = 0;
  memset(&m_Aabb, 0, sizeof(VCNAabb));
}
...
void VCNPolygon::Set(const Vector3 *pPoints, int nNumP, const unsigned int *pIndis, int nNumI)
{
  Vector3 vcEdge0, 
        vcEdge1;
  ...
  m_pPoints = new Vector3[nNumP];
  m_pIndis  = new unsigned int[nNumI];

  m_NumP = nNumP;
  m_NumI = nNumI;

  memcpy(m_pPoints, pPoints, sizeof(Vector3)*nNumP);
  ...
  m_Plane.m_vcN = CrossProduct(vcEdge0, vcEdge1);
...
void VCNPolygon::CalcBoundingBox(void) 
{
  Vector3 vcMax, vcMin;
...
void VCNPolygon::SwapFaces(void)
{
...
void VCNPolygon::Clip(const VCNPlane &Plane, VCNPolygon *pFront, VCNPolygon *pBack)
{
  if (!pFront && !pBack) 
    return;

  Vector3 vcHit, vcA, vcB;
  VCNRay    Ray;

  // cast away const
  VCNPlane  *pPlane = ((VCNPlane*)&Plane);

  VCNUInt nNumFront=0,     // number points on frontside
      nNumBack=0,      // number points on backside
      nLoop=0,
      nCurrent=0;

  std::vector<Vector3> pvcFront; pvcFront.resize( m_NumP*3 );
  std::vector<Vector3> pvcBack; pvcFront.resize( m_NumP*3 );
  ...
  for (nLoop=1; nLoop < (VCNUInt)(m_NumP+1); nLoop++) 
  {
    if (nLoop == m_NumP) 
      nCurrent = 0;
    else 
    ...
    else 
    {
      ...
      float fLength = Ray.m_vcDir.Length();
  ...
  std::vector<unsigned int> pnFront;
  std::vector<unsigned int> pnBack;

  if (nNumFront > 2) 
  {
    pnFront.resize((nNumFront-2)*3);
  ...
  if (pFront && !pnFront.empty()) 
  {
    pFront->Set(&pvcFront[0], nNumFront, &pnFront[0], (nNumFront-2)*3);
  ...
  if (pBack && !pnBack.empty()) 
  {
    pBack->Set(&pvcBack[0], nNumBack, &pnBack[0],  (nNumBack-2)*3);
...
void VCNPolygon::Clip(const VCNAabb &aabb) 
{
  VCNPolygon BackPoly, ClippedPoly;
  VCNPlane   Planes[6];
  bool       bClipped=false;

  // cast away const
  VCNAabb *pAabb = ((VCNAabb*)&aabb);
  ...
  for (int i=0; i<6; i++) 
  {
    if ( Planes[i].Classify(ClippedPoly) == VCNCLIPPED )
    {
...
int VCNPolygon::Cull(const VCNAabb &aabb) 
{
  VCNPlane Planes[6];
  ...
  for (int p=0; p<6; p++)
  {
    ...
    if (bFirst) 
    {
      ...
      if (nInside  == m_NumP) return VCNVISIBLE;
    }
...
void VCNPolygon::CopyOf(const VCNPolygon &Poly) 
{
...
bool VCNPolygon::Intersects(const VCNRay &Ray, bool bCull, float *t)
{
   // cast away const
   VCNRay *pRay = (VCNRay*)&Ray;
...
bool VCNPolygon::Intersects(const VCNRay &Ray, bool bCull, float fL, float *t) 
{
  // cast away const
  VCNRay *pRay = (VCNRay*)&Ray;
